...
const AdminUserWorkspace = ({ firebase, showToast, targetUser, onBack }) => {
  ...
  function calcStudent(entries, proj) {
    ...
    const subResults = entries.map((entry, i) => {
      ...
      const gp = gpFn(r.pct), grade = gradeFn(gp);
      return { ...r, gp, grade, pass: r.pct >= Number(thresh) };
    });
  ...
  const addStudent = async () => {
    ...
    try {
      ...
      await firebase.db.collection("users").doc(uid).collection("projects").doc(pr.id).update({ students: pr.students });
                  ...
                  {Array(Number(subjectCount)).fill(null).map((_, i) => {
                    ...
                    return (
                      <div key={i} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 12, padding: "12px 14px" }}>
                        <input placeholder={`Subject ${i + 1} name`} value={subjectNames[i] || ""} onChange={e => { const u = [...subjectNames]; u[i] = e.target.value; setSubjectNames(u); }} style={{ marginBottom: 12 }} />
                        <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--text3)", marginBottom: 6 }}>Skip Components</div>
                        <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
                          {[{k:"skipCA",l:"CA",icon:"📝",col:"#818cf8"},{k:"skipMCQ",l:"MCQ",icon:"💬",col:"#38bdf8"},{k:"skipCQ",l:"CQ",icon:"✍️",col:"#fb923c"}].map(({k,l,icon,col}) => (
                            <label key={k} onClick={()=>{ const u=[...subjectSettings]; u[i]={...(u[i]||{caFull:30,examFull:100,examPct:70}),[k]:!ss[k]}; setSubjectSettings(u); }} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"8px 4px",borderRadius:12,border:`2px solid ${ss[k]?col:"var(--border)"}`,background:ss[k]?`color-mix(in srgb,${col} 15%,var(--surface))`:"var(--surface3)",cursor:"pointer",transition:"all 0.18s",userSelect:"none",position:"relative" }}>
                              <span style={{ fontSize: 16 }}>{icon}</span>
                              <span style={{ fontSize: 10.5, fontWeight: 800, color: ss[k]?col:"var(--text3)" }}>{l}</span>
                              <span style={{ fontSize: 9, fontWeight: 700, color: ss[k]?"#ef4444":"var(--text3)", letterSpacing: "0.05em" }}>{ss[k]?"SKIP":"ON"}</span>
                              {ss[k]&&<div style={{ position:"absolute",top:-6,right:-6,width:14,height:14,borderRadius:"50%",background:"#ef4444",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#fff",fontWeight:800 }}>✕</div>}
                              <input type="checkbox" checked={!!ss[k]} onChange={()=>{}} style={{ display:"none" }}/>
                            </label>
                          ))}
                        </div>
                        <div style={{ display: "grid", gridTemplateColumns: `${ss.skipCA?"":"1fr "}1fr 1fr`, gap: 8 }}>
                          {!ss.skipCA&&<div>
                ...
                <Card style={{ marginBottom: 14, borderRadius: 18 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
                    {Array(activeProject.subjectCount).fill(null).map((_, si) => {
                      ...
                      const passLive = r.pct >= (activeProject.passThreshold ?? 33);
...
export default function Home() {
  ...
  // ── Calculation ───────────────────────────────────────────────────
  function calcSubject(entry, ss, proj){
    const skipCA  = !!ss.skipCA;
    const skipMCQ = !!ss.skipMCQ;
    const skipCQ  = !!ss.skipCQ;
    const caFull   = skipCA ? 0 : Number(ss.caFull??30);
    ...
    const assignTotal=(entry.assignments||[]).reduce((s,v)=>s+Number(v||0),0);
    const ca   = skipCA ? 0 : Math.round(maxAssign>0?(assignTotal/maxAssign)*caFull:0);
    const mcq  = skipMCQ ? 0 : Number(entry.mcq||0);
    const cq   = skipCQ  ? 0 : Number(entry.cq||0);
    const rawExam = mcq+cq;
    const b    = Math.round(rawExam*(examPct/100));
    const ab   = ca+b;
    const subFull = caFull+(examPct/100)*examFull;
    const pct  = subFull>0?(ab/subFull)*100:0;
    return {ca,mcq,cq,rawExam,b,ab,subFull,pct,assignTotal,assignments:entry.assignments||[],skipCA,skipMCQ,skipCQ};
  }
                      ...
                      <div style={{display:"flex",flexDirection:"column",gap:10}}>
                        {Array(Number(subjectCount)).fill(null).map((_,i)=>{
                          ...
                          return(
                            <div key={i} style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:12,padding:"12px 14px"}}>
                              <input placeholder={`Subject ${i+1} name`} value={subjectNames[i]||""} onChange={e=>{const u=[...subjectNames];u[i]=e.target.value;setSubjectNames(u);}} style={{marginBottom:12}}/>
                              {/* Skip toggles — beautiful pill buttons */}
                              <div style={{fontSize:9,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--text3)",marginBottom:6}}>Skip Components</div>
                              <div style={{display:"flex",gap:6,marginBottom:12}}>
                                {[{k:"skipCA",l:"CA",icon:"📝",col:"#818cf8"},{k:"skipMCQ",l:"MCQ",icon:"💬",col:"#38bdf8"},{k:"skipCQ",l:"CQ",icon:"✍️",col:"#fb923c"}].map(({k,l,icon,col})=>(
                                  <label key={k} onClick={()=>updateSubjectSetting(i,k,!ss[k])} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"8px 4px",borderRadius:12,border:`2px solid ${ss[k]?col:"var(--border)"}`,background:ss[k]?`color-mix(in srgb,${col} 15%,var(--surface))`:"",cursor:"pointer",transition:"all 0.18s",userSelect:"none",position:"relative"}}>
                                    <span style={{fontSize:16}}>{icon}</span>
                                    <span style={{fontSize:10.5,fontWeight:800,color:ss[k]?col:"var(--text3)"}}>{l}</span>
                                    <span style={{fontSize:9,fontWeight:700,color:ss[k]?"#ef4444":"var(--text3)",letterSpacing:"0.05em"}}>{ss[k]?"SKIP":"ON"}</span>
                                    {ss[k]&&<div style={{position:"absolute",top:-6,right:-6,width:14,height:14,borderRadius:"50%",background:"#ef4444",display:"flex",alignItems:"center",justifyContent:"center",fontSize:9,color:"#fff",fontWeight:800}}>✕</div>}
                              ...
                              <div style={{marginTop:8,fontSize:11,color:"var(--text2)",display:"flex",gap:12,flexWrap:"wrap"}}>
                                {!ss.skipCA&&<span>CA: <b style={{color:"#818cf8"}}>{ss.caFull??30}</b></span>}
                                {(!ss.skipMCQ||!ss.skipCQ)&&<span>B: <b style={{color:"var(--amber)"}}>{((Number(ss.examPct)||70)/100*(Number(ss.examFull)||100)).toFixed(0)}</b></span>}
                                <span>Max: <b style={{color:"var(--sky)"}}>{Math.round((ss.skipCA?0:Number(ss.caFull??30))+((Number(ss.examPct)||70)/100)*Number(ss.examFull||100))}</b></span>
                              </div>
                            </div>
                          );
                        })}
                        ...
                        {Array(activeProject.subjectCount).fill(null).map((_,si)=>{
                          ...
                          return(
                            <div key={si} style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:14,overflow:"hidden"}}>
                              ...
                              <div style={{padding:"8px 12px",display:"flex",flexDirection:"column",gap:6}}>
                                {!ss.skipCA&&Array(na).fill(null).map((_,ai)=>(
                                  <div key={ai} style={{display:"flex",alignItems:"center",gap:10}}>
                                ...
                                {!ss.skipMCQ&&<div style={{display:"flex",alignItems:"center",gap:10}}>
                                  <span style={{fontSize:11.5,color:"var(--sky)",fontWeight:600,minWidth:90}}>MCQ / Oral</span>
                                  <input type="number" placeholder="0" min={0} value={entry.mcq??""} onChange={e=>setSubjectEntries(prev=>prev.map((en,s)=>s===si?{...en,mcq:e.target.value}:en))} style={{flex:1,textAlign:"center"}}/>
                                  <span style={{fontSize:11,color:"var(--text3)",minWidth:28}}>/{ss.examFull||100}</span>
                                </div>}
                                {!ss.skipCQ&&<div style={{display:"flex",alignItems:"center",gap:10}}>
                                  <span style={{fontSize:11.5,color:"var(--amber)",fontWeight:600,minWidth:90}}>CQ / Written</span>
                                  <input type="number" placeholder="0" min={0} value={entry.cq??""} onChange={e=>setSubjectEntries(prev=>prev.map((en,s)=>s===si?{...en,cq:e.target.value}:en))} style={{flex:1,textAlign:"center"}}/>
                                  <span style={{fontSize:11,color:"var(--text3)",minWidth:28}}>/{ss.examFull||100}</span>
                                </div>}
                                <div style={{fontSize:11,color:"var(--text2)",display:"flex",gap:10,flexWrap:"wrap",paddingTop:4,borderTop:"1px solid var(--border)"}}>
                                  {!ss.skipCA&&<span>CA: <b style={{color:"#818cf8",fontFamily:"var(--font-mono)"}}>{r.ca}</b></span>}
                                  {(!ss.skipMCQ||!ss.skipCQ)&&<span>Exam: <b style={{color:"var(--sky)",fontFamily:"var(--font-mono)"}}>{r.rawExam}</b></span>}
                                  <span>B: <b style={{color:"var(--amber)",fontFamily:"var(--font-mono)"}}>{r.b}</b></span>
