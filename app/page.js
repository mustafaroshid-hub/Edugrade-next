...
const downloadMarksheet = async (student, project, showToast) => {
  ...
  const GREEN=[16,185,129],RED=[239,68,68],INDIGO=[99,102,241],PURPLE=[139,92,246];
  ...
  // Logo
  const lx=14,ly=10,ls=16;
  doc.setFillColor(...SKY); doc.roundedRect(lx,ly,ls,ls,3,3,"F");
  doc.setFillColor(255,255,255); doc.rect(lx+2,ly+2.5,5,10.5,"F");
  doc.setFillColor(224,242,254); doc.rect(lx+7.5,ly+2.5,5.5,10.5,"F");
  doc.setDrawColor(...SKY_DK); doc.setLineWidth(0.6);
  doc.line(lx+7.2,ly+2.5,lx+7.2,ly+13);
  doc.setLineWidth(1);
  doc.line(lx+8.8,ly+8.2,lx+10,ly+9.5); doc.line(lx+10,ly+9.5,lx+12.2,ly+6.5);
  doc.setFillColor(255,255,255); doc.circle(lx+13.5,ly+2.5,1.2,"F");
  doc.setFont("helvetica","bold"); doc.setFontSize(17); doc.setTextColor(...DARK);
  doc.text("EduGrade",lx+ls+4,ly+7);
  doc.setFont("helvetica","normal"); doc.setFontSize(8.5); doc.setTextColor(...GREY);
  doc.text("Student Marksheet",lx+ls+4,ly+12.5);

  // Institution + class badge (top right)
  ...
  doc.setFillColor(...SKY_LT); doc.roundedRect(W-72,ly-1,58,20,3,3,"F");
  doc.setDrawColor(...SKY_MID); doc.setLineWidth(0.5); doc.roundedRect(W-72,ly-1,58,20,3,3,"S");
  doc.setFont("helvetica","normal"); doc.setFontSize(6.5); doc.setTextColor(...GREY);
  doc.text("INSTITUTION",W-68,ly+4);
  doc.text("CLASS / EXAM",W-68,ly+10.5);
  doc.setFont("helvetica","bold"); doc.setFontSize(7.5); doc.setTextColor(...DARK);
  doc.text((institution||examLabel).slice(0,18),W-48,ly+4);
  doc.text((className?" "+className+" · ":"")+examLabel.slice(0,14),W-48,ly+10.5);
  ...
  // Student Info header
  const siY=35;
  doc.setFillColor(...LGREY); doc.roundedRect(14,siY,W-28,7,2,2,"F");
  doc.setFont("helvetica","bold"); doc.setFontSize(7); doc.setTextColor(...GREY);
  doc.text("STUDENT INFORMATION",18,siY+4.8);

  const r1=siY+14,r2=siY+22;
  doc.setFont("helvetica","normal"); doc.setFontSize(7.5); doc.setTextColor(...GREY);
  doc.text("Name",18,r1-0.5); doc.text("Roll No.",18,r2-0.5);
  doc.setFont("helvetica","bold"); doc.setFontSize(10.5); doc.setTextColor(...DARK);
  const sName=student.name.length>28?student.name.slice(0,28)+"…":student.name;
  doc.text(sName,37,r1+0.5); doc.text(String(student.roll),37,r2+0.5);
  doc.setDrawColor(...SKY_MID); doc.setLineWidth(0.3); doc.line(14,r2+6,W-14,r2+6);

  // Subject marks table
  const subjectNames = project.subjectNames||Array.from({length:project.subjectCount},(_,i)=>`Subject ${i+1}`);
  const numA = project.numAssignments||1;
  const aoof = project.assignOutOf||20;
  ...
  const tableHead = [["#","Subject","Full","Assign (CA)","MCQ+CQ",`Exam (B)`,`A+B (Final)`,"Grade","Pass"]];
  ...
  const tableBody = subjectNames.map((subName,i)=>{
    ...
    const subFull = (ss.caFull||30)+((ss.examPct||70)/100)*(ss.examFull||100);
    return [
      i+1,
      subName||`Subject ${i+1}`,
      subFull.toFixed(0),
      (sr.ca!=null?Math.round(sr.ca):"-"),
      (sr.rawExam!=null?sr.rawExam:"-"),
      (sr.b!=null?Math.round(sr.b):"-"),
      (sr.ab!=null?Math.round(sr.ab):"-"),
      sr.grade||"-",
      sr.pass?"PASS":"FAIL",
    ];
  });
    ...
    didParseCell:(data)=>{
      if(data.section==="body"){
        if(data.column.index===6){
          ...
          const sf=(ss.caFull||30)+((ss.examPct||70)/100)*(ss.examFull||100);
          const pct=sf>0?(v/sf)*100:0;
          data.cell.styles.textColor=pct>=80?GREEN:pct<33?RED:DARK;
        }
        if(data.column.index===7){const g=data.cell.raw;data.cell.styles.textColor=["A+","A","A-"].includes(g)?GREEN:g==="F"?RED:DARK;}
        if(data.column.index===8){data.cell.styles.textColor=data.cell.raw==="PASS"?GREEN:RED;}
        if(data.column.index===3){data.cell.styles.textColor=INDIGO;}
        if(data.column.index===5){data.cell.styles.textColor=PURPLE;}
      }
    },
  ...
  const tileRowY=rsY+11,tileH=22;
  [
    {label:"GRADE",    value:student.grade,      color:INDIGO},
    {label:"GPA",      value:String(student.gpa), color:INDIGO},
    {label:"TOTAL",    value:String(student.total!=null?Math.round(student.total):"-"), color:SKY},
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:16}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                      <div>
                        ...
                        <input placeholder="e.g. Al-Hera" value={institution} onChange={e=>setInstitution(e.target.value)}/>
                      </div>
                      <div>
                        <Label>Class / Grade</Label>
                        <input placeholder="e.g. Class 8" value={className} onChange={e=>setClassName(e.target.value)}/>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input type="number" placeholder="e.g. 6" value={subjectCount} min={1} max={20}
                        onChange={e=>{
