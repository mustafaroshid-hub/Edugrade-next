        ...
        <div style={{ padding: "20px 22px 0", borderBottom: "1.5px solid var(--border)", paddingBottom: 0 }}>
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: -1 }}>
            <button style={{
                ...
                border: "1.5px solid var(--border)",
                borderBottom: "1.5px solid var(--surface)",
                background: "var(--surface)",
                color: "var(--text)",
...
const downloadMarksheet = async (student, project, showToast) => {
  ...
  [
    ...
    { label: "Total GP", value: student.hasZeroGP ? "FAIL" : String((student.subjectResults||[]).reduce((s,r)=>s+(r.gp||0),0).toFixed(2)), large: true },
    ...
    if (large) doc.setTextColor(...(value==="FAIL"?RED:[14, 100, 180])); else doc.setTextColor(...DARK);
  ...
  const gpRowY = rsY + 26;
  const gpCols = [
    { label: "GPA",            value: student.hasZeroGP ? "0.00" : String(student.gpa) },
    { label: "LG",             value: student.hasZeroGP ? "F" : (student.grade || gradeFn(student.gpa)) },
...
export default function Home() {
  ...
  function calcStudent(entries, proj){
    ...
    const hasZeroGP=subResults.some(r=>r.gp===0);
    const avgGP=subResults.reduce((s,r)=>s+r.gp,0)/(subResults.length||1);
    const gpa=hasZeroGP?0:Math.min(parseFloat(avgGP.toFixed(2)),5);
    const grade=fail?"F":gradeFn(gpa);
    return {gpa,grade,total,subResults,status:fail?"FAIL":"PASS",hasZeroGP};
  }
  ...
  const deleteAccount=async()=>{
    try{
      ...
      // If deleteUser fails (needs recent login), sign out anyway
      showToast("✗ "+e.message);
      await firebase.auth.signOut();
    }
  };
              ...
              <div>
                ...
                <Card style={{marginBottom:14}}>
                  ...
                  <div style={{display:"flex",gap:12,marginBottom:10}}>
                    ...
                    <div style={{flex:1,textAlign:"center",padding:"12px 8px",background:"var(--surface2)",borderRadius:12}}>
                      <div style={{fontWeight:800,fontSize:22,color:"var(--indigo)"}}>{projects.reduce((a,p)=>a+(p.students?.filter(s=>s.status==="PASS").length||0),0)}</div>
                      <div style={{fontSize:11,color:"var(--text2)",marginTop:2}}>Passed</div>
                    </div>
                  </div>
                  <div style={{height:1,background:"var(--border)",margin:"10px 0"}}/>
                  <div style={{fontSize:11,color:"var(--text2)",textAlign:"center"}}>
                    Member since {user?.metadata?.creationTime ? new Date(user.metadata.creationTime).toLocaleDateString("en-GB",{month:"long",year:"numeric"}) : "—"}
                  </div>
                </Card>

                <Card style={{marginBottom:14}}>
                  <div style={{fontSize:10,fontWeight:700,color:"var(--text2)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:14}}>Default Grading</div>
                  <div style={{fontSize:12,color:"var(--text2)",marginBottom:10,lineHeight:1.5}}>Current pass threshold applies to all new projects unless overridden per project.</div>
                  {[
                    {range:"80–100",gp:5,lg:"A+"},
                    {range:"70–79",gp:4,lg:"A"},
                    {range:"60–69",gp:3.5,lg:"A-"},
                    {range:"50–59",gp:3,lg:"B"},
                    {range:"40–49",gp:2,lg:"C"},
                    {range:"33–39",gp:1,lg:"D"},
                    {range:"00–32",gp:0,lg:"F"},
                  ].map(({range,gp,lg})=>(
                    <div key={range} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid var(--border)"}}>
                      <span style={{fontSize:12,fontWeight:600}}>{range}</span>
                      <span style={{fontSize:12,color:"var(--text2)"}}>GP: {gp}</span>
                      <span style={{fontSize:12,fontWeight:700,color:lg==="F"?"var(--red)":lg.startsWith("A")?"var(--green)":"var(--sky)"}}>{lg}</span>
                    </div>
                  ))}
                </Card>

                <Card style={{marginBottom:14}}>
                  <div style={{fontSize:10,fontWeight:700,color:"var(--text2)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:14}}>About</div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <span style={{fontSize:13,color:"var(--text2)"}}>App</span>
                    <span style={{fontSize:13,fontWeight:600}}>EduGrade</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}>
                    <span style={{fontSize:13,color:"var(--text2)"}}>Version</span>
                    <span style={{fontSize:13,fontWeight:600}}>1.0.0</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between"}}>
                    <span style={{fontSize:13,color:"var(--text2)"}}>Platform</span>
                    <span style={{fontSize:13,fontWeight:600}}>Web</span>
                  </div>
                </Card>
                ...
                <Card style={{marginBottom:80,border:"1.5px solid color-mix(in srgb,var(--red) 30%,var(--border))"}}>
                  ...
                  <div style={{fontSize:12,color:"var(--text2)",marginBottom:12,lineHeight:1.5}}>Permanently delete your account and all projects and student data. This cannot be undone.</div>
