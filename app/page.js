...
const Dock = ({ page, go, isAdmin, onAdminOpen }) => {
  const tabs = [
    ...
    { id: "app",     icon: <IcoEdit s={18}/>,    label: "Entry" },
    ...
    <nav style={{
      ...
      {tabs.map(t => {
        ...
        return (
          <button key={t.id} onClick={() => go(t.id)} style={{
            ...
            <span style={{ display:"flex", alignItems:"center", justifyContent:"center", transition: "all 0.18s" }}>{t.icon}</span>
      ...
      {isAdmin && (
        <button onClick={onAdminOpen} style={{
          ...
          <span style={{display:"flex",alignItems:"center",justifyContent:"center"}}><IcoLightning s={18} c="var(--yellow)"/></span>
...
// ─── SVG Icons ────────────────────────────────────────────────────────────────
const Icon = ({ d, size=20, stroke="currentColor", fill="none", sw=2, extra="" }) => (
...
const IcoHome    = ({s=20,c="currentColor"}) => <Icon size={s} stroke={c} d={["M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"]} extra={<polyline points="9 22 9 12 15 12 15 22"/>}/>;
const IcoPlus    = ({s=20,c="currentColor"}) => <Icon size={s} stroke={c} d={["M12 5v14","M5 12h14"]}/>;
const IcoEdit    = ({s=20,c="currentColor"}) => <Icon size={s} stroke={c} d={["M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7","M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"]}/>;
const IcoBar     = ({s=20,c="currentColor"}) => <Icon size={s} stroke={c} d={["M18 20V10","M12 20V4","M6 20v-6"]}/>;
const IcoTrophy  = ({s=20,c="currentColor"}) => <Icon size={s} stroke={c} d={["M8 21h8","M12 17v4","M7 4v6a5 5 0 0 0 10 0V4"]} extra={<><line x1="3" y1="8" x2="7" y2="8"/><line x1="17" y1="8" x2="21" y2="8"/></>}/>;
const IcoSettings= ({s=20,c="currentColor"}) => <Icon size={s} stroke={c} d={["M12 15a3 3 0 1 0 0-6 3 3 0 0 0 0 6z","M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"]}/>;
const IcoDoc     = ({s=20,c="currentColor"}) => <Icon size={s} stroke={c} d={["M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z","M14 2v6h6","M16 13H8","M16 17H8","M10 9H8"]}/>;
const IcoUsers   = ({s=20,c="currentColor"}) => <Icon size={s} stroke={c} d={["M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"]} extra={<><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></>}/>;
const IcoLightning=({s=20,c="currentColor"}) => <Icon size={s} stroke={c} fill={c} d={["M13 2L3 14h9l-1 8 10-12h-9l1-8z"]}/>;
...
export default function Home() {
  ...
  const deleteProject=async(id)=>{
    setConfirmModal({title:"Delete Project",msg:"Delete this project and ALL student data? This cannot be undone.",ok:"Delete",onOk:async()=>{
    try{
  ...
  const addStudent=async()=>{
    ...
    try{
      ...
      if(idx!==-1){
        if(!window.confirm("Student exists. Update this student's data?")){ setSaving(false); return; }
        pr.students[idx]=obj;
      ...
      <div style={{maxWidth:520,margin:"0 auto",padding:"0 16px 120px",minHeight:"100vh"}}>
        ...
        {fbReady&&authReady&&user&&!fbError&&!isAccountBlocked&&(
          <>
            ...
            {/* ── HOME ── */}
            {page==="home"&&(
              <div style={{animation:"slideIn 0.22s ease",paddingTop:24}}>
                {/* Greeting */}
                <div style={{marginBottom:24}}>
                  ...
                  <button onClick={signOut} style={{marginTop:8,background:"none",border:"1px solid var(--border)",borderRadius:8,padding:"4px 12px",fontSize:11,color:"var(--text2)",cursor:"pointer",fontWeight:600}}>Sign Out</button>
                </div>
                  ...
                  return(
                    <div key={p.id} className="project-card" style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:16,marginBottom:9,overflow:"hidden"}}>
                      <div style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px"}}>
                        <div style={{width:46,height:46,borderRadius:14,flexShrink:0,background:"color-mix(in srgb,var(--sky) 10%,var(--surface2))",border:"1.5px solid color-mix(in srgb,var(--sky) 18%,var(--border))",display:"flex",alignItems:"center",justifyContent:"center"}}>
                          <IcoDoc s={22} c="var(--sky)"/>
                        </div>
                      ...
                      <Card style={{border:`1.5px solid color-mix(in srgb,${gradeColor(viewStudent.grade)} 28%,var(--border))`,background:`color-mix(in srgb,${gradeColor(viewStudent.grade)} 4%,var(--surface))`,animation:"up 0.25s ease",borderRadius:18,marginBottom:12}}>
                        ...
                        <div style={{display:"flex",gap:9}}>
                          <Btn variant="ghost" size="sm" style={{flex:1}} onClick={()=>{
                            // Load student data back into entry form for editing
                            ...
                            setViewStudent(null);
                            go("app");
                            showToast("✏ Edit mode — change marks and save");
                          }}>✏ Edit</Btn>
              ...
              <div style={{animation:"slideIn 0.22s ease",paddingTop:24}}>
                ...
                <Card style={{marginBottom:14}}>
                  <div style={{display:"flex",alignItems:"center",gap:14}}>
                    <div style={{width:52,height:52,borderRadius:16,background:"linear-gradient(135deg,var(--sky),var(--indigo))",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:20,color:"#fff",flexShrink:0}}>
                ...
                {/* Appearance */}
                <Card style={{marginBottom:14}}>
                  <div style={{fontWeight:700,fontSize:13,marginBottom:14,color:"var(--text2)",textTransform:"uppercase",letterSpacing:"0.08em",fontSize:10}}>Appearance</div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div>
                      <div style={{fontWeight:600,fontSize:14}}>Dark Mode</div>
                      <div style={{color:"var(--text2)",fontSize:12,marginTop:2}}>Switch between light and dark</div>
                    </div>
                    <button onClick={()=>setDark(d=>!d)} style={{
                      width:48,height:26,borderRadius:13,border:"none",cursor:"pointer",
                      background:dark?"var(--sky)":"var(--border)",position:"relative",transition:"background 0.2s",flexShrink:0,
                    }}>
                      <div style={{
                        width:20,height:20,borderRadius:"50%",background:"#fff",
                        position:"absolute",top:3,left:dark?24:3,transition:"left 0.2s",
                ...
                {/* Stats */}
                <Card style={{marginBottom:14}}>
                  <div style={{fontWeight:700,fontSize:10,marginBottom:14,color:"var(--text2)",textTransform:"uppercase",letterSpacing:"0.08em"}}>Your Data</div>
                  <div style={{display:"flex",gap:12}}>
                    ...
                    <div style={{flex:1,textAlign:"center",padding:"12px 8px",background:"var(--surface2)",borderRadius:12}}>
                      <div style={{fontWeight:800,fontSize:22,color:"var(--green)"}}>{projects.reduce((a,p)=>a+p.students.length,0)}</div>
