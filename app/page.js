"use client";
import { initializeApp, getApps } from 'firebase/app';
import { getAuth } from 'firebase/auth';
import { getFirestore } from 'firebase/firestore';
import { useEffect, useState, useCallback, useRef } from "react";

const FIREBASE_CONFIG = {
  apiKey: "AIzaSyAO8YIGatrM_OsiDYKOpObnCDyz5cbssm4",
  authDomain: "edugrade-42851.firebaseapp.com",
  projectId: "edugrade-42851",
  storageBucket: "edugrade-42851.firebasestorage.app",
  messagingSenderId: "966927749579",
  appId: "1:966927749579:web:1b3aa2aa1746d3b2548057",
  measurementId: "G-LMCWHSE517"
};

const ADMIN_EMAIL = "mustafaroshid@gmail.com";

const DEFAULT_GRADING = [
  { min:80, gp:5, grade:"A+" }, { min:70, gp:4, grade:"A" },
  { min:60, gp:3.5, grade:"A-" }, { min:50, gp:3, grade:"B" },
  { min:40, gp:2, grade:"C" }, { min:33, gp:1, grade:"D" }, { min:0, gp:0, grade:"F" },
];
const makeGetGP = (scale) => (m) => {
  for (const r of [...(scale||DEFAULT_GRADING)].sort((a,b)=>b.min-a.min)) if (m>=r.min) return r.gp;
  return 0;
};
const makeGetGrade = (scale) => (g) => {
  for (const r of [...(scale||DEFAULT_GRADING)].sort((a,b)=>b.gp-a.gp)) if (g>=r.gp) return r.grade;
  return "F";
};
const getGP    = makeGetGP(null);
const getGrade = makeGetGrade(null);
const gradeColor = (g) =>
  g === "A+" ? "#0ea5e9" : g === "A" ? "#0284c7" : g === "A-" ? "#0369a1"
  : g === "B" ? "#6366f1" : g === "C" ? "#f59e0b" : g === "D" ? "#fb923c" : "#ef4444";

const getMeritRank = (students) =>
  [...students]
    .sort((a, b) => b.gpa - a.gpa || b.total - a.total)
    .reduce((acc, s, i) => { acc[String(s.roll)] = i + 1; return acc; }, {});

// ─── CSS ─────────────────────────────────────────────────────────────────────
const GlobalStyle = ({ dark }) => {
  const css = `
    @import url('https://fonts.googleapis.com/css2?family=Sora:wght@300;400;500;600;700;800&family=JetBrains+Mono:wght@400;500;600&display=swap');
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    :root {
      --bg:       ${dark ? "#060d19" : "#f0f4f9"};
      --surface:  ${dark ? "#0d1626" : "#ffffff"};
      --surface2: ${dark ? "#111e30" : "#f5f8fc"};
      --surface3: ${dark ? "#172338" : "#eaeff6"};
      --border:   ${dark ? "#1e2f47" : "#dde4ef"};
      --text:     ${dark ? "#e2e8f5" : "#0b1729"};
      --text2:    ${dark ? "#5b7299" : "#7a8ea8"};
      --text3:    ${dark ? "#354d6e" : "#a8b8cc"};
      --sky:      #0ea5e9;
      --sky2:     #38bdf8;
      --sky3:     #0284c7;
      --sky4:     #075985;
      --indigo:   #6366f1;
      --green:    #10b981;
      --red:      #ef4444;
      --yellow:   #f59e0b;
      --amber:    #f97316;
      --glow:     ${dark ? "rgba(14,165,233,0.15)" : "rgba(14,165,233,0.1)"};
      --font-sans: 'Sora', sans-serif;
      --font-mono: 'JetBrains Mono', monospace;
    }
    body {
      background: var(--bg); color: var(--text);
      font-family: var(--font-sans); min-height: 100vh;
      transition: background 0.4s, color 0.4s;
      -webkit-font-smoothing: antialiased;
    }
    ::-webkit-scrollbar { width: 4px; }
    ::-webkit-scrollbar-track { background: transparent; }
    ::-webkit-scrollbar-thumb { background: var(--border); border-radius: 4px; }

    input[type="text"], input[type="number"], input[type="email"], input[type="password"], input:not([type]) {
      width: 100%; padding: 11px 14px; border-radius: 10px;
      border: 1.5px solid var(--border); background: var(--surface2);
      color: var(--text); font-family: var(--font-sans); font-size: 13.5px;
      font-weight: 400; outline: none; transition: border-color 0.2s, box-shadow 0.2s, background 0.2s;
    }
    input:focus {
      border-color: var(--sky);
      background: var(--surface);
      box-shadow: 0 0 0 3px var(--glow);
    }
    input::placeholder { color: var(--text3); }
    button { cursor: pointer; font-family: var(--font-sans); }
    table { width: 100%; border-collapse: collapse; }
    th {
      background: var(--surface2); color: var(--text2);
      font-size: 10px; font-weight: 700; letter-spacing: 0.08em;
      text-transform: uppercase; padding: 10px 14px; text-align: left;
      white-space: nowrap; border-bottom: 1.5px solid var(--border);
    }
    td { padding: 11px 14px; border-bottom: 1px solid var(--border); font-size: 13px; }
    tr:last-child td { border-bottom: none; }
    tr:hover td { background: color-mix(in srgb, var(--sky) 4%, transparent); }

    @keyframes up { from { opacity:0; transform:translateY(18px); } to { opacity:1; transform:translateY(0); } }
    @keyframes splashFade { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:translateY(0)} }
    @keyframes splashOut { from{opacity:1} to{opacity:0} }
    @keyframes toastPop { 0%{opacity:0;transform:translateX(-50%) translateY(-10px) scale(0.92);}100%{opacity:1;transform:translateX(-50%) translateY(0) scale(1);} }
    @keyframes spin { from{transform:rotate(0deg);}to{transform:rotate(360deg);} }
    @keyframes badgePop { 0%{transform:scale(0);}70%{transform:scale(1.2);}100%{transform:scale(1);} }
    @keyframes fadeIn { from{opacity:0;}to{opacity:1;} }
    @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.35} }
    @keyframes pulseGlow {
      0%,100% { box-shadow: 0 0 0 0 rgba(14,165,233,0); }
      50% { box-shadow: 0 0 20px 6px rgba(14,165,233,0.18); }
    }
    @keyframes shimmer {
      0% { background-position: -400px 0; }
      100% { background-position: 400px 0; }
    }
    @keyframes orbit {
      from { transform: rotate(0deg); }
      to { transform: rotate(360deg); }
    }
    @keyframes counterOrbit {
      from { transform: rotate(0deg); }
      to { transform: rotate(-360deg); }
    }
    @keyframes floatUp {
      0%,100% { transform: translateY(0px); }
      50% { transform: translateY(-6px); }
    }
    @keyframes gradientShift {
      0%,100% { background-position: 0% 50%; }
      50% { background-position: 100% 50%; }
    }

    .hover-lift { transition: transform 0.2s, box-shadow 0.2s; }
    .hover-lift:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(0,0,0,0.15); }

    .project-card { transition: all 0.2s; }
    .project-card:hover { border-color: color-mix(in srgb, var(--sky) 40%, var(--border)) !important; }

    input[type=number]::-webkit-inner-spin-button,
    input[type=number]::-webkit-outer-spin-button { -webkit-appearance: none; margin: 0; }
  `;
  return <style>{css}</style>;
};

// ─── EduGrade Logo SVG Component ─────────────────────────────────────────────
const EduGradeLogo = ({ size = 44, style = {} }) => (
  <svg width={size} height={size} viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg" style={style}>
    {/* Graduation cap shape */}
    <rect width="44" height="44" rx="12" fill="url(#logoGrad)" />
    {/* Book pages */}
    <rect x="9" y="14" width="11" height="16" rx="2" fill="rgba(255,255,255,0.25)" />
    <rect x="11" y="14" width="11" height="16" rx="2" fill="rgba(255,255,255,0.35)" />
    {/* Star/sparkle top right */}
    <circle cx="33" cy="12" r="3" fill="rgba(255,255,255,0.9)" />
    <path d="M33 9v6M30 12h6" stroke="rgba(14,165,233,0.5)" strokeWidth="1.5" strokeLinecap="round"/>
    {/* Check mark */}
    <path d="M19 22.5l2.5 2.5 5-5" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
    {/* Bottom bar */}
    <rect x="9" y="33" width="26" height="3" rx="1.5" fill="rgba(255,255,255,0.2)" />
    <rect x="9" y="33" width="18" height="3" rx="1.5" fill="rgba(255,255,255,0.6)" />
    <defs>
      <linearGradient id="logoGrad" x1="0" y1="0" x2="44" y2="44" gradientUnits="userSpaceOnUse">
        <stop offset="0%" stopColor="#38bdf8"/>
        <stop offset="100%" stopColor="#0369a1"/>
      </linearGradient>
    </defs>
  </svg>
);

// ─── Primitives ───────────────────────────────────────────────────────────────
const Card = ({ children, style = {}, animate, glass }) => (
  <div style={{
    background: glass ? "color-mix(in srgb, var(--surface) 88%, transparent)" : "var(--surface)",
    border: "1.5px solid var(--border)",
    borderRadius: 18,
    padding: 20,
    animation: animate ? "up 0.3s ease" : "none",
    backdropFilter: glass ? "blur(16px)" : "none",
    ...style,
  }}>{children}</div>
);

const Label = ({ children }) => (
  <div style={{
    fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase",
    color: "var(--text2)", marginBottom: 8
  }}>{children}</div>
);

const Btn = ({ children, onClick, variant = "primary", style = {}, disabled, loading, size = "md" }) => {
  const sizes = {
    sm: { padding: "6px 13px", fontSize: 11.5, borderRadius: 9 },
    md: { padding: "11px 20px", fontSize: 13.5, borderRadius: 11 },
    lg: { padding: "14px 24px", fontSize: 14.5, borderRadius: 13 },
  };
  const base = {
    display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7,
    border: "none", fontWeight: 700,
    transition: "all 0.18s", cursor: disabled || loading ? "not-allowed" : "pointer",
    opacity: disabled || loading ? 0.5 : 1, letterSpacing: "0.01em",
    ...sizes[size],
  };
  const v = {
    primary: {
      background: "linear-gradient(135deg, #38bdf8 0%, #0284c7 100%)",
      color: "#fff",
      boxShadow: "0 4px 16px rgba(14,165,233,0.35)",
    },
    secondary: {
      background: "var(--surface2)", color: "var(--text)",
      border: "1.5px solid var(--border)"
    },
    danger: {
      background: "color-mix(in srgb,var(--red) 10%,transparent)", color: "var(--red)",
      border: "1.5px solid color-mix(in srgb,var(--red) 22%,transparent)"
    },
    ghost: {
      background: "transparent", color: "var(--text2)",
      border: "1.5px solid var(--border)"
    },
    sky: {
      background: "color-mix(in srgb,var(--sky) 12%,transparent)",
      color: "var(--sky)", border: "1.5px solid color-mix(in srgb,var(--sky) 28%,transparent)"
    },
    green: {
      background: "color-mix(in srgb,var(--green) 12%,transparent)",
      color: "var(--green)", border: "1.5px solid color-mix(in srgb,var(--green) 28%,transparent)"
    },
  };
  return (
    <button style={{ ...base, ...v[variant], ...style }} onClick={onClick} disabled={disabled || loading}>
      {loading
        ? <span style={{ animation: "spin 0.7s linear infinite", display: "inline-block", fontSize: 16 }}>⟳</span>
        : children}
    </button>
  );
};

const Pill = ({ children, color }) => (
  <span style={{
    display: "inline-block", padding: "2px 9px", borderRadius: 20,
    fontSize: 10.5, fontWeight: 700,
    background: `color-mix(in srgb,${color} 13%,transparent)`, color,
  }}>{children}</span>
);

const Toast = ({ msg }) => (
  <div style={{
    position: "fixed", top: 22, left: "50%", transform: "translateX(-50%)",
    background: "var(--surface)", border: "1.5px solid var(--border)",
    color: "var(--text)", padding: "10px 22px", borderRadius: 40, fontSize: 13,
    fontWeight: 600, zIndex: 9999, boxShadow: "0 12px 40px rgba(0,0,0,0.28)",
    animation: "toastPop 0.24s cubic-bezier(0.34,1.56,0.64,1)", whiteSpace: "nowrap",
    display: "flex", alignItems: "center", gap: 8,
  }}>{msg}</div>
);

const StatTile = ({ label, value, color, style = {}, onClick, actionLabel }) => (
  <div style={{
    background: "var(--surface2)", border: "1.5px solid var(--border)",
    borderRadius: 14, padding: "14px 16px", flex: 1, ...style
  }}>
    <div style={{
      fontSize: 10, fontWeight: 700, letterSpacing: "0.09em", textTransform: "uppercase",
      color: "var(--text2)", marginBottom: 7
    }}>{label}</div>
    <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 8 }}>
      <div style={{
        fontSize: 26, fontWeight: 800, color: color || "var(--sky)",
        fontFamily: "var(--font-mono)",
        lineHeight: 1,
      }}>{value}</div>
      {onClick && (
        <button onClick={onClick} style={{
          padding: "5px 11px", borderRadius: 8,
          border: "1.5px solid var(--border)",
          background: "var(--surface)", color: "var(--sky)",
          fontSize: 10.5, fontWeight: 700,
          cursor: "pointer", transition: "all 0.15s",
          flexShrink: 0, whiteSpace: "nowrap",
        }}
          onMouseEnter={e => {
            e.currentTarget.style.background = "color-mix(in srgb,var(--sky) 10%,var(--surface))";
            e.currentTarget.style.borderColor = "var(--sky)";
          }}
          onMouseLeave={e => {
            e.currentTarget.style.background = "var(--surface)";
            e.currentTarget.style.borderColor = "var(--border)";
          }}
        >
          {actionLabel || "View →"}
        </button>
      )}
    </div>
  </div>
);

const Spinner = () => (
  <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "60vh" }}>
    <div style={{
      width: 36, height: 36,
      border: "3px solid var(--border)", borderTop: "3px solid var(--sky)",
      borderRadius: "50%", animation: "spin 0.8s linear infinite"
    }} />
  </div>
);

const SectionHeader = ({ label, title, subtitle }) => (
  <div style={{ marginBottom: 24 }}>
    {label && (
      <div style={{
        fontSize: 10, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase",
        color: "var(--sky)", marginBottom: 7,
        display: "flex", alignItems: "center", gap: 7,
      }}>
        <span style={{
          display: "inline-block", width: 20, height: 2,
          background: "linear-gradient(90deg, var(--sky), transparent)", borderRadius: 2
        }} />
        {label}
      </div>
    )}
    {title && <h2 style={{ fontSize: 26, fontWeight: 800, lineHeight: 1.15, marginBottom: 4, letterSpacing: "-0.02em" }}>{title}</h2>}
    {subtitle && <p style={{ color: "var(--text2)", fontSize: 13 }}>{subtitle}</p>}
  </div>
);

// ─── Student List Modal ───────────────────────────────────────────────────────
const StudentListModal = ({ title, students, color, onClose }) => (
  <div style={{
    position: "fixed", inset: 0, zIndex: 6000,
    background: "rgba(0,0,0,0.6)",
    display: "flex", alignItems: "center", justifyContent: "center",
    backdropFilter: "blur(8px)", padding: "20px 16px",
  }} onClick={e => e.target === e.currentTarget && onClose()}>
    <div style={{
      background: "var(--surface)", border: "1.5px solid var(--border)",
      borderRadius: 22, width: "100%", maxWidth: 400,
      maxHeight: "72vh", display: "flex", flexDirection: "column",
      animation: "up 0.26s cubic-bezier(0.34,1.56,0.64,1)",
      boxShadow: "0 28px 80px rgba(0,0,0,0.4)",
    }}>
      <div style={{
        padding: "18px 20px",
        borderBottom: "1.5px solid var(--border)",
        display: "flex", justifyContent: "space-between", alignItems: "center"
      }}>
        <div>
          <div style={{
            fontSize: 9.5, fontWeight: 700, letterSpacing: "0.12em",
            textTransform: "uppercase", color, marginBottom: 4
          }}>{title}</div>
          <div style={{ fontWeight: 800, fontSize: 18, letterSpacing: "-0.02em" }}>
            {students.length} Student{students.length !== 1 ? "s" : ""}
          </div>
        </div>
        <button onClick={onClose} style={{
          width: 32, height: 32, borderRadius: "50%",
          border: "1.5px solid var(--border)",
          background: "var(--surface2)", color: "var(--text2)",
          fontSize: 17, display: "flex", alignItems: "center", justifyContent: "center",
        }}>×</button>
      </div>
      <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
        {students.length === 0 ? (
          <div style={{ textAlign: "center", color: "var(--text2)", padding: "30px 0", fontSize: 13 }}>
            No students in this category
          </div>
        ) : students.map((s, i) => (
          <div key={s.roll} style={{
            display: "flex", alignItems: "center", gap: 12,
            padding: "10px 14px", borderRadius: 12, marginBottom: 7,
            background: "var(--surface2)", border: "1.5px solid var(--border)",
          }}>
            <div style={{
              width: 30, height: 30, borderRadius: "50%", flexShrink: 0,
              background: `color-mix(in srgb,${color} 18%,transparent)`,
              display: "flex", alignItems: "center", justifyContent: "center",
              fontSize: 12, fontWeight: 800, color,
            }}>{i + 1}</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: 13 }}>{s.name}</div>
              <div style={{ color: "var(--text2)", fontSize: 11 }}>Roll: {s.roll}</div>
            </div>
            <Pill color={color}>{s.grade}</Pill>
          </div>
        ))}
      </div>
    </div>
  </div>
);

// ─── Blocked Modal ────────────────────────────────────────────────────────────
const BlockedModal = ({ onSignOut }) => (
  <div style={{
    position: "fixed", inset: 0, zIndex: 99999,
    background: "rgba(0,0,0,0.8)",
    display: "flex", alignItems: "center", justifyContent: "center",
    backdropFilter: "blur(8px)",
  }}>
    <Card style={{ maxWidth: 340, textAlign: "center", animation: "splashIn 0.4s ease", padding: 32 }}>
      <div style={{ fontSize: 48, marginBottom: 16 }}>🚫</div>
      <h2 style={{ fontWeight: 800, fontSize: 20, marginBottom: 10, letterSpacing: "-0.02em" }}>Account Blocked</h2>
      <p style={{ color: "var(--text2)", fontSize: 13, marginBottom: 24, lineHeight: 1.6 }}>
        Your account has been blocked by the administrator. Contact your admin for assistance.
      </p>
      <Btn onClick={onSignOut} style={{ width: "100%" }}>Sign Out</Btn>
    </Card>
  </div>
);

// ─── Logo Splash ──────────────────────────────────────────────────────────────
const LogoSplash = ({ onDone }) => {
  const [out, setOut] = useState(false);
  useEffect(() => {
    const t1 = setTimeout(() => setOut(true), 1200);
    const t2 = setTimeout(() => onDone(), 1600);
    return () => { clearTimeout(t1); clearTimeout(t2); };
  }, [onDone]);
  return (
    <div style={{
      position:"fixed",inset:0,zIndex:99999,
      background:"var(--bg)",
      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16,
      animation: out ? "splashOut 0.4s ease forwards" : "none",
    }}>
      <div style={{
        width:72,height:72,borderRadius:20,
        background:"linear-gradient(135deg,#38bdf8,#0284c7)",
        display:"flex",alignItems:"center",justifyContent:"center",
        boxShadow:"0 8px 32px rgba(14,165,233,0.3)",
        animation:"splashFade 0.4s ease",
      }}>
        <svg width="38" height="38" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/>
        </svg>
      </div>
      <div style={{animation:"splashFade 0.4s 0.1s ease both"}}>
        <div style={{fontWeight:800,fontSize:24,letterSpacing:"-0.03em",textAlign:"center"}}>EduGrade</div>
        <div style={{fontSize:11,color:"var(--text2)",textAlign:"center",letterSpacing:"0.1em",marginTop:2}}>STUDENT RESULT MANAGEMENT</div>
      </div>
    </div>
  );
};

// ─── Auth Page ────────────────────────────────────────────────────────────────
const AuthPage = ({ firebase, onAuth, showToast }) => {
  const [mode, setMode] = useState("login");
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [password, setPass] = useState("");
  const [loading, setLoading] = useState(false);

  const handleSubmit = async () => {
    if (!email || (!password && mode !== "reset")) return showToast("⚠ Fill all fields");
    if (mode === "signup" && !name.trim()) return showToast("⚠ Enter your name");
    if (password.length < 6 && mode !== "reset") return showToast("⚠ Password min 6 chars");
    setLoading(true);
    try {
      if (mode === "login") {
        const cred = await firebase.auth.signInWithEmailAndPassword(email, password);
        const snap = await firebase.db.collection("users").doc(cred.user.uid).get();
        if (snap.exists && snap.data().blocked) {
          await firebase.auth.signOut();
          showToast("🚫 Your account has been blocked");
          setLoading(false); return;
        }
        showToast("✓ Welcome back!");
      } else if (mode === "signup") {
        const cred = await firebase.auth.createUserWithEmailAndPassword(email, password);
        await firebase.db.collection("users").doc(cred.user.uid).set({
          name: name.trim(), email, createdAt: new Date().toISOString(), blocked: false, uid: cred.user.uid,
        });
        await firebase.db.collection("adminNotifications").add({
          type: "newUser", name: name.trim(), email,
          uid: cred.user.uid, createdAt: new Date().toISOString(), read: false,
        });
        showToast("✓ Account created!");
      } else {
        await firebase.auth.sendPasswordResetEmail(email);
        showToast("✓ Reset email sent"); setMode("login");
      }
    } catch (e) { showToast("✗ " + (e.message?.replace("Firebase: ", "") || "Error")); }
    setLoading(false);
  };

  return (
    <div style={{
      minHeight: "100vh", display: "flex", alignItems: "center",
      justifyContent: "center", padding: "20px 16px",
      background: "radial-gradient(ellipse at 50% 0%, color-mix(in srgb, var(--sky) 8%, transparent) 0%, transparent 60%)",
    }}>
      <div style={{ width: "100%", maxWidth: 380, animation: "up 0.35s ease" }}>
        <div style={{ textAlign: "center", marginBottom: 30 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 16 }}>
            <div style={{
              width: 68, height: 68, borderRadius: 18,
              background: "linear-gradient(135deg, #38bdf8, #0284c7)",
              display: "flex", alignItems: "center", justifyContent: "center",
              boxShadow: "0 8px 28px rgba(14,165,233,0.4)",
              animation: "pulseGlow 3s ease-in-out infinite",
            }}>
              <svg width="42" height="42" viewBox="0 0 52 52" fill="none">
                <rect x="8" y="10" width="14" height="32" rx="3" fill="rgba(255,255,255,0.3)" />
                <rect x="10" y="10" width="14" height="32" rx="3" fill="rgba(255,255,255,0.45)" />
                <path d="M24 12h14a3 3 0 013 3v26a3 3 0 01-3 3H24V12z" fill="rgba(255,255,255,0.6)" />
                <path d="M30 20l3 3 6-6" stroke="rgba(3,105,161,0.9)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                <path d="M28 30h10M28 35h7" stroke="rgba(3,105,161,0.5)" strokeWidth="2" strokeLinecap="round" />
              </svg>
            </div>
          </div>
          <h1 style={{ fontSize: 28, fontWeight: 800, marginBottom: 5, letterSpacing: "-0.03em" }}>EduGrade</h1>
          <p style={{ color: "var(--text2)", fontSize: 13 }}>
            {mode === "login" ? "Sign in to your workspace" : mode === "signup" ? "Create your account" : "Reset your password"}
          </p>
        </div>

        <Card style={{ padding: 26 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {mode === "signup" && (
              <div><Label>Full Name</Label>
                <input placeholder="Your name" value={name} onChange={e => setName(e.target.value)} />
              </div>
            )}
            <div><Label>Email Address</Label>
              <input type="email" placeholder="you@example.com" value={email}
                onChange={e => setEmail(e.target.value)}
                onKeyDown={e => e.key === "Enter" && handleSubmit()} />
            </div>
            {mode !== "reset" && (
              <div><Label>Password</Label>
                <input type="password" placeholder="••••••••" value={password}
                  onChange={e => setPass(e.target.value)}
                  onKeyDown={e => e.key === "Enter" && handleSubmit()} />
              </div>
            )}
            <Btn onClick={handleSubmit} loading={loading} style={{ width: "100%", marginTop: 4 }}>
              {mode === "login" ? "Sign In →" : mode === "signup" ? "Create Account →" : "Send Reset Email →"}
            </Btn>
          </div>

          <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 8, alignItems: "center" }}>
            {mode === "login" && (
              <>
                <button onClick={() => setMode("reset")} style={{
                  background: "none", border: "none", color: "var(--sky)",
                  fontSize: 12, cursor: "pointer", fontWeight: 600
                }}>Forgot password?</button>
                <div style={{ color: "var(--text2)", fontSize: 12.5 }}>
                  No account?{" "}
                  <span style={{ color: "var(--sky)", cursor: "pointer", fontWeight: 700 }}
                    onClick={() => setMode("signup")}>Sign up</span>
                </div>
              </>
            )}
            {mode !== "login" && (
              <div style={{ color: "var(--text2)", fontSize: 12.5 }}>
                <span style={{ color: "var(--sky)", cursor: "pointer", fontWeight: 700 }}
                  onClick={() => setMode("login")}>← Back to Sign In</span>
              </div>
            )}
          </div>
        </Card>
      </div>
    </div>
  );
};

// ─── Admin Panel ──────────────────────────────────────────────────────────────
// ─── Admin User Workspace (impersonation) ─────────────────────────────────────
const AdminUserWorkspace = ({ firebase, showToast, targetUser, onBack, onUserUpdated }) => {
  const [projects,            setProjects]            = useState([]);
  const [loading,             setLoading]             = useState(true);
  const [page,                setPage]                = useState("home"); // "home" | "entry" | "setup"
  const [activeProjectIndex,  setActiveProjectIndex]  = useState(null);
  const [renamingProject,     setRenamingProject]     = useState(null);
  const [renameValue,         setRenameValue]         = useState("");
  const [confirmModal,        setConfirmModal]        = useState(null);
  const [viewStudent,         setViewStudent]         = useState(null);
  const [saving,              setSaving]              = useState(false);

  // entry form
  const [studentName,    setStudentName]    = useState("");
  const [roll,           setRoll]           = useState("");
  const [subjectEntries, setSubjectEntries] = useState([]);

  // setup form
  const [institution,    setInstitution]    = useState("");
  const [className,      setClassName]      = useState("");
  const [examName,       setExamName]       = useState("");
  const [subjectCount,   setSubjectCount]   = useState("");
  const [subjectNames,   setSubjectNames]   = useState([]);
  const [numAssignments, setNumAssignments] = useState(1);
  const [assignOutOf,    setAssignOutOf]    = useState(20);
  const [passThreshold,  setPassThreshold]  = useState(33);
  const [subjectSettings,setSubjectSettings]= useState([]);

  // subject reorder — setup form (pre-creation) drag state
  const [setupDragIdx, setSetupDragIdx] = useState(null);
  const setupDragStartY = useRef(0);

  // subject reorder — existing project modal
  const [reorderModalOpen, setReorderModalOpen] = useState(false);
  const [reorderList,      setReorderList]      = useState([]); // [{origIndex, name}]
  const [dragIdx,          setDragIdx]           = useState(null);
  const dragStartY = useRef(0);

  const uid = targetUser.id;

  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const snap = await firebase.db.collection("users").doc(uid)
          .collection("projects").orderBy("createdAt", "asc").get();
        setProjects(snap.docs.map(d => ({ id: d.id, ...d.data() })));
      } catch (e) { showToast("✗ " + e.message); }
      setLoading(false);
    })();
  }, [uid]);

  const activeProject = projects[activeProjectIndex] ?? null;

  function freshEntries(proj) {
    return Array(proj.subjectCount || 0).fill(null).map(() => ({
      assignments: Array(proj.numAssignments || 1).fill(""),
      mcq: "", cq: "",
    }));
  }

  function calcSubject(entry, ss, proj) {
    const caFull = Number(ss.caFull ?? 30), examFull = Number(ss.examFull ?? 100), examPct = Number(ss.examPct ?? 70);
    const na = Number(proj.numAssignments || 1), aoof = Number(proj.assignOutOf || 20);
    const assignTotal = (entry.assignments || []).reduce((s, v) => s + Number(v || 0), 0);
    const ca    = Math.round(na * aoof > 0 ? (assignTotal / (na * aoof)) * caFull : 0);
    const mcq   = Number(entry.mcq || 0), cq = Number(entry.cq || 0);
    const rawExam = mcq + cq;
    const b     = Math.round(rawExam * (examPct / 100) + 1e-9);
    const ab    = ca + b;
    const subFull = caFull + (examPct / 100) * examFull;
    const pct   = subFull > 0 ? (ab / subFull) * 100 : 0;
    return { ca, mcq, cq, rawExam, b, ab, subFull, pct, assignTotal, assignments: entry.assignments || [] };
  }

  function calcStudent(entries, proj) {
    const scale = proj?.gradingScale || null;
    const gpFn = makeGetGP(scale), gradeFn = makeGetGrade(scale);
    const thresh = proj?.passThreshold ?? 33;
    const ss = proj?.subjectSettings || [];
    const subResults = entries.map((entry, i) => {
      const s = ss[i] || { caFull: 30, examFull: 100, examPct: 70 };
      const r = calcSubject(entry, s, proj);
      // Exam component (MCQ+CQ, before the examPct scaling) must independently
      // clear the pass threshold against the exam's full marks — a student
      // can't pass a subject on CA marks alone if their raw exam score is
      // below the threshold, even if the combined mark clears it.
      const examFullN   = Number(s.examFull ?? 100);
      const examOnlyPct = examFullN > 0 ? (r.rawExam / examFullN) * 100 : 0;
      const examPassed  = examOnlyPct >= Number(thresh);
      const overallPassed = r.pct >= Number(thresh);
      const pass  = examPassed && overallPassed;
      const gp    = pass ? gpFn(r.pct) : 0;
      const grade = pass ? gradeFn(gp) : "F";
      return { ...r, gp, grade, pass, examOnlyPct };
    });
    const total   = Math.round(subResults.reduce((s, r) => s + r.ab, 0));
    const fail    = subResults.some(r => !r.pass);
    const avgGP   = subResults.reduce((s, r) => s + r.gp, 0) / (subResults.length || 1);
    const gpa     = fail ? 0 : Math.min(parseFloat(avgGP.toFixed(2)), 5);
    const grade   = fail ? "F" : makeGetGrade(scale)(gpa);
    return { gpa, grade, total, subResults, status: fail ? "FAIL" : "PASS" };
  }

  const getSS = (i) => ({ caFull: 30, examFull: 100, examPct: 70, ...(subjectSettings[i] || {}) });

  // ── Subject reorder: setup form (pre-creation, plain local-state swap) ──
  const moveSetupSubject = (idx, dir) => {
    const target = idx + dir;
    if (target < 0 || target >= Number(subjectCount)) return;
    const namesArr = [...subjectNames]; const setsArr = [...subjectSettings];
    const [nMoved] = namesArr.splice(idx, 1); namesArr.splice(target, 0, nMoved);
    const [sMoved] = setsArr.splice(idx, 1); setsArr.splice(target, 0, sMoved);
    setSubjectNames(namesArr); setSubjectSettings(setsArr);
  };
  const handleSetupPointerDown = (idx) => (e) => {
    setSetupDragIdx(idx); setupDragStartY.current = e.clientY;
    e.currentTarget.setPointerCapture?.(e.pointerId);
  };
  const handleSetupPointerMove = (e) => {
    if (setupDragIdx === null) return;
    const deltaY = e.clientY - setupDragStartY.current;
    const itemHeight = 150; // approx row height incl. margin
    if (Math.abs(deltaY) >= itemHeight) {
      const shift = deltaY > 0 ? 1 : -1;
      const target = setupDragIdx + shift;
      if (target >= 0 && target < Number(subjectCount)) {
        moveSetupSubject(setupDragIdx, shift);
        setSetupDragIdx(target); setupDragStartY.current = e.clientY;
      }
    }
  };
  const handleSetupPointerUp = () => setSetupDragIdx(null);

  // ── Subject reorder: existing project (modal + Firestore transaction) ──
  const openReorderModal = () => {
    const full = Array.from({ length: activeProject.subjectCount || 0 }, (_, i) => ({
      origIndex: i, name: (activeProject.subjectNames?.[i] || "").trim() || `Subject ${i + 1}`,
    }));
    setReorderList(full); setReorderModalOpen(true);
  };
  const moveReorderItem = (idx, dir) => {
    setReorderList(prev => {
      const target = idx + dir;
      if (target < 0 || target >= prev.length) return prev;
      const arr = [...prev];
      const [moved] = arr.splice(idx, 1); arr.splice(target, 0, moved);
      return arr;
    });
  };
  const handleReorderPointerDown = (idx) => (e) => {
    setDragIdx(idx); dragStartY.current = e.clientY;
    e.currentTarget.setPointerCapture?.(e.pointerId);
  };
  const handleReorderPointerMove = (e) => {
    if (dragIdx === null) return;
    const deltaY = e.clientY - dragStartY.current;
    const itemHeight = 56;
    if (Math.abs(deltaY) >= itemHeight) {
      const shift = deltaY > 0 ? 1 : -1;
      const target = dragIdx + shift;
      if (target >= 0 && target < reorderList.length) {
        moveReorderItem(dragIdx, shift);
        setDragIdx(target); dragStartY.current = e.clientY;
      }
    }
  };
  const handleReorderPointerUp = () => setDragIdx(null);
  const saveReorderOrder = async () => {
    const newOrder = reorderList.map(it => it.origIndex);
    const newNames = reorderList.map(it => it.name);
    setSaving(true);
    try {
      const result = await firebase.db.reorderSubjects(uid, activeProject.id, newOrder, newNames);
      const pr = { ...activeProject, ...result };
      const updated = [...projects]; updated[activeProjectIndex] = pr;
      setProjects(updated);
      setSubjectEntries(freshEntries(pr));
      setViewStudent(null);
      setReorderModalOpen(false);
      showToast("✓ Subject order updated");
    } catch (e) { showToast("✗ " + e.message); }
    setSaving(false);
  };

  // ── Project CRUD ───────────────────────────────────────────────────────────────
  const createProject = async () => {
    if (!examName.trim() || !subjectCount) return showToast("⚠ Fill exam name and subject count");
    const n = Number(subjectCount);
    if (n < 1 || n > 20) return showToast("⚠ Subjects: 1–20");
    setSaving(true);
    try {
      const finalSettings = Array(n).fill(null).map((_, i) => ({
        caFull: Number(subjectSettings[i]?.caFull ?? 30),
        examFull: Number(subjectSettings[i]?.examFull ?? 100),
        examPct: Number(subjectSettings[i]?.examPct ?? 70),
      }));
      const np = {
        institution: institution.trim(), className: className.trim(),
        examName: examName.trim(), subjectCount: n,
        subjectNames: subjectNames.filter(s => s.trim()),
        numAssignments: Number(numAssignments) || 1,
        assignOutOf: Number(assignOutOf) || 20,
        passThreshold: Number(passThreshold) || 33,
        subjectSettings: finalSettings, gradingScale: null,
        students: [], createdAt: new Date().toISOString(),
      };
      const ref = await firebase.db.collection("users").doc(uid).collection("projects").add(np);
      const newP = { ...np, id: ref.id };
      const updated = [...projects, newP];
      setProjects(updated);
      setActiveProjectIndex(updated.length - 1);
      setSubjectEntries(freshEntries(newP));
      setInstitution(""); setClassName(""); setExamName(""); setSubjectCount("");
      setSubjectNames([]); setSubjectSettings([]); setNumAssignments(1); setAssignOutOf(20); setPassThreshold(33);
      setPage("entry"); showToast("✓ Project created");
    } catch (e) { showToast("✗ " + e.message); }
    setSaving(false);
  };

  const deleteProject = (id) => {
    setConfirmModal({ title: "Delete Project", msg: "Delete this project and ALL student data?", ok: "Delete", onOk: async () => {
      setConfirmModal(null);
      try {
        await firebase.db.collection("users").doc(uid).collection("projects").doc(id).delete();
        const updated = projects.filter(p => p.id !== id);
        setProjects(updated);
        if (activeProject?.id === id) { setActiveProjectIndex(null); setPage("home"); }
        showToast("✓ Deleted");
      } catch (e) { showToast("✗ " + e.message); }
    }});
  };

  const renameProject = async (id, newName) => {
    if (!newName.trim()) return showToast("⚠ Name required");
    try {
      await firebase.db.collection("users").doc(uid).collection("projects").doc(id).update({ examName: newName.trim() });
      setProjects(prev => prev.map(p => p.id === id ? { ...p, examName: newName.trim() } : p));
      setRenamingProject(null); setRenameValue(""); showToast("✓ Renamed");
    } catch (e) { showToast("✗ " + e.message); }
  };

  // ── Student CRUD ───────────────────────────────────────────────────────────────
  const addStudent = async () => {
    if (!activeProject) return;
    if (!studentName.trim() || !roll.trim()) return showToast("⚠ Name & Roll required");
    const { gpa, grade, total, subResults, status } = calcStudent(subjectEntries, activeProject);
    const obj = { name: studentName.trim(), roll: roll.trim(), gpa, grade, total, status, subjectResults: subResults, finalMarks: subResults.map(r => r.ab) };
    setSaving(true);
    try {
      let confirmedUpdate = true;
      const nextStudents = await firebase.db.mutateStudents(uid, activeProject.id, (current) => {
        const list = [...current];
        const idx = list.findIndex(s => s.roll === obj.roll);
        if (idx !== -1) {
          if (!window.confirm("Student exists. Update?")) { confirmedUpdate = false; return current; }
          list[idx] = obj;
        } else { list.push(obj); }
        return list;
      });
      if (!confirmedUpdate) { setSaving(false); return; }
      const pr = { ...activeProject, students: nextStudents };
      const updated = [...projects]; updated[activeProjectIndex] = pr;
      setProjects(updated); setViewStudent(obj);
      setStudentName(""); setRoll(""); setSubjectEntries(freshEntries(activeProject));
      showToast("✓ Saved");
    } catch (e) { showToast("✗ " + e.message); }
    setSaving(false);
  };

  const deleteStudent = (rollNo) => {
    setConfirmModal({ title: "Remove Student", msg: "Remove this student permanently?", ok: "Remove", onOk: async () => {
      setConfirmModal(null);
      try {
        const nextStudents = await firebase.db.mutateStudents(uid, activeProject.id, (current) => current.filter(s => s.roll !== rollNo));
        const pr = { ...activeProject, students: nextStudents };
        const updated = [...projects]; updated[activeProjectIndex] = pr;
        setProjects(updated);
        if (viewStudent?.roll === rollNo) setViewStudent(null);
        showToast("✓ Student removed");
      } catch (e) { showToast("✗ " + e.message); }
    }});
  };

  const meritRanks   = activeProject ? getMeritRank(activeProject.students) : {};
  const sortedByRoll = activeProject ? [...activeProject.students].sort((a, b) => String(a.roll).localeCompare(String(b.roll), undefined, { numeric: true })) : [];

  // ── UI ─────────────────────────────────────────────────────────────────────────
  const [adminTab, setAdminTab] = useState("entry"); // "entry" | "stats" | "merit"

  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 5500, background: "var(--bg)", display: "flex", flexDirection: "column", overflowY: "auto" }}>
      {/* Admin impersonation banner */}
      <div style={{ background: "linear-gradient(90deg,#f59e0b,#fb923c)", padding: "8px 16px", display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        <button onClick={onBack} style={{ background: "rgba(0,0,0,0.2)", border: "none", borderRadius: 8, color: "#fff", padding: "4px 10px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>← Users</button>
        <div style={{ flex: 1, fontSize: 11.5, fontWeight: 700, color: "#fff" }}>
          👁 Viewing as <span style={{ fontWeight: 800 }}>{targetUser.name || targetUser.email}</span>
        </div>
        {targetUser.email !== ADMIN_EMAIL && (
          <button onClick={async () => {
            const newBlocked = !targetUser.blocked;
            try {
              await firebase.db.collection("users").doc(uid).update({ blocked: newBlocked });
              showToast(newBlocked ? "🚫 User blocked" : "✓ User unblocked");
              onUserUpdated?.({ ...targetUser, blocked: newBlocked });
            } catch (e) { showToast("✗ " + e.message); }
          }} style={{ background: targetUser.blocked ? "#10b981" : "#ef4444", border: "none", borderRadius: 8, color: "#fff", padding: "4px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer" }}>
            {targetUser.blocked ? "Unblock" : "Block"}
          </button>
        )}
      </div>

      {confirmModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 24px" }}>
          <div style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 20, padding: 24, width: "100%", maxWidth: 340, animation: "up 0.2s ease" }}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 8 }}>{confirmModal.title}</div>
            <div style={{ color: "var(--text2)", fontSize: 13, marginBottom: 20, lineHeight: 1.5 }}>{confirmModal.msg}</div>
            <div style={{ display: "flex", gap: 10 }}>
              <Btn variant="ghost" onClick={() => setConfirmModal(null)} style={{ flex: 1 }}>Cancel</Btn>
              <Btn variant="danger" onClick={() => { setConfirmModal(null); confirmModal.onOk(); }} style={{ flex: 1 }}>{confirmModal.ok}</Btn>
            </div>
          </div>
        </div>
      )}

      {renamingProject && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 999, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 20px" }}>
          <div style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 20, padding: 24, width: "100%", maxWidth: 380 }}>
            <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 16 }}>Rename Project</div>
            <input autoFocus value={renameValue} onChange={e => setRenameValue(e.target.value)} onKeyDown={e => e.key === "Enter" && renameProject(renamingProject, renameValue)} placeholder="Project name" style={{ marginBottom: 14 }} />
            <div style={{ display: "flex", gap: 10 }}>
              <Btn variant="secondary" onClick={() => setRenamingProject(null)} style={{ flex: 1 }}>Cancel</Btn>
              <Btn onClick={() => renameProject(renamingProject, renameValue)} style={{ flex: 1 }}>Save</Btn>
            </div>
          </div>
        </div>
      )}

      <div style={{ maxWidth: 520, margin: "0 auto", padding: "0 16px 120px", width: "100%" }}>

        {/* ── HOME ── */}
        {page === "home" && (
          <div style={{ animation: "up 0.28s ease", paddingTop: 20 }}>
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.12em", textTransform: "uppercase", color: "var(--sky)", marginBottom: 6 }}>Dashboard</div>
              <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-0.02em", marginBottom: 2 }}>{targetUser.name || "—"}'s Projects</h1>
              <p style={{ color: "var(--text2)", fontSize: 12 }}>{targetUser.email}</p>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ fontWeight: 700, fontSize: 14 }}>Projects ({projects.length})</div>
              <Btn size="sm" onClick={() => setPage("setup")}>+ New</Btn>
            </div>
            {loading ? <Spinner /> : projects.length === 0 ? (
              <div style={{ textAlign: "center", padding: "48px 24px", background: "var(--surface)", border: "1.5px dashed var(--border)", borderRadius: 20 }}>
                <div style={{ fontSize: 40, marginBottom: 12 }}>📋</div>
                <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 8 }}>No projects yet</div>
                <Btn onClick={() => setPage("setup")}>＋ New Project</Btn>
              </div>
            ) : projects.map((p, i) => {
              const pass = p.students.filter(s => s.status === "PASS").length;
              const passRate = p.students.length ? ((pass / p.students.length) * 100).toFixed(0) : 0;
              const accent = ["#38bdf8","#a78bfa","#34d399","#fb923c","#f472b6","#facc15"][i % 6];
              return (
                <div key={p.id} style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderLeft: `3px solid ${accent}`, borderRadius: 16, marginBottom: 9, overflow: "hidden" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 16px" }}>
                    <div style={{ width: 46, height: 46, borderRadius: 14, flexShrink: 0, background: `color-mix(in srgb,${accent} 12%,var(--surface2))`, border: `1.5px solid color-mix(in srgb,${accent} 20%,var(--border))`, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      <IcoDoc s={22} c={accent} />
                    </div>
                    <div onClick={() => { setActiveProjectIndex(i); setSubjectEntries(freshEntries(p)); setViewStudent(null); setPage("entry"); }} style={{ flex: 1, cursor: "pointer" }}>
                      <div style={{ fontWeight: 700, fontSize: 14.5, marginBottom: 2 }}>{p.examName}</div>
                      <div style={{ color: "var(--text2)", fontSize: 12 }}>
                        {p.institution && <span>{p.institution}{p.className ? " · " + p.className : ""} · </span>}
                        {p.students.length} students · {p.subjectCount} subjects
                        {p.students.length > 0 && <span style={{ marginLeft: 8, color: "var(--green)", fontWeight: 600 }}>{passRate}% pass</span>}
                      </div>
                      {p.students.length > 0 && (
                        <div style={{ height: 3, borderRadius: 2, background: "var(--border)", marginTop: 7, overflow: "hidden" }}>
                          <div style={{ height: 3, borderRadius: 2, background: "var(--green)", width: `${passRate}%`, transition: "width 0.8s ease" }} />
                        </div>
                      )}
                    </div>
                  </div>
                  <div style={{ display: "flex", borderTop: "1px solid var(--border)" }}>
                    {[
                      { icon: "✏️", label: "Rename", action: () => { setRenamingProject(p.id); setRenameValue(p.examName); } },
                      { icon: "🗑", label: "Delete", action: () => deleteProject(p.id), danger: true },
                    ].map(({ icon, label, action, danger }) => (
                      <button key={label} onClick={e => { e.stopPropagation(); action(); }} style={{ flex: 1, padding: "8px 2px", border: "none", background: "transparent", color: danger ? "var(--red)" : "var(--text2)", fontSize: 10.5, fontWeight: 600, cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 2, borderRight: label !== "Delete" ? "1px solid var(--border)" : undefined }}>
                        <span style={{ fontSize: 14 }}>{icon}</span><span>{label}</span>
                      </button>
                    ))}
                  </div>
                </div>
              );
            })}
          </div>
        )}

        {/* ── SETUP ── */}
        {page === "setup" && (
          <div style={{ animation: "up 0.28s ease", paddingTop: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 18 }}>
              <button onClick={() => setPage("home")} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 700, color: "var(--text2)", cursor: "pointer" }}>← Back</button>
              <h2 style={{ fontWeight: 800, fontSize: 18, letterSpacing: "-0.02em" }}>New Project</h2>
            </div>
            <Card style={{ marginBottom: 14 }}>
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <div><Label>Exam / Project Name ✱</Label><input placeholder="e.g. Annual Exam 2025" value={examName} onChange={e => setExamName(e.target.value)} /></div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                  <div><Label>Institution</Label><input placeholder="School name" value={institution} onChange={e => setInstitution(e.target.value)} /></div>
                  <div><Label>Class / Grade</Label><input placeholder="e.g. Class 6" value={className} onChange={e => setClassName(e.target.value)} /></div>
                </div>
                <div><Label>Number of Subjects ✱</Label>
                  <input type="number" placeholder="e.g. 5" value={subjectCount} min={1} max={20} onChange={e => {
                    setSubjectCount(e.target.value);
                    const n = Number(e.target.value);
                    setSubjectNames(Array(n).fill(""));
                    setSubjectSettings(Array(n).fill(null).map(() => ({ caFull: 30, examFull: 100, examPct: 70 })));
                  }} />
                </div>
              </div>
            </Card>
            {subjectCount && Number(subjectCount) >= 1 && (
              <Card style={{ marginBottom: 14 }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: "var(--sky)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>Subjects <span style={{opacity:0.7,textTransform:"none",letterSpacing:0}}>· drag ⠿ or use arrows to reorder</span></div>
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }} onPointerMove={handleSetupPointerMove} onPointerUp={handleSetupPointerUp} onPointerLeave={handleSetupPointerUp}>
                  {Array(Number(subjectCount)).fill(null).map((_, i) => {
                    const ss = getSS(i);
                    return (
                      <div key={i} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 14, padding: "14px 16px", transition:"border-color 0.2s", position:"relative", overflow:"hidden", opacity: setupDragIdx===i?0.6:1 }}>
                        <div style={{position:"absolute",top:0,left:0,width:3,height:"100%",background:"linear-gradient(180deg,var(--sky),var(--indigo))",opacity:0.6}}/>
                        <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                          <div onPointerDown={handleSetupPointerDown(i)} style={{cursor:"grab",touchAction:"none",fontSize:16,color:"var(--text3)",padding:"2px 4px",flexShrink:0,userSelect:"none"}}>⠿</div>
                          <div style={{width:22,height:22,borderRadius:7,background:"color-mix(in srgb,var(--sky) 16%,var(--surface))",color:"var(--sky)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,flexShrink:0}}>{i+1}</div>
                          <input placeholder={`Subject ${i + 1} name`} value={subjectNames[i] || ""} onChange={e => { const u = [...subjectNames]; u[i] = e.target.value; setSubjectNames(u); }} style={{ margin:0 }} />
                          <div style={{display:"flex",flexDirection:"column",gap:2,flexShrink:0}}>
                            <button onClick={() => moveSetupSubject(i, -1)} disabled={i===0} style={{background:"var(--surface3)",border:"1px solid var(--border)",borderRadius:5,color:"var(--text2)",width:22,height:16,fontSize:9,cursor:i===0?"default":"pointer",opacity:i===0?0.35:1,lineHeight:"1"}}>▲</button>
                            <button onClick={() => moveSetupSubject(i, 1)} disabled={i===Number(subjectCount)-1} style={{background:"var(--surface3)",border:"1px solid var(--border)",borderRadius:5,color:"var(--text2)",width:22,height:16,fontSize:9,cursor:i===Number(subjectCount)-1?"default":"pointer",opacity:i===Number(subjectCount)-1?0.35:1,lineHeight:"1"}}>▼</button>
                          </div>
                        </div>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                          <div>
                            <div style={{ fontSize: 9.5, fontWeight: 700, color: "#818cf8", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 4 }}>CA Full</div>
                            <input type="number" value={ss.caFull??30} min={0} onChange={e=>{ const u=[...subjectSettings]; u[i]={...(u[i]||{caFull:30,examFull:100,examPct:70}),caFull:e.target.value}; setSubjectSettings(u); }} style={{ textAlign: "center" }} />
                          </div>
                          <div>
                            <div style={{ fontSize: 9.5, fontWeight: 700, color: "var(--sky)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 4 }}>Exam Full</div>
                            <input type="number" value={ss.examFull??100} min={1} onChange={e=>{ const u=[...subjectSettings]; u[i]={...(u[i]||{caFull:30,examFull:100,examPct:70}),examFull:e.target.value}; setSubjectSettings(u); }} style={{ textAlign: "center" }} />
                          </div>
                          <div>
                            <div style={{ fontSize: 9.5, fontWeight: 700, color: "var(--amber)", letterSpacing: "0.06em", textTransform: "uppercase", marginBottom: 4 }}>Exam %</div>
                            <input type="number" value={ss.examPct??70} min={0} max={100} onChange={e=>{ const u=[...subjectSettings]; u[i]={...(u[i]||{caFull:30,examFull:100,examPct:70}),examPct:e.target.value}; setSubjectSettings(u); }} style={{ textAlign: "center" }} />
                          </div>
                        </div>
                        <div style={{marginTop:10,fontSize:11,color:"var(--text2)",display:"flex",gap:12,flexWrap:"wrap",paddingTop:8,borderTop:"1px solid var(--border)"}}>
                          <span>CA: <b style={{color:"#818cf8"}}>{ss.caFull??30}</b></span>
                          <span>B: <b style={{color:"var(--amber)"}}>{((Number(ss.examPct)||70)/100*(Number(ss.examFull)||100)).toFixed(0)}</b></span>
                          <span>Max: <b style={{color:"var(--sky)"}}>{Math.round(Number(ss.caFull??30)+((Number(ss.examPct)||70)/100)*Number(ss.examFull||100))}</b></span>
                        </div>
                      </div>
                    );
                  })}
                </div>
              </Card>
            )}
            <Card style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 10, fontWeight: 700, color: "var(--sky)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>CA Settings</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 12, color: "var(--text2)", flex: 1 }}>Number of Assignments</span>
                  <input type="number" value={numAssignments} min={1} max={10} onChange={e => setNumAssignments(Math.max(1, Number(e.target.value)))} style={{ width: 70, textAlign: "center" }} />
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 12, color: "var(--text2)", flex: 1 }}>Full Marks per Assignment</span>
                  <input type="number" value={assignOutOf} min={1} onChange={e => setAssignOutOf(Number(e.target.value))} style={{ width: 70, textAlign: "center" }} />
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 12, color: "var(--text2)", flex: 1 }}>Pass Threshold (%)</span>
                  <input type="number" value={passThreshold} min={0} max={100} onChange={e => setPassThreshold(Number(e.target.value))} style={{ width: 70, textAlign: "center" }} />
                </div>
              </div>
            </Card>
            <Btn onClick={createProject} loading={saving} style={{ width: "100%", marginBottom: 80 }} size="lg">Create Project →</Btn>
          </div>
        )}

        {/* ── ENTRY ── */}
        {page === "entry" && adminTab === "entry" && (
          <div style={{ animation: "up 0.28s ease", paddingTop: 20 }}>
            {!activeProject ? (
              <div style={{ textAlign: "center", padding: "60px 0" }}>
                <div style={{ color: "var(--text2)", marginBottom: 16 }}>No project selected</div>
                <Btn onClick={() => setPage("home")}>← Go Home</Btn>
              </div>
            ) : (
              <>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                  <button onClick={() => { setPage("home"); setViewStudent(null); }} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 700, color: "var(--text2)", cursor: "pointer" }}>← Projects</button>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, fontSize: 15, letterSpacing: "-0.01em" }}>{activeProject.examName}</div>
                    <div style={{ color: "var(--text2)", fontSize: 11 }}>{[activeProject.institution, activeProject.className].filter(Boolean).join(" · ") || "Mark Entry"}</div>
                  </div>
                  <button onClick={openReorderModal} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 10, padding: "6px 10px", fontSize: 12, fontWeight: 700, color: "var(--sky)", cursor: "pointer", flexShrink: 0 }}>🔀 Reorder</button>
                </div>

                {reorderModalOpen && (
                  <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 9999 }} onClick={() => setReorderModalOpen(false)}>
                    <div onClick={e => e.stopPropagation()} style={{ background: "var(--surface)", borderRadius: "20px 20px 0 0", padding: 20, width: "100%", maxWidth: 480, maxHeight: "80vh", overflowY: "auto", border: "1.5px solid var(--border)", borderBottom: "none" }}>
                      <div style={{ fontWeight: 800, fontSize: 15, marginBottom: 4 }}>Reorder Subjects</div>
                      <div style={{ fontSize: 11.5, color: "var(--text2)", marginBottom: 14 }}>Drag ⠿ or use the arrows. This updates the order everywhere — entry page, marksheet, and every student already saved.</div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }} onPointerMove={handleReorderPointerMove} onPointerUp={handleReorderPointerUp} onPointerLeave={handleReorderPointerUp}>
                        {reorderList.map((item, i) => (
                          <div key={item.origIndex} style={{ display: "flex", alignItems: "center", gap: 10, background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 12, padding: "10px 12px", opacity: dragIdx===i?0.6:1 }}>
                            <div onPointerDown={handleReorderPointerDown(i)} style={{cursor:"grab",touchAction:"none",fontSize:16,color:"var(--text3)",userSelect:"none"}}>⠿</div>
                            <div style={{width:20,height:20,borderRadius:6,background:"color-mix(in srgb,var(--sky) 16%,var(--surface))",color:"var(--sky)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,flexShrink:0}}>{i+1}</div>
                            <div style={{flex:1,fontSize:13,fontWeight:600}}>{item.name}</div>
                            <button onClick={() => moveReorderItem(i, -1)} disabled={i===0} style={{background:"var(--surface3)",border:"1px solid var(--border)",borderRadius:6,color:"var(--text2)",width:26,height:22,fontSize:11,cursor:i===0?"default":"pointer",opacity:i===0?0.35:1}}>▲</button>
                            <button onClick={() => moveReorderItem(i, 1)} disabled={i===reorderList.length-1} style={{background:"var(--surface3)",border:"1px solid var(--border)",borderRadius:6,color:"var(--text2)",width:26,height:22,fontSize:11,cursor:i===reorderList.length-1?"default":"pointer",opacity:i===reorderList.length-1?0.35:1}}>▼</button>
                          </div>
                        ))}
                      </div>
                      <div style={{ display: "flex", gap: 10 }}>
                        <Btn variant="secondary" onClick={() => setReorderModalOpen(false)} style={{ flex: 1 }}>Cancel</Btn>
                        <Btn onClick={saveReorderOrder} loading={saving} style={{ flex: 1 }}>Save Order</Btn>
                      </div>
                    </div>
                  </div>
                )}

                {/* Add Student Card */}
                <Card style={{ marginBottom: 14, borderRadius: 18 }}>
                  <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 14 }}>+ Add / Update Student</div>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 14 }}>
                    <div><Label>Student Name</Label><input placeholder="Full name" value={studentName} onChange={e => setStudentName(e.target.value)} /></div>
                    <div><Label>Roll No.</Label><input placeholder="Roll" value={roll} onChange={e => setRoll(e.target.value)} /></div>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 14 }}>
                    {Array(activeProject.subjectCount).fill(null).map((_, si) => {
                      const subName = activeProject.subjectNames?.[si] || `Subject ${si + 1}`;
                      const ss = activeProject.subjectSettings?.[si] || { caFull: 30, examFull: 100, examPct: 70 };
                      const entry = subjectEntries[si] || { assignments: [], mcq: "", cq: "" };
                      const na = activeProject.numAssignments || 1, aoof = activeProject.assignOutOf || 20;
                      const r = calcSubject(entry, ss, activeProject);
                      const gpFn = makeGetGP(activeProject.gradingScale || null);
                      const gradeFn = makeGetGrade(activeProject.gradingScale || null);
                      const grLive = gradeFn(gpFn(r.pct));
                      const examFullLive   = Number(ss.examFull ?? 100);
                      const examOnlyPctLive = examFullLive > 0 ? (r.rawExam / examFullLive) * 100 : 0;
                      const passLive = r.pct >= (activeProject.passThreshold ?? 33) && examOnlyPctLive >= (activeProject.passThreshold ?? 33);
                      const subFull = Number(ss.caFull) + ((Number(ss.examPct) || 70) / 100) * Number(ss.examFull || 100);
                      return (
                        <div key={si} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
                          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", borderBottom: "1px solid var(--border)", background: "var(--surface3)" }}>
                            <div style={{ fontWeight: 700, fontSize: 13 }}>{subName}</div>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              <span style={{ fontSize: 11, color: "var(--text2)" }}>/{Math.round(subFull)}</span>
                              <span style={{ fontWeight: 800, fontSize: 14, color: gradeColor(grLive), fontFamily: "var(--font-mono)" }}>{grLive}</span>
                              <span style={{ fontWeight: 700, fontSize: 13, color: "var(--sky)", fontFamily: "var(--font-mono)" }}>{r.ab}</span>
                              <Pill color={passLive ? "var(--green)" : "var(--red)"}>{passLive ? "P" : "F"}</Pill>
                            </div>
                          </div>
                          <div style={{ padding: "8px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
                            {Array(na).fill(null).map((_, ai) => (
                              <div key={ai} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                <span style={{ fontSize: 11, color: "var(--indigo)", fontWeight: 600, minWidth: 80 }}>Assignment {ai + 1}</span>
                                <input type="number" placeholder="0" min={0} max={aoof}
                                  value={(entry.assignments || [])[ai] ?? ""}
                                  onChange={e => setSubjectEntries(prev => prev.map((en, s) => {
                                    if (s !== si) return en;
                                    const a = [...(en.assignments || [])]; a[ai] = e.target.value;
                                    return { ...en, assignments: a };
                                  }))}
                                  style={{ flex: 1, textAlign: "center" }} />
                                <span style={{ fontSize: 11, color: "var(--text3)", minWidth: 28 }}>/{aoof}</span>
                              </div>
                            ))}
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              <span style={{ fontSize: 11.5, color: "var(--sky)", fontWeight: 600, minWidth: 90 }}>MCQ / Oral</span>
                              <input type="number" placeholder="0" min={0} value={entry.mcq ?? ""} onChange={e => setSubjectEntries(prev => prev.map((en, s) => s === si ? { ...en, mcq: e.target.value } : en))} style={{ flex: 1, textAlign: "center" }} />
                              <span style={{ fontSize: 11, color: "var(--text3)", minWidth: 28 }}>/{ss.examFull || 100}</span>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              <span style={{ fontSize: 11.5, color: "var(--amber)", fontWeight: 600, minWidth: 90 }}>CQ / Written</span>
                              <input type="number" placeholder="0" min={0} value={entry.cq ?? ""} onChange={e => setSubjectEntries(prev => prev.map((en, s) => s === si ? { ...en, cq: e.target.value } : en))} style={{ flex: 1, textAlign: "center" }} />
                              <span style={{ fontSize: 11, color: "var(--text3)", minWidth: 28 }}>/{ss.examFull || 100}</span>
                            </div>
                            <div style={{ fontSize: 11, color: "var(--text2)", display: "flex", gap: 10, flexWrap: "wrap", paddingTop: 4, borderTop: "1px solid var(--border)" }}>
                              <span>CA: <b style={{ color: "var(--indigo)", fontFamily: "var(--font-mono)" }}>{r.ca}</b></span>
                              <span>MCQ+CQ: <b style={{ color: "var(--sky)", fontFamily: "var(--font-mono)" }}>{r.rawExam}</b></span>
                              <span>B: <b style={{ color: "var(--amber)", fontFamily: "var(--font-mono)" }}>{r.b}</b></span>
                              <span>Total: <b style={{ color: "var(--green)", fontFamily: "var(--font-mono)" }}>{r.ab}/{Math.round(subFull)}</b></span>
                            </div>
                          </div>
                        </div>
                      );
                    })}
                  </div>
                  <Btn onClick={addStudent} loading={saving} style={{ width: "100%" }} size="lg">Save Student</Btn>
                </Card>

                {/* Student list */}
                {sortedByRoll.length > 0 && (
                  <Card style={{ padding: 0, borderRadius: 18, overflow: "hidden", marginBottom: 14 }}>
                    <div style={{ padding: "14px 16px 10px", fontWeight: 700, fontSize: 13, borderBottom: "1px solid var(--border)" }}>
                      Students ({sortedByRoll.length})
                    </div>
                    {sortedByRoll.map(s => {
                      const rank = meritRanks[s.roll];
                      const isViewing = viewStudent?.roll === s.roll;
                      return (
                        <div key={s.roll}>
                          <div onClick={() => setViewStudent(isViewing ? null : s)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", cursor: "pointer", background: isViewing ? "color-mix(in srgb,var(--sky) 6%,var(--surface))" : undefined, borderBottom: "1px solid var(--border)" }}>
                            <div style={{ width: 34, height: 34, borderRadius: 10, background: "linear-gradient(135deg,var(--sky),var(--indigo))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: "#fff", flexShrink: 0 }}>
                              {(s.name || "?")[0].toUpperCase()}
                            </div>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ fontWeight: 700, fontSize: 13 }}>{s.name}</div>
                              <div style={{ color: "var(--text2)", fontSize: 11 }}>Roll: {s.roll} · #{rank} merit</div>
                            </div>
                            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                              <span style={{ fontWeight: 800, fontSize: 14, color: gradeColor(s.grade), fontFamily: "var(--font-mono)" }}>{s.gpa}</span>
                              <Pill color={gradeColor(s.grade)}>{s.grade}</Pill>
                              <Pill color={s.status === "PASS" ? "var(--green)" : "var(--red)"}>{s.status}</Pill>
                            </div>
                          </div>
                          {isViewing && (
                            <div style={{ padding: "10px 16px 14px", background: "var(--surface2)", borderBottom: "1px solid var(--border)" }}>
                              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
                                {(s.subjectResults || []).map((sr, si) => (
                                  <div key={si} style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 10, padding: "8px 12px", minWidth: 90 }}>
                                    <div style={{ fontSize: 10, color: "var(--text2)", fontWeight: 700, marginBottom: 3 }}>{activeProject.subjectNames?.[si] || `Sub ${si + 1}`}</div>
                                    <div style={{ fontWeight: 800, fontSize: 15, color: gradeColor(sr.grade), fontFamily: "var(--font-mono)" }}>{sr.ab}</div>
                                    <div style={{ fontSize: 10, color: "var(--text2)" }}>{sr.pct?.toFixed(1)}% · {sr.grade}</div>
                                  </div>
                                ))}
                              </div>
                              <div style={{ display: "flex", gap: 8 }}>
                                <Btn variant="sky" size="sm" style={{ flex: 1 }} onClick={() => {
                                  setStudentName(s.name); setRoll(s.roll);
                                  const entries = (s.subjectResults || []).map(sr => ({
                                    assignments: sr.assignments || Array(activeProject.numAssignments || 1).fill(""),
                                    mcq: String(sr.mcq || ""), cq: String(sr.cq || ""),
                                  }));
                                  setSubjectEntries(entries);
                                  window.scrollTo({ top: 0, behavior: "smooth" });
                                  showToast("✏️ Loaded for editing");
                                }}>✏️ Edit Marks</Btn>
                                <Btn variant="danger" size="sm" style={{ flex: 1 }} onClick={() => deleteStudent(s.roll)}>🗑 Delete</Btn>
                              </div>
                            </div>
                          )}
                        </div>
                      );
                    })}
                  </Card>
                )}
              </>
            )}
          </div>
        )}
        {/* ── ADMIN STATS ── */}
        {page === "entry" && activeProject && adminTab === "stats" && (
          <div style={{ animation: "up 0.28s ease", paddingTop: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <button onClick={() => setPage("home")} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 700, color: "var(--text2)", cursor: "pointer" }}>← Projects</button>
              <div style={{ flex: 1, fontWeight: 800, fontSize: 15 }}>{activeProject.examName} · Stats</div>
            </div>
            {activeProject.students.length === 0 ? (
              <div style={{ textAlign: "center", color: "var(--text2)", padding: "60px 0" }}>No students yet</div>
            ) : (() => {
              const pass = activeProject.students.filter(s => s.status === "PASS").length;
              const fail = activeProject.students.length - pass;
              const avgGPA = (activeProject.students.reduce((a, s) => a + s.gpa, 0) / activeProject.students.length).toFixed(2);
              const topStudent = [...activeProject.students].sort((a, b) => b.gpa - a.gpa)[0];
              const gradeDist = activeProject.students.reduce((acc, s) => { acc[s.grade] = (acc[s.grade] || 0) + 1; return acc; }, {});
              return (
                <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    {[
                      { label: "Total", value: activeProject.students.length, color: "var(--sky)" },
                      { label: "Pass", value: pass, color: "var(--green)" },
                      { label: "Fail", value: fail, color: "var(--red)" },
                      { label: "Avg GPA", value: avgGPA, color: "var(--indigo)" },
                      { label: "Pass Rate", value: `${((pass/activeProject.students.length)*100).toFixed(1)}%`, color: "var(--amber)" },
                      { label: "Top GPA", value: topStudent?.gpa, color: "var(--yellow)" },
                    ].map(({ label, value, color }) => (
                      <div key={label} style={{ background: "var(--surface)", border: "1.5px solid var(--border)", borderRadius: 14, padding: "14px 16px", textAlign: "center" }}>
                        <div style={{ fontSize: 9.5, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--text2)", marginBottom: 4 }}>{label}</div>
                        <div style={{ fontSize: 22, fontWeight: 800, color, fontFamily: "var(--font-mono)" }}>{value}</div>
                      </div>
                    ))}
                  </div>
                  <Card>
                    <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 10 }}>Grade Distribution</div>
                    {Object.entries(gradeDist).sort().map(([g, cnt]) => (
                      <div key={g} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                        <div style={{ width: 32, fontWeight: 800, fontSize: 13, color: gradeColor(g) }}>{g}</div>
                        <div style={{ flex: 1, height: 8, borderRadius: 4, background: "var(--border)", overflow: "hidden" }}>
                          <div style={{ height: 8, borderRadius: 4, background: gradeColor(g), width: `${(cnt/activeProject.students.length)*100}%`, transition: "width 0.6s ease" }}/>
                        </div>
                        <div style={{ fontWeight: 700, fontSize: 12, color: "var(--text2)", minWidth: 24 }}>{cnt}</div>
                      </div>
                    ))}
                  </Card>
                  {topStudent && (
                    <Card style={{ border: "1.5px solid var(--yellow)", background: "color-mix(in srgb,var(--yellow) 5%,var(--surface))" }}>
                      <div style={{ fontSize: 10, fontWeight: 700, color: "var(--yellow)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 6 }}>🏆 Top Student</div>
                      <div style={{ fontWeight: 800, fontSize: 16 }}>{topStudent.name}</div>
                      <div style={{ color: "var(--text2)", fontSize: 12 }}>Roll: {topStudent.roll} · GPA {topStudent.gpa} · {topStudent.grade}</div>
                    </Card>
                  )}
                </div>
              );
            })()}
          </div>
        )}

        {/* ── ADMIN MERIT ── */}
        {page === "entry" && activeProject && adminTab === "merit" && (
          <div style={{ animation: "up 0.28s ease", paddingTop: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
              <button onClick={() => setPage("home")} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 10, padding: "6px 12px", fontSize: 12, fontWeight: 700, color: "var(--text2)", cursor: "pointer" }}>← Projects</button>
              <div style={{ flex: 1, fontWeight: 800, fontSize: 15 }}>{activeProject.examName} · Merit</div>
            </div>
            {activeProject.students.length === 0 ? (
              <div style={{ textAlign: "center", color: "var(--text2)", padding: "60px 0" }}>No students yet</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {[...activeProject.students].sort((a, b) => b.gpa - a.gpa || b.total - a.total).map((s, idx) => {
                  const rank = idx + 1;
                  const medal = rank === 1 ? "🥇" : rank === 2 ? "🥈" : rank === 3 ? "🥉" : null;
                  return (
                    <div key={s.roll} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 14px", borderRadius: 14, background: rank <= 3 ? `color-mix(in srgb,${gradeColor(s.grade)} 8%,var(--surface))` : "var(--surface2)", border: `1.5px solid ${rank <= 3 ? gradeColor(s.grade) : "var(--border)"}` }}>
                      <div style={{ width: 32, textAlign: "center", fontWeight: 800, fontSize: medal ? 22 : 14, color: "var(--text2)", flexShrink: 0 }}>{medal || `#${rank}`}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: 13 }}>{s.name}</div>
                        <div style={{ color: "var(--text2)", fontSize: 11 }}>Roll: {s.roll} · Total: {s.total}</div>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        <span style={{ fontWeight: 800, fontSize: 16, color: gradeColor(s.grade), fontFamily: "var(--font-mono)" }}>{s.gpa}</span>
                        <Pill color={gradeColor(s.grade)}>{s.grade}</Pill>
                        <Pill color={s.status === "PASS" ? "var(--green)" : "var(--red)"}>{s.status}</Pill>
                      </div>
                    </div>
                  );
                })}
              </div>
            )}
          </div>
        )}

      </div>

      {/* Admin bottom nav (only when in entry page with a project) */}
      {page === "entry" && activeProject && (
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 100, background: "var(--surface)", borderTop: "1.5px solid var(--border)", display: "flex", justifyContent: "space-around", padding: "8px 0 12px" }}>
          {[
            { id: "entry", label: "Entry", icon: "✏️" },
            { id: "stats",  label: "Stats",  icon: "📊" },
            { id: "merit",  label: "Merit",  icon: "🏆" },
          ].map(({ id, label, icon }) => (
            <button key={id} onClick={() => setAdminTab(id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "transparent", border: "none", cursor: "pointer", color: adminTab === id ? "var(--sky)" : "var(--text3)", padding: "4px 0" }}>
              <span style={{ fontSize: 18 }}>{icon}</span>
              <span style={{ fontSize: 10, fontWeight: adminTab === id ? 800 : 500 }}>{label}</span>
              {adminTab === id && <div style={{ width: 20, height: 2, borderRadius: 1, background: "var(--sky)" }}/>}
            </button>
          ))}
        </div>
      )}
    </div>
  );
};

// ─── Admin Panel (User List) ───────────────────────────────────────────────────
const AdminPanel = ({ firebase, showToast, onClose }) => {
  const [users,        setUsers]        = useState([]);
  const [loading,      setLoading]      = useState(true);
  const [permError,    setPermError]    = useState(false);
  const [impersonating,setImpersonating]= useState(null);
  const [search,       setSearch]       = useState("");

  useEffect(() => {
    if (!firebase) return;
    (async () => {
      setLoading(true);
      try {
        const snap = await firebase.db.collection("users").get();
        setUsers(snap.docs.map(d => ({ id: d.id, ...d.data() })));
      } catch (e) {
        if (e.code === "permission-denied" || e.message?.includes("permission")) setPermError(true);
        else showToast("✗ " + e.message);
      }
      setLoading(false);
    })();
  }, [firebase]);

  const filteredUsers = users.filter(u =>
    !search || u.name?.toLowerCase().includes(search.toLowerCase()) || u.email?.toLowerCase().includes(search.toLowerCase())
  );

  // If impersonating a user, show full workspace
  if (impersonating) {
    return (
      <AdminUserWorkspace
        firebase={firebase}
        showToast={showToast}
        targetUser={impersonating}
        onBack={() => setImpersonating(null)}
        onUserUpdated={(updatedUser) => {
          setImpersonating(updatedUser);
          setUsers(prev => prev.map(u => u.id === updatedUser.id ? updatedUser : u));
        }}
      />
    );
  }

  return (
    <div style={{ position:"fixed",inset:0,zIndex:5000,background:"rgba(0,0,0,0.65)",display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(8px)" }}
      onClick={e => e.target===e.currentTarget && onClose()}>
      <div style={{ background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:"22px 22px 0 0",width:"100%",maxWidth:560,maxHeight:"88vh",display:"flex",flexDirection:"column",animation:"up 0.3s ease",boxShadow:"0 -16px 60px rgba(0,0,0,0.28)" }}>

        <div style={{ padding:"20px 22px 14px",borderBottom:"1.5px solid var(--border)" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
            <div>
              <div style={{ fontSize:9.5,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"var(--yellow)",marginBottom:4 }}>⚡ Admin Control</div>
              <h2 style={{ fontWeight:800,fontSize:20,letterSpacing:"-0.02em" }}>Dashboard</h2>
            </div>
            <button onClick={onClose} style={{ width:34,height:34,borderRadius:"50%",border:"1.5px solid var(--border)",background:"var(--surface2)",color:"var(--text2)",fontSize:17,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer" }}>×</button>
          </div>
          {!loading && !permError && (
            <div style={{ display:"flex",gap:8,marginBottom:14 }}>
              {[
                { label:"Total", value:users.length,                         color:"var(--sky)"   },
                { label:"Active",value:users.filter(u=>!u.blocked).length,   color:"var(--green)" },
                { label:"Blocked",value:users.filter(u=>u.blocked).length,   color:"var(--red)"   },
              ].map(({ label,value,color }) => (
                <div key={label} style={{ flex:1,background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:12,padding:"10px 12px",textAlign:"center" }}>
                  <div style={{ fontSize:9,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.08em",color:"var(--text2)",marginBottom:3 }}>{label}</div>
                  <div style={{ fontSize:20,fontWeight:800,color,fontFamily:"var(--font-mono)" }}>{value}</div>
                </div>
              ))}
            </div>
          )}
          {!loading && !permError && users.length > 0 && (
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="🔍  Search by name or email…" style={{ marginBottom:0 }} />
          )}
        </div>

        <div style={{ flex:1,overflowY:"auto",padding:16 }}>
          {loading ? <Spinner /> : permError ? (
            <div style={{ padding:"24px 4px" }}>
              <div style={{ textAlign:"center",fontSize:36,marginBottom:14 }}>🔒</div>
              <div style={{ fontWeight:800,fontSize:16,textAlign:"center",marginBottom:8 }}>Firestore Permission Denied</div>
              <p style={{ color:"var(--text2)",fontSize:12.5,lineHeight:1.7,marginBottom:16,textAlign:"center" }}>Update your Firestore rules in Firebase Console:</p>
              <div style={{ background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:12,padding:"14px 16px",fontFamily:"var(--font-mono)",fontSize:11,color:"var(--sky)",lineHeight:1.9,whiteSpace:"pre-wrap",userSelect:"all" }}>{`rules_version = '2';\nservice cloud.firestore {\n  match /databases/{database}/documents {\n    function isAdmin() {\n      return request.auth != null &&\n        request.auth.token.email == "${ADMIN_EMAIL}";\n    }\n    match /users/{userId} {\n      allow read, write: if request.auth.uid == userId || isAdmin();\n      match /projects/{projectId} {\n        allow read, write: if request.auth.uid == userId || isAdmin();\n      }\n    }\n    match /adminNotifications/{docId} {\n      allow read, write: if isAdmin() || request.auth != null;\n    }\n  }\n}`}</div>
              <Btn onClick={onClose} style={{ width:"100%",marginTop:16 }} variant="secondary">Close</Btn>
            </div>
          ) : filteredUsers.length === 0 ? (
            <div style={{ textAlign:"center",color:"var(--text2)",padding:"40px 0",fontSize:13 }}>{search ? "No users match" : "No users found"}</div>
          ) : filteredUsers.map(u => (
            <div key={u.id} onClick={() => setImpersonating(u)} style={{ display:"flex",alignItems:"center",gap:12,padding:"13px 15px",borderRadius:16,marginBottom:8,background:u.blocked?"color-mix(in srgb,var(--red) 5%,var(--surface2))":"var(--surface2)",border:`1.5px solid ${u.blocked?"color-mix(in srgb,var(--red) 22%,var(--border))":"var(--border)"}`,cursor:"pointer",transition:"opacity 0.15s" }}>
              <div style={{ width:42,height:42,borderRadius:14,flexShrink:0,background:u.blocked?"color-mix(in srgb,var(--red) 18%,var(--surface))":"linear-gradient(135deg,#38bdf8,#0284c7)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:u.blocked?18:17,fontWeight:800,color:"#fff" }}>
                {u.blocked ? "🚫" : (u.name||"?")[0].toUpperCase()}
              </div>
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ fontWeight:700,fontSize:13.5,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap" }}>
                  {u.name}
                  {u.email===ADMIN_EMAIL&&<Pill color="var(--yellow)">Admin</Pill>}
                  {u.blocked&&<Pill color="var(--red)">Blocked</Pill>}
                </div>
                <div style={{ color:"var(--text2)",fontSize:11.5,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginTop:1 }}>{u.email}</div>
              </div>
              <div style={{ color:"var(--text3)",fontSize:18 }}>›</div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
};

// ─── SVG Icons ────────────────────────────────────────────────────────────────
const SvgIcon = ({ children, size=20, stroke="currentColor", fill="none", sw=2 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill={fill} stroke={stroke}
    strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round">{children}</svg>
);
const IcoHome     = ({s=20,c="currentColor"}) => <SvgIcon size={s} stroke={c}><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></SvgIcon>;
const IcoPlus     = ({s=20,c="currentColor"}) => <SvgIcon size={s} stroke={c}><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></SvgIcon>;
const IcoPencil   = ({s=20,c="currentColor"}) => <SvgIcon size={s} stroke={c}><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></SvgIcon>;
const IcoBar      = ({s=20,c="currentColor"}) => <SvgIcon size={s} stroke={c}><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></SvgIcon>;
const IcoTrophy   = ({s=20,c="currentColor"}) => <SvgIcon size={s} stroke={c}><path d="M8 21h8"/><path d="M12 17v4"/><path d="M7 4v6a5 5 0 0 0 10 0V4"/><line x1="3" y1="8" x2="7" y2="8"/><line x1="17" y1="8" x2="21" y2="8"/></SvgIcon>;
const IcoSettings = ({s=20,c="currentColor"}) => <SvgIcon size={s} stroke={c}><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></SvgIcon>;
const IcoDoc      = ({s=20,c="currentColor"}) => <SvgIcon size={s} stroke={c}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></SvgIcon>;
const IcoUsers    = ({s=20,c="currentColor"}) => <SvgIcon size={s} stroke={c}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></SvgIcon>;
const IcoSun      = ({s=16}) => <SvgIcon size={s}><circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="1" y1="12" x2="3" y2="12"/><line x1="21" y1="12" x2="23" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></SvgIcon>;
const IcoMoon     = ({s=16}) => <SvgIcon size={s}><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></SvgIcon>;

// ─── Dock ─────────────────────────────────────────────────────────────────────
const Dock = ({ page, go, isAdmin, onAdminOpen }) => {
  const tabs = [
    { id: "home",     icon: <IcoHome s={18}/>,     label: "Home" },
    { id: "setup",    icon: <IcoPlus s={18}/>,     label: "New" },
    { id: "app",      icon: <IcoPencil s={18}/>,   label: "Entry" },
    { id: "summary",  icon: <IcoBar s={18}/>,      label: "Stats" },
    { id: "merit",    icon: <IcoTrophy s={18}/>,   label: "Merit" },
    { id: "settings", icon: <IcoSettings s={18}/>, label: "Settings" },
  ];
  return (
    <nav style={{
      position: "fixed", bottom: 0, left: 0, right: 0,
      display: "flex", gap: 0, padding: "6px 4px 10px",
      background: "var(--surface)",
      borderTop: "1.5px solid var(--border)",
      boxShadow: "0 -4px 24px rgba(0,0,0,0.18)",
      zIndex: 1000,
    }}>
      {tabs.map(t => {
        const active = page === t.id;
        return (
          <button key={t.id} onClick={() => go(t.id)} style={{
            display: "flex", flexDirection: "column", alignItems: "center", gap: 2,
            padding: "6px 4px", borderRadius: 10, border: "none", flex: 1,
            background: "transparent",
            color: active ? "var(--sky)" : "var(--text2)",
            fontSize: 16, transition: "all 0.18s", cursor: "pointer",
          }}>
            <span style={{ display:"flex", alignItems:"center", justifyContent:"center", transition: "all 0.18s", opacity: active ? 1 : 0.6, color: active ? "var(--sky)" : "var(--text2)" }}>{t.icon}</span>
            <span style={{ fontSize: 9, fontWeight: active ? 700 : 500, letterSpacing: "0.03em" }}>{t.label}</span>
          </button>
        );
      })}
      {isAdmin && (
        <button onClick={onAdminOpen} style={{
          display: "flex", flexDirection: "column", alignItems: "center", gap: 2,
          padding: "6px 4px", borderRadius: 10, border: "none", flex: 1,
          background: "transparent", color: "var(--yellow)", transition: "all 0.18s", cursor: "pointer",
        }}>
          <span style={{fontSize:18}}>⚡</span>
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "0.03em" }}>Admin</span>
        </button>
      )}
    </nav>
  );
};
// ─── PDF: Marksheet ───────────────────────────────────────────────────────────
const downloadMarksheet = async (student, project, showToast) => {
  if (!student) return showToast("⚠ No student selected");
  showToast("⏳ Generating marksheet…");

  const loadScript = (src) => new Promise((res, rej) => {
    if (document.querySelector(`script[src="${src}"]`)) { res(); return; }
    const s = document.createElement("script");
    s.src = src; s.onload = res; s.onerror = rej;
    document.head.appendChild(s);
  });
  try {
    await loadScript("https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js");
    await loadScript("https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.8.2/jspdf.plugin.autotable.min.js");
  } catch { return showToast("✗ Could not load PDF library"); }

  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const W = doc.internal.pageSize.getWidth();
  const H = doc.internal.pageSize.getHeight();

  const BLACK  = [0, 0, 0];
  const WHITE  = [255, 255, 255];
  const LGREY  = [245, 245, 245];
  const MGREY  = [200, 200, 200];
  const DGREY  = [80, 80, 80];
  const DARK   = [15, 28, 46];
  const GREEN  = [16, 185, 129];
  const RED    = [239, 68, 68];

  // ── Outer border ─────────────────────────────────────────────────────────────
  doc.setDrawColor(...BLACK);
  doc.setLineWidth(1.2);
  doc.rect(8, 8, W - 16, H - 16, "S");

  // ── Header: Institution name ──────────────────────────────────────────────────
  const institution = project.institution || "Institution";
  const examName    = project.examName    || "Annual Progress Report";
  const className   = project.className   || "";

  // Grade-based color accent
  const gradeAccent = student.hasZeroGP ? [220,38,38] :
    student.grade==="A+"||student.grade==="A" ? [5,150,105] :
    student.grade==="A-"||student.grade==="B" ? [14,100,180] :
    student.grade==="C" ? [180,130,0] : [107,114,128];
  // Top color strip
  doc.setFillColor(...gradeAccent.map(v=>Math.min(255,Math.round(v*0.8+150))));
  doc.rect(0,0,W,3,"F");
  doc.setFillColor(...gradeAccent.map(v=>Math.min(255,Math.round(v*0.6+170))));
  doc.rect(0,3,W,1.5,"F");
  doc.setFont("helvetica", "bold");
  doc.setFontSize(18);
  doc.setTextColor(...gradeAccent);
  doc.text(institution, W / 2, 21, { align: "center" });

  doc.setFont("helvetica", "normal");
  doc.setFontSize(9);
  doc.setTextColor(...DGREY);
  doc.text(className || "—", W / 2, 27, { align: "center" });

  doc.setFont("helvetica", "bold");
  doc.setFontSize(13);
  doc.setTextColor(...DARK);
  doc.text("ANNUAL", W / 2, 34, { align: "center" });
  doc.text("PROGRESS REPORT", W / 2, 40, { align: "center" });

  // ── Grade Interval table (top right) ─────────────────────────────────────────
  const gi = project.gradingScale || [
    { min:80, max:100, gp:5, grade:"A+" }, { min:70, max:79, gp:4, grade:"A" },
    { min:60, max:69, gp:3.5, grade:"A-" }, { min:50, max:59, gp:3, grade:"B" },
    { min:40, max:49, gp:2, grade:"C" }, { min:33, max:39, gp:1, grade:"D" }, { min:0, max:32, gp:0, grade:"F" },
  ];
  const gx = W - 70, gy = 12, gw = 60, gh = 5;
  doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(...DARK);
  doc.text("Grace Interval", gx + gw / 2, gy + 4, { align: "center" });
  // header row
  doc.setDrawColor(...MGREY); doc.setLineWidth(0.3);
  doc.rect(gx, gy + 5.5, gw, gh, "FD");
  doc.setFillColor(...LGREY); doc.rect(gx, gy + 5.5, gw, gh, "F");
  doc.setFont("helvetica", "bold"); doc.setFontSize(6); doc.setTextColor(...DARK);
  doc.text("Grade Interval", gx + 14, gy + 9.3, { align: "center" });
  doc.text("GPA",            gx + 42, gy + 9.3, { align: "center" });
  doc.text("LG",             gx + 56, gy + 9.3, { align: "center" });
  // data rows
  const sortedGi = [...gi].sort((a, b) => b.min - a.min);
  sortedGi.forEach((row, idx) => {
    const ry = gy + 5.5 + (idx + 1) * gh;
    if (idx % 2 === 1) { doc.setFillColor(248, 248, 248); doc.rect(gx, ry, gw, gh, "F"); }
    doc.setDrawColor(...MGREY); doc.rect(gx, ry, gw, gh, "S");
    doc.setFont("helvetica", "normal"); doc.setFontSize(6); doc.setTextColor(...DARK);
    const nextRow = sortedGi[idx + 1];
    const minLabel = row.max != null ? `${row.min} – ${row.max}` : (!nextRow ? `00 – ${row.min}` : (idx === 0 ? `${row.min} – 100` : `${nextRow.min} – ${row.min}`));
    doc.text(minLabel,       gx + 14, ry + 3.6, { align: "center" });
    doc.text(String(row.gp), gx + 42, ry + 3.6, { align: "center" });
    doc.text(row.grade,      gx + 56, ry + 3.6, { align: "center" });
  });

  // ── Student Info fields ───────────────────────────────────────────────────────
  const infoY = 46;
  doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(...DARK);
  doc.text("Student Name  :", 12, infoY + 6);
  doc.text("Class             :", 12, infoY + 13);
  doc.text("Roll No          :", 12, infoY + 20);

  doc.setFont("helvetica", "bold");
  doc.text(student.name, 48, infoY + 6);
  doc.text(className || "—", 48, infoY + 13);
  doc.text(String(student.roll), 48, infoY + 20);

  const subjectNames = (project.subjectNames && project.subjectNames.length > 0) ? project.subjectNames : Array.from({ length: project.subjectCount }, (_, i) => `Subject ${i + 1}`);
  doc.setFont("helvetica", "normal"); doc.setFontSize(9); doc.setTextColor(...DARK);
  doc.text("Total Subjects  :", W / 2 - 10, infoY + 13);
  doc.text("Total Marks     :", W / 2 - 10, infoY + 20);
  doc.setFont("helvetica", "bold");
  doc.text(String(subjectNames.length), W / 2 + 28, infoY + 13);
  const maxTotalMarks = (project.subjectSettings || []).reduce((acc, ss) => {
    return acc + Math.round((ss.caFull || 30) + ((ss.examPct || 70) / 100) * (ss.examFull || 100));
  }, 0) || (project.subjectCount * 100);
  doc.text(String(maxTotalMarks), W / 2 + 28, infoY + 20);

  // underlines
  doc.setDrawColor(...DARK); doc.setLineWidth(0.3);
  doc.line(48, infoY + 7, 100, infoY + 7);
  doc.line(48, infoY + 14, 85, infoY + 14);
  doc.line(48, infoY + 21, 85, infoY + 21);
  doc.line(W / 2 + 28, infoY + 14, W - 12, infoY + 14);
  doc.line(W / 2 + 28, infoY + 21, W - 12, infoY + 21);

  // ── Subject Marks Table ───────────────────────────────────────────────────────
  const subSettings = project.subjectSettings || Array(project.subjectCount).fill(null).map(() => ({ caFull: 30, examFull: 100, examPct: 70 }));
  const gpFn    = makeGetGP(project.gradingScale || null);
  const gradeFn = makeGetGrade(project.gradingScale || null);

  // Columns matching B&W: Subject | Full Marks | (A) CA | MCQ/Oral | Creative/Written | Total | (B) | Obtained A+B | GP
  const tableHead = [[
    "Subject",
    "Full\nMarks",
    "(A)\nCA",
    "MCQ",
    "CQ /\nWritten",
    "Total",
    "(B)",
    "Obtained\nMarks\nA+B",
    "GP",
  ]];

  const tableBody = subjectNames.map((subName, i) => {
    const sr  = (student.subjectResults || [])[i] || {};
    const ss  = subSettings[i] || { caFull: 30, examFull: 100, examPct: 70 };
    const subFull = Math.round((ss.caFull || 30) + ((ss.examPct || 70) / 100) * (ss.examFull || 100));
    const ca      = sr.ca      != null ? Math.round(sr.ca)  : "-";
    const mcq     = sr.mcq     != null ? sr.mcq             : "-";
    const cq      = sr.cq      != null ? sr.cq              : "-";
    const rawExam = sr.rawExam != null ? sr.rawExam         : "-";
    const b       = sr.b       != null ? Math.round(sr.b)   : "-";
    const ab      = sr.ab      != null ? Math.round(sr.ab)  : "-";
    const gp      = sr.gp      != null ? sr.gp              : "-";

    return [
      subName || `Subject ${i + 1}`,
      subFull,
      ca,
      mcq,
      cq,
      rawExam,
      b,
      { content: String(ab), styles: { fontStyle: "bold" } },
      gp,
    ];
  });

  doc.autoTable({
    head: tableHead,
    body: tableBody,
    startY: infoY + 26,
    styles: {
      font: "helvetica",
      fontSize: 8,
      cellPadding: { top: 3, bottom: 3, left: 2.5, right: 2.5 },
      textColor: DARK,
      lineColor: MGREY,
      lineWidth: 0.3,
    },
    headStyles: {
      fillColor: WHITE,
      textColor: DARK,
      fontStyle: "bold",
      fontSize: 7,
      cellPadding: { top: 2.5, bottom: 2.5, left: 2.5, right: 2.5 },
      halign: "center",
      valign: "middle",
      lineColor: BLACK,
      lineWidth: 0.4,
    },
    alternateRowStyles: { fillColor: WHITE },
    columnStyles: {
      0: { cellWidth: 'auto', fontStyle: "bold" },
      1: { halign: "center", cellWidth: 14 },
      2: { halign: "center", cellWidth: 13 },
      3: { halign: "center", cellWidth: 14 },
      4: { halign: "center", cellWidth: 18 },
      5: { halign: "center", cellWidth: 13 },
      6: { halign: "center", cellWidth: 11 },
      7: { halign: "center", cellWidth: 24, fontStyle: "bold" },
      8: { halign: "center", cellWidth: 10 },
    },
    tableWidth: W - 20,
    didParseCell: (data) => {
      if (data.section === "body") {
        if (data.column.index === 7) {
          const v = parseFloat(data.cell.raw?.content ?? data.cell.raw);
          const ss = subSettings[data.row.index] || {};
          const sf = Math.round((ss.caFull || 30) + ((ss.examPct || 70) / 100) * (ss.examFull || 100));
          const pct = sf > 0 ? (v / sf) * 100 : 0;
          data.cell.styles.textColor = pct >= 80 ? GREEN : pct < 33 ? RED : DARK;
        }
      }
    },
    margin: { left: 10, right: 10 },
    tableLineColor: BLACK,
    tableLineWidth: 0.5,
  });

  // ── Result Summary row ────────────────────────────────────────────────────────
  const rsY = doc.lastAutoTable.finalY + 8;
  const meritRanks = getMeritRank(project.students);
  const myRank     = meritRanks[String(student.roll)] || "—";

  // Three-cell summary: Total Obtained | Percentage | Total GP
  const sumW = (W - 20) / 3;
  [
    { label: "Total Obtained Marks", value: String(student.total != null ? Math.round(student.total) : "—") },
    { label: "Percentage of Marks (%)", value: (() => {
      const maxMarks = subjectNames.reduce((acc, _, i) => {
        const ss = subSettings[i] || { caFull: 30, examFull: 100, examPct: 70 };
        return acc + Math.round((ss.caFull || 30) + ((ss.examPct || 70) / 100) * (ss.examFull || 100));
      }, 0);
      return maxMarks > 0 ? ((student.total / maxMarks) * 100).toFixed(1) + "%" : "—";
    })() },
    { label: "Total GP", value: String((student.subjectResults||[]).reduce((s,r)=>s+(r.gp||0),0).toFixed(2)), large: true },
  ].forEach(({ label, value, large }, i) => {
    const tx = 10 + i * sumW;
    doc.setDrawColor(...BLACK); doc.setLineWidth(0.4);
    doc.rect(tx, rsY, sumW, 20, "S");
    doc.setFont("helvetica", "bold"); doc.setFontSize(7); doc.setTextColor(...DARK);
    doc.text(label, tx + sumW / 2, rsY + 5, { align: "center" });
    doc.setDrawColor(...MGREY); doc.setLineWidth(0.3);
    doc.line(tx + 6, rsY + 8, tx + sumW - 6, rsY + 8);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(large ? 14 : 11);
    if (large) doc.setTextColor(14, 100, 180); else doc.setTextColor(...DARK);
    doc.text(value, tx + sumW / 2, rsY + 16, { align: "center" });
  });

  // ── GPA / LG / Merit Position row ────────────────────────────────────────────
  const gpRowY = rsY + 26;
  const gpCols = [
    { label: "GPA",            value: String(student.gpa) },
    { label: "LG",             value: student.grade || gradeFn(student.gpa) },
    { label: "Merit Position", value: `#${myRank}` },
  ];
  const gpW = (W - 20) / 3;
  gpCols.forEach(({ label, value }, i) => {
    const tx = 10 + i * gpW;
    doc.setDrawColor(...BLACK); doc.setLineWidth(0.4);
    doc.rect(tx, gpRowY, gpW, 20, "S");
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.setTextColor(...DARK);
    doc.text(label, tx + gpW / 2, gpRowY + 6, { align: "center" });
    doc.setDrawColor(...MGREY); doc.setLineWidth(0.3);
    doc.line(tx + 8, gpRowY + 10, tx + gpW - 8, gpRowY + 10);
    doc.setFont("helvetica", "bold"); doc.setFontSize(10); doc.setTextColor(14, 100, 180);
    doc.text(value, tx + gpW / 2, gpRowY + 17, { align: "center" });
  });

  // ── Signature lines ───────────────────────────────────────────────────────────
  const sigY = gpRowY + 28;
  doc.setDrawColor(...DARK); doc.setLineWidth(0.3);
  doc.line(14, sigY, 70, sigY);
  doc.line(W - 70, sigY, W - 14, sigY);
  doc.setFont("helvetica", "normal"); doc.setFontSize(7.5); doc.setTextColor(...DGREY);
  doc.text("Student Signature", 42, sigY + 5, { align: "center" });
  doc.text("Authorized Signature", W - 42, sigY + 5, { align: "center" });

  doc.save(`${student.name.replace(/\s+/g, "_")}_Marksheet.pdf`);
  showToast("✓ Marksheet downloaded!");
};

// ─── PDF: Merit List — FIXED & Improved ──────────────────────────────────────
// FIX 1: Table column widths tuned so GPA, Grade, Status headers don't wrap
// FIX 2: Better logo
// NEW: Pass/fail bar chart added
const downloadMeritListPDF = async (project, showToast, sortMode = "roll") => {
  if (!project?.students.length) return showToast("⚠ No students to export");
  showToast("⏳ Generating Merit List PDF…");

  const loadScript = (src) => new Promise((res, rej) => {
    if (document.querySelector(`script[src="${src}"]`)) { res(); return; }
    const s = document.createElement("script");
    s.src = src; s.onload = res; s.onerror = rej;
    document.head.appendChild(s);
  });

  try {
    await loadScript("https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js");
    await loadScript("https://cdnjs.cloudflare.com/ajax/libs/jspdf-autotable/3.8.2/jspdf.plugin.autotable.min.js");
  } catch { return showToast("✗ Could not load PDF library"); }

  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const W = doc.internal.pageSize.getWidth();
  const H = doc.internal.pageSize.getHeight();

  const SKY     = [14, 165, 233];
  const SKY_LT  = [240, 249, 255];
  const SKY_MID = [186, 230, 253];
  const SKY_DK  = [3, 105, 161];
  const DARK    = [15, 28, 46];
  const GREY    = [100, 116, 132];
  const LGREY   = [241, 245, 249];
  const WHITE   = [255, 255, 255];
  const GREEN   = [16, 185, 129];
  const RED     = [239, 68, 68];
  const GOLD    = [180, 130, 0];
  const SILVER  = [100, 116, 139];
  const BRONZE  = [160, 80, 30];
  const INDIGO  = [99, 102, 241];

  // ── Top accent ────────────────────────────────────────────────────────────
  doc.setFillColor(...SKY_DK);
  doc.rect(0, 0, W / 2, 5, "F");
  doc.setFillColor(...SKY);
  doc.rect(W / 2, 0, W / 2, 5, "F");

  // ── Logo ──────────────────────────────────────────────────────────────────
  const logoX = 14, logoY = 10, logoSize = 16;
  doc.setFillColor(...SKY);
  doc.roundedRect(logoX, logoY, logoSize, logoSize, 3, 3, "F");
  doc.setFillColor(255, 255, 255);
  doc.rect(logoX + 2, logoY + 2.5, 5, 10.5, "F");
  doc.setFillColor(224, 242, 254);
  doc.rect(logoX + 7.5, logoY + 2.5, 5.5, 10.5, "F");
  doc.setDrawColor(...SKY_DK);
  doc.setLineWidth(0.6);
  doc.line(logoX + 7.2, logoY + 2.5, logoX + 7.2, logoY + 13);
  doc.setDrawColor(...SKY_DK);
  doc.setLineWidth(1);
  doc.line(logoX + 8.8, logoY + 8.2, logoX + 10, logoY + 9.5);
  doc.line(logoX + 10, logoY + 9.5, logoX + 12.2, logoY + 6.5);
  doc.setFillColor(255, 255, 255);
  doc.circle(logoX + 13.5, logoY + 2.5, 1.2, "F");

  doc.setFont("helvetica", "bold");
  doc.setFontSize(17);
  doc.setTextColor(...DARK);
  doc.text("EduGrade", logoX + logoSize + 4, logoY + 7);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(8.5);
  doc.setTextColor(...GREY);
  doc.text(`Merit List  ·  Sorted by ${sortMode === "merit" ? "Merit" : "Roll No."}`, logoX + logoSize + 4, logoY + 12.5);

  // ── Info badge (top right) ─────────────────────────────────────────────────
  const passCount = project.students.filter(s => s.status === "PASS").length;
  const failCount = project.students.length - passCount;
  const avgGPA = (project.students.reduce((a, s) => a + s.gpa, 0) / project.students.length).toFixed(2);
  const examBadgeLabel = project.examName.length > 22 ? project.examName.slice(0, 22) + "…" : project.examName;

  doc.setFillColor(...SKY_LT);
  doc.roundedRect(W - 72, logoY - 1, 58, 20, 3, 3, "F");
  doc.setDrawColor(...SKY_MID);
  doc.setLineWidth(0.5);
  doc.roundedRect(W - 72, logoY - 1, 58, 20, 3, 3, "S");
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.setTextColor(...GREY);
  doc.text("EXAM", W - 68, logoY + 4);
  doc.text("STUDENTS", W - 68, logoY + 12);
  doc.setFont("helvetica", "bold");
  doc.setFontSize(8.5);
  doc.setTextColor(...DARK);
  doc.text(examBadgeLabel, W - 47, logoY + 4);
  doc.text(String(project.students.length), W - 47, logoY + 12);

  // ── Divider ───────────────────────────────────────────────────────────────
  doc.setDrawColor(...SKY_MID);
  doc.setLineWidth(0.5);
  doc.line(14, 31, W - 14, 31);

  // ── Stats tiles ───────────────────────────────────────────────────────────
  const tileW = (W - 28 - 12) / 4;
  [
    { label: "TOTAL", value: project.students.length, color: SKY },
    { label: "PASSED", value: passCount, color: GREEN },
    { label: "FAILED", value: failCount, color: RED },
    { label: "AVG GPA", value: avgGPA, color: INDIGO },
  ].forEach(({ label, value, color }, i) => {
    const tx = 14 + i * (tileW + 4);
    doc.setFillColor(...SKY_LT);
    doc.roundedRect(tx, 35, tileW, 15, 2, 2, "F");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(6.5);
    doc.setTextColor(...GREY);
    doc.text(label, tx + tileW / 2, 40.5, { align: "center" });
    doc.setFont("helvetica", "bold");
    doc.setFontSize(11);
    doc.setTextColor(...color);
    doc.text(String(value), tx + tileW / 2, 47, { align: "center" });
  });

  // ── Pass rate bar ─────────────────────────────────────────────────────────
  const barY = 55;
  const passRate = (passCount / project.students.length);
  doc.setFillColor(...LGREY);
  doc.roundedRect(14, barY, W - 28, 5, 2, 2, "F");
  if (passRate > 0) {
    doc.setFillColor(...GREEN);
    doc.roundedRect(14, barY, (W - 28) * passRate, 5, 2, 2, "F");
  }
  doc.setFont("helvetica", "normal");
  doc.setFontSize(6.5);
  doc.setTextColor(...GREY);
  doc.text(`Pass rate: ${(passRate * 100).toFixed(1)}%`, 16, barY + 3.5);

  // ── Table — FIX: explicit column widths prevent header text wrapping ───────
  const sortedRows = sortMode === "merit"
    ? [...project.students].sort((a, b) => b.gpa - a.gpa || b.total - a.total)
    : [...project.students].sort((a, b) =>
        String(a.roll).localeCompare(String(b.roll), undefined, { numeric: true })
      );
  const meritRanks = getMeritRank(project.students);

  // Real max marks per subject (respects each subject's own CA/exam settings —
  // previously this assumed every subject was flatly out of 100).
  const maxMarks = Array.from({ length: project.subjectCount || 0 }).reduce((acc, _, i) => {
    const ss = project.subjectSettings?.[i] || { caFull: 30, examFull: 100, examPct: 70 };
    return acc + Math.round((ss.caFull || 30) + ((ss.examPct || 70) / 100) * (ss.examFull || 100));
  }, 0);

  // Total printable width = 210 - 2*12 = 186mm
  // Col widths sum should equal 186
  const colWidths = {
    0: 16,  // Rank
    1: 52,  // Name
    2: 18,  // Roll No.
    3: 18,  // Total
    4: 16,  // GPA
    5: 16,  // Grade
    6: 22,  // Percent
    7: 28,  // Status
  };

  const head = [["Rank", "Name", "Roll", "Total", "GPA", "Grade", "Percent", "Status"]];
  const body = sortedRows.map(s => [
    `#${meritRanks[s.roll]}`,
    s.name,
    s.roll,
    s.total,
    s.gpa,
    s.grade,
    `${maxMarks > 0 ? ((s.total / maxMarks) * 100).toFixed(1) : "0.0"}%`,
    s.status,
  ]);

  doc.autoTable({
    head,
    body,
    startY: 64,
    // FIX: Use smaller font and tight padding in header so text never wraps
    styles: {
      font: "helvetica",
      fontSize: 8.5,
      cellPadding: { top: 3.5, bottom: 3.5, left: 3.5, right: 3.5 },
      overflow: "hidden",
    },
    headStyles: {
      fillColor: SKY,
      textColor: WHITE,
      fontStyle: "bold",
      fontSize: 7.5,           // ← Smaller so single-word headers fit
      cellPadding: { top: 3, bottom: 3, left: 3.5, right: 3.5 },
      minCellHeight: 8,        // ← Controlled height prevents over-expansion
      valign: "middle",
      halign: "center",
    },
    alternateRowStyles: { fillColor: SKY_LT },
    columnStyles: {
      0: { halign: "center", cellWidth: colWidths[0] },
      1: { cellWidth: colWidths[1] },
      2: { halign: "center", cellWidth: colWidths[2] },
      3: { halign: "center", cellWidth: colWidths[3] },
      4: { halign: "center", cellWidth: colWidths[4] },
      5: { halign: "center", cellWidth: colWidths[5] },
      6: { halign: "center", cellWidth: colWidths[6] },
      7: { halign: "center", cellWidth: colWidths[7] },
    },
    didParseCell: (data) => {
      if (data.section === "body") {
        const rankRaw = data.row.raw[0];
        // Subtle full-row tint for the top 3 ranks, overriding the zebra stripe.
        if (rankRaw === "#1") data.cell.styles.fillColor = [255, 251, 235];
        else if (rankRaw === "#2") data.cell.styles.fillColor = [248, 250, 252];
        else if (rankRaw === "#3") data.cell.styles.fillColor = [255, 247, 237];
        if (data.column.index === 0) {
          const raw = data.cell.raw;
          if (raw === "#1") { data.cell.styles.textColor = GOLD; data.cell.styles.fontStyle = "bold"; }
          else if (raw === "#2") { data.cell.styles.textColor = SILVER; data.cell.styles.fontStyle = "bold"; }
          else if (raw === "#3") { data.cell.styles.textColor = BRONZE; data.cell.styles.fontStyle = "bold"; }
          else { data.cell.styles.textColor = GREY; }
        }
        if (data.column.index === 7) {
          data.cell.styles.textColor = data.cell.raw === "PASS" ? GREEN : RED;
          data.cell.styles.fontStyle = "bold";
        }
        if (data.column.index === 5) {
          const gradeColors = {
            "A+": SKY, "A": SKY, "A-": [2, 132, 199], "B": INDIGO,
            "C": [245, 158, 11], "D": [251, 146, 60], "F": RED,
          };
          data.cell.styles.textColor = gradeColors[data.cell.raw] || DARK;
          data.cell.styles.fontStyle = "bold";
        }
      }
    },
    margin: { left: 12, right: 12 },
    didDrawPage: (data) => {
      // Repeat logo on each page
      if (data.pageNumber > 1) {
        doc.setFillColor(...SKY_DK);
        doc.rect(0, 0, W / 2, 3, "F");
        doc.setFillColor(...SKY);
        doc.rect(W / 2, 0, W / 2, 3, "F");
      }
    },
  });

  // ── Footer on all pages ────────────────────────────────────────────────────
  const generatedOn = new Date().toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" });
  const pageCount = doc.internal.getNumberOfPages();
  for (let i = 1; i <= pageCount; i++) {
    doc.setPage(i);
    doc.setFillColor(...SKY_DK);
    doc.rect(0, H - 10, W / 2, 10, "F");
    doc.setFillColor(...SKY);
    doc.rect(W / 2, H - 10, W / 2, 10, "F");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    doc.setTextColor(...WHITE);
    doc.text(`EduGrade — Academic Excellence Platform  ·  Generated ${generatedOn}`, 14, H - 3.5);
    doc.text(`Page ${i} of ${pageCount}`, W - 14, H - 3.5, { align: "right" });
  }

  doc.save(`${project.examName.replace(/\s+/g, "_")}_Merit_List.pdf`);
  showToast("✓ Merit List PDF downloaded!");
};
// ─── Firebase Loader ──────────────────────────────────────────────────────────
const useFirebase = () => {
  const [firebase, setFirebase] = useState(null);
  const [fbReady,  setFbReady]  = useState(false);
  const [fbError,  setFbError]  = useState(false);

  useEffect(() => {
    try {
      const app = getApps().length
        ? getApps()[0]
        : initializeApp(FIREBASE_CONFIG);
      const auth = getAuth(app);
      const db   = getFirestore(app);
      // Wrap in compat-style object so rest of code works unchanged
      const fbAuth = {
        signInWithEmailAndPassword: (email, pass) =>
          import('firebase/auth').then(({ signInWithEmailAndPassword }) =>
            signInWithEmailAndPassword(auth, email, pass)),
        createUserWithEmailAndPassword: (email, pass) =>
          import('firebase/auth').then(({ createUserWithEmailAndPassword }) =>
            createUserWithEmailAndPassword(auth, email, pass)),
        sendPasswordResetEmail: (email) =>
          import('firebase/auth').then(({ sendPasswordResetEmail }) =>
            sendPasswordResetEmail(auth, email)),
        signOut: () => import('firebase/auth').then(({ signOut }) => signOut(auth)),
        onAuthStateChanged: (cb) => {
          let unsub;
          import('firebase/auth').then(({ onAuthStateChanged }) => {
            unsub = onAuthStateChanged(auth, cb);
          });
          return () => unsub && unsub();
        },
        currentUser: auth.currentUser,
      };
      const fbDb = {
        collection: (col) => ({
          doc: (id) => ({
            get: () => import('firebase/firestore').then(({ doc, getDoc }) =>
              getDoc(doc(db, col, id)).then(s => ({
                exists: s.exists(), data: () => s.data(), id: s.id
              }))),
            set: (data) => import('firebase/firestore').then(({ doc, setDoc }) =>
              setDoc(doc(db, col, id), data)),
            update: (data) => import('firebase/firestore').then(({ doc, updateDoc }) =>
              updateDoc(doc(db, col, id), data)),
            delete: () => import('firebase/firestore').then(({ doc, deleteDoc }) =>
              deleteDoc(doc(db, col, id))),
            collection: (subCol) => ({
              add: (data) => import('firebase/firestore').then(({ collection: col2, addDoc }) =>
                addDoc(col2(db, col, id, subCol), data).then(r => ({ id: r.id }))),
              get: () => import('firebase/firestore').then(({ collection: col2, getDocs }) =>
                getDocs(col2(db, col, id, subCol)).then(s => ({
                  docs: s.docs.map(d => ({ id: d.id, data: () => d.data(), exists: d.exists() }))
                }))),
              orderBy: (field, dir) => ({
                get: () => import('firebase/firestore').then(({ collection: col2, query, orderBy, getDocs }) =>
                  getDocs(query(col2(db, col, id, subCol), orderBy(field, dir || 'asc'))).then(s => ({
                    docs: s.docs.map(d => ({ id: d.id, data: () => d.data(), exists: d.exists() }))
                  }))),
              }),
              doc: (sid) => ({
                update: (data) => import('firebase/firestore').then(({ doc, updateDoc }) =>
                  updateDoc(doc(db, col, id, subCol, sid), data)),
                delete: () => import('firebase/firestore').then(({ doc, deleteDoc }) =>
                  deleteDoc(doc(db, col, id, subCol, sid))),
              }),
            }),
          }),
          add: (data) => import('firebase/firestore').then(({ collection: col2, addDoc }) =>
            addDoc(col2(db, col), data).then(r => ({ id: r.id }))),
          get: () => import('firebase/firestore').then(({ collection: col2, getDocs }) =>
            getDocs(col2(db, col)).then(s => ({
              docs: s.docs.map(d => ({ id: d.id, data: () => d.data(), exists: d.exists() }))
            }))),
          orderBy: (field, dir) => ({
            limit: (n) => ({
              get: () => import('firebase/firestore').then(({ collection: col2, query, orderBy, limit, getDocs }) =>
                getDocs(query(col2(db, col), orderBy(field, dir || 'desc'), limit(n))).then(s => ({
                  docs: s.docs.map(d => ({ id: d.id, data: () => d.data() }))
                }))),
            }),
            desc: () => ({
              limit: (n) => ({
                get: () => import('firebase/firestore').then(({ collection: col2, query, orderBy, limit, getDocs }) =>
                  getDocs(query(col2(db, col), orderBy(field, 'desc'), limit(n))).then(s => ({
                    docs: s.docs.map(d => ({ id: d.id, data: () => d.data() }))
                  }))),
              }),
            }),
            get: () => import('firebase/firestore').then(({ collection: col2, query, orderBy, getDocs }) =>
              getDocs(query(col2(db, col), orderBy(field, dir || 'asc'))).then(s => ({
                docs: s.docs.map(d => ({ id: d.id, data: () => d.data() }))
              }))),
          }),
        }),
      };
      // Atomic read-modify-write for a project's `students` array.
      // Prevents rapid successive saves from overwriting each other's data
      // (previously each save wrote the whole array from stale local state).
      fbDb.mutateStudents = (uid, projectId, mutateFn) =>
        import('firebase/firestore').then(({ doc, runTransaction }) =>
          runTransaction(db, async (tx) => {
            const ref = doc(db, "users", uid, "projects", projectId);
            const snap = await tx.get(ref);
            const data = snap.data() || {};
            const current = data.students || [];
            const next = mutateFn(current);
            tx.update(ref, { students: next });
            return next;
          }));
      // Reorder a project's subjects. `newOrder` is an array of the OLD subject
      // indices in their desired NEW sequence (e.g. [2,0,1] moves subject 3 to
      // first place). Reads the latest data inside a transaction so it can't
      // race with a concurrent mark-entry save, and re-maps every existing
      // student's subjectResults/finalMarks so their marks stay attached to
      // the correct subject after the move — marks/scores themselves never change.
      fbDb.reorderSubjects = (uid, projectId, newOrder, newNames) =>
        import('firebase/firestore').then(({ doc, runTransaction }) =>
          runTransaction(db, async (tx) => {
            const ref = doc(db, "users", uid, "projects", projectId);
            const snap = await tx.get(ref);
            const data = snap.data() || {};
            const defaultSS = { caFull: 30, examFull: 100, examPct: 70 };
            const oldSettings = data.subjectSettings || [];
            const oldStudents = data.students || [];
            const newSettings = newOrder.map(i => oldSettings[i] || defaultSS);
            const newStudents = oldStudents.map(st => ({
              ...st,
              subjectResults: newOrder.map(i => (st.subjectResults || [])[i] ?? null),
              finalMarks: newOrder.map(i => (st.finalMarks || [])[i] ?? 0),
            }));
            const payload = { subjectNames: newNames, subjectSettings: newSettings, students: newStudents };
            tx.update(ref, payload);
            return payload;
          }));
      setFirebase({ auth: fbAuth, db: fbDb });
      setFbReady(true);
    } catch(e) { console.error('Firebase init error:', e); setFbError(true); }
  }, []);

  return { firebase, fbReady, fbError };
};

// ─── Main App ─────────────────────────────────────────────────────────────────
export default function Home() {
  const { firebase, fbReady, fbError } = useFirebase();

  const [splash,   setSplash]   = useState(true);
  const [page,     setPage]     = useState("home");
  const [dark,     setDark]     = useState(true);
  const [toast,    setToast]    = useState("");
  const [showAdmin,setShowAdmin]= useState(false);
  const [isAccountBlocked, setIsAccountBlocked] = useState(false);

  const [user,      setUser]      = useState(null);
  const [userData,  setUserData]  = useState(null);
  const [authReady, setAuthReady] = useState(false);
  const [projects,  setProjects]  = useState([]);
  const [activeProjectIndex, setActiveProjectIndex] = useState(null);

  // ── Setup form state ───────────────────────────────────────────────
  const [institution,   setInstitution]   = useState("");
  const [className,     setClassName]     = useState("");
  const [examName,      setExamName]      = useState("");
  const [subjectCount,  setSubjectCount]  = useState("");
  const [subjectNames,  setSubjectNames]  = useState([]);
  const [numAssignments,setNumAssignments]= useState(1);
  const [assignOutOf,   setAssignOutOf]   = useState(20);
  const [passThreshold, setPassThreshold] = useState(33);
  const [subjectSettings, setSubjectSettings] = useState([]); // [{caFull,examFull,examPct}]
  const [showGradingEditor,setShowGradingEditor]=useState(false);
  const [customGrading,    setCustomGrading]    =useState(null);
  const [renamingProject,  setRenamingProject]  =useState(null);
  const [renameValue,      setRenameValue]      =useState("");

  // ── Entry form state ───────────────────────────────────────────────
  const [studentName,   setStudentName]   = useState("");
  const [roll,          setRoll]          = useState("");
  const [subjectEntries,setSubjectEntries]= useState([]); // [{assignments:[], mcq:'', cq:''}]
  const [viewStudent,   setViewStudent]   = useState(null);
  const [confirmModal,  setConfirmModal]  = useState(null);
  const [searchRoll,    setSearchRoll]    = useState("");
  const [saving,        setSaving]        = useState(false);

  // subject reorder — setup form (pre-creation) drag state
  const [setupDragIdx, setSetupDragIdx] = useState(null);
  const setupDragStartY = useRef(0);

  // subject reorder — existing project modal
  const [reorderModalOpen, setReorderModalOpen] = useState(false);
  const [reorderList,      setReorderList]      = useState([]); // [{origIndex, name}]
  const [dragIdx,          setDragIdx]           = useState(null);
  const dragStartY = useRef(0);
  const [studentModal,  setStudentModal]  = useState(null);

  const showToast = useCallback((msg)=>{ setToast(msg); setTimeout(()=>setToast(""),2500); },[]);

  useEffect(()=>{
    const d=localStorage.getItem("edu_dark");
    if(d!==null) setDark(JSON.parse(d));
  },[]);
  useEffect(()=>{ localStorage.setItem("edu_dark",JSON.stringify(dark)); },[dark]);

  useEffect(()=>{
    if(!firebase) return;
    const unsub=firebase.auth.onAuthStateChanged(async(u)=>{
      setUser(u); setAuthReady(true);
      if(u){
        const snap=await firebase.db.collection("users").doc(u.uid).get();
        if(snap.exists){
          const data=snap.data();
          setUserData(data);
          if(data.blocked){ setIsAccountBlocked(true); return; }
        }
        try{
          setProjectsLoading(true);
          const projSnap=await firebase.db.collection("users").doc(u.uid)
            .collection("projects").orderBy("createdAt","asc").get();
          setProjects(projSnap.docs.map(d=>({id:d.id,...d.data()})));
          setProjectsLoading(false);
        }catch(e){ console.error(e); setProjectsLoading(false); }
      } else {
        setProjects([]); setActiveProjectIndex(null); setUserData(null);
      }
    });
    return()=>unsub();
  },[firebase]);

  const [projectsLoading, setProjectsLoading] = useState(false);
  const isAdmin = user?.email===ADMIN_EMAIL;

  const go=useCallback((p)=>{ setPage(p); },[]);

  const activeProject = projects[activeProjectIndex]??null;

  // ── Helper: fresh entry state for a project ───────────────────────
  function freshEntries(proj){
    const n=proj.subjectCount||0;
    const na=proj.numAssignments||1;
    return Array(n).fill(null).map(()=>({
      assignments: Array(na).fill(""),
      mcq: "", cq: "",
    }));
  }

  // ── Calculation ───────────────────────────────────────────────────
  function calcSubject(entry, ss, proj){
    const caFull   = Number(ss.caFull??30);
    const examFull = Number(ss.examFull??100);
    const examPct  = Number(ss.examPct??70);
    const na       = Number(proj.numAssignments||1);
    const aoof     = Number(proj.assignOutOf||20);
    const maxAssign= na*aoof;
    const assignTotal=(entry.assignments||[]).reduce((s,v)=>s+Number(v||0),0);
    const ca   = Math.round(maxAssign>0?(assignTotal/maxAssign)*caFull:0);
    const mcq  = Number(entry.mcq||0);
    const cq   = Number(entry.cq||0);
    const rawExam = mcq+cq;
    const b    = Math.round(rawExam*(examPct/100)+1e-9);
    const ab   = ca+b;
    const subFull = caFull+(examPct/100)*examFull;
    const pct  = subFull>0?(ab/subFull)*100:0;
    return {ca,mcq,cq,rawExam,b,ab,subFull,pct,assignTotal,assignments:entry.assignments||[]};
  }

  function calcStudent(entries, proj){
    const scale    = proj?.gradingScale||null;
    const gpFn     = makeGetGP(scale);
    const gradeFn  = makeGetGrade(scale);
    const thresh   = proj?.passThreshold??33;
    const ss       = proj?.subjectSettings||[];
    const subResults = entries.map((entry,i)=>{
      const s=ss[i]||{caFull:30,examFull:100,examPct:70};
      const r=calcSubject(entry,s,proj);
      // Exam component (MCQ+CQ, before the examPct scaling) must independently
      // clear the pass threshold against the exam's full marks — a student
      // can't pass a subject on CA marks alone if their raw exam score is
      // below the threshold, even if the combined mark clears it.
      const examFullN   = Number(s.examFull??100);
      const examOnlyPct = examFullN>0 ? (r.rawExam/examFullN)*100 : 0;
      const examPassed   = examOnlyPct>=Number(thresh);
      const overallPassed = r.pct>=Number(thresh);
      const pass = examPassed && overallPassed;
      const gp    = pass ? gpFn(r.pct) : 0;
      const grade = pass ? gradeFn(gp) : "F";
      return {...r,gp,grade,pass,examOnlyPct};
    });
    const total=Math.round(subResults.reduce((s,r)=>s+r.ab,0));
    const fail=subResults.some(r=>!r.pass);
    const avgGP=subResults.reduce((s,r)=>s+r.gp,0)/(subResults.length||1);
    const gpa=fail?0:Math.min(parseFloat(avgGP.toFixed(2)),5);
    const grade=fail?"F":gradeFn(gpa);
    return {gpa,grade,total,subResults,status:fail?"FAIL":"PASS"};
  }

  // ── Create Project ─────────────────────────────────────────────────
  const createProject = async()=>{
    if(!examName.trim()||!subjectCount) return showToast("⚠ Fill exam name and subject count");
    const n=Number(subjectCount);
    if(n<1||n>20) return showToast("⚠ Subjects: 1–20");
    setSaving(true);
    try{
      const finalSettings=Array(n).fill(null).map((_,i)=>({
        caFull:   Number(subjectSettings[i]?.caFull  ??30),
        examFull: Number(subjectSettings[i]?.examFull??100),
        examPct:  Number(subjectSettings[i]?.examPct ??70),
      }));
      const np={
        institution: institution.trim(),
        className:   className.trim(),
        examName:    examName.trim(),
        subjectCount:n,
        subjectNames:subjectNames.filter(s=>s.trim()),
        numAssignments:Number(numAssignments)||1,
        assignOutOf:   Number(assignOutOf)||20,
        passThreshold: Number(passThreshold)||33,
        subjectSettings:finalSettings,
        gradingScale:showGradingEditor&&customGrading?customGrading:null,
        students:[],
        createdAt:new Date().toISOString(),
      };
      const ref=await firebase.db.collection("users").doc(user.uid).collection("projects").add(np);
      const newP={...np,id:ref.id};
      const updated=[...projects,newP];
      setProjects(updated);
      setActiveProjectIndex(updated.length-1);
      setSubjectEntries(freshEntries(newP));
      // reset form
      setInstitution("");setClassName("");setExamName("");setSubjectCount("");
      setSubjectNames([]);setSubjectSettings([]);setNumAssignments(1);setAssignOutOf(20);
      setPassThreshold(33);setShowGradingEditor(false);setCustomGrading(null);
      go("app"); showToast("✓ Project created");
    }catch(e){ showToast("✗ "+e.message); }
    setSaving(false);
  };

  const deleteProject=async(id)=>{
    setConfirmModal({title:"Delete Project",msg:"Delete this project and ALL student data? This cannot be undone.",ok:"Delete",onOk:async()=>{ setConfirmModal(null);
    try{
      await firebase.db.collection("users").doc(user.uid).collection("projects").doc(id).delete();
      const updated=projects.filter(p=>p.id!==id);
      setProjects(updated);
      if(activeProject?.id===id) setActiveProjectIndex(null);
      showToast("✓ Deleted");
    }catch(e){ showToast("✗ "+e.message); }
    }});
    return;
  };

  const renameProject=async(id,newName)=>{
    if(!newName.trim()) return showToast("⚠ Name required");
    try{
      await firebase.db.collection("users").doc(user.uid).collection("projects").doc(id).update({examName:newName.trim()});
      setProjects(prev=>prev.map(q=>q.id===id?{...q,examName:newName.trim()}:q));
      setRenamingProject(null); setRenameValue(""); showToast("✓ Renamed");
    }catch(e){ showToast("✗ "+e.message); }
  };

  // ── Add Student ─────────────────────────────────────────────────────
  const addStudent=async()=>{
    if(!activeProject) return;
    if(!studentName.trim()||!roll.trim()) return showToast("⚠ Name & Roll required");
    const {gpa,grade,total,subResults,status}=calcStudent(subjectEntries,activeProject);
    const obj={
      name:studentName.trim(), roll:roll.trim(),
      gpa, grade, total, status,
      subjectResults: subResults,
      finalMarks: subResults.map(r=>r.ab),
    };
    setSaving(true);
    try{
      let confirmedUpdate = true;
      const nextStudents = await firebase.db.mutateStudents(user.uid, activeProject.id, (current) => {
        const list = [...current];
        const idx = list.findIndex(s=>s.roll===obj.roll);
        if(idx!==-1){
          if(!window.confirm("Student exists. Update?")){ confirmedUpdate = false; return current; }
          list[idx]=obj;
        } else { list.push(obj); }
        return list;
      });
      if (!confirmedUpdate) { setSaving(false); return; }
      const pr={...activeProject,students:nextStudents};
      const updated=[...projects]; updated[activeProjectIndex]=pr;
      setProjects(updated); setViewStudent(obj);
      setStudentName(""); setRoll(""); setSubjectEntries(freshEntries(activeProject));
      showToast("✓ Saved");
    }catch(e){ showToast("✗ "+e.message); }
    setSaving(false);
  };

  const deleteStudent=async(rollNo)=>{
    if(!activeProject) return;
    setConfirmModal({title:"Remove Student",msg:"Remove this student permanently?",ok:"Remove",onOk:async()=>{ setConfirmModal(null);
    try{
      const nextStudents = await firebase.db.mutateStudents(user.uid, activeProject.id, (current) => current.filter(s=>s.roll!==rollNo));
      const pr={...activeProject,students:nextStudents};
      const updated=[...projects]; updated[activeProjectIndex]=pr;
      setProjects(updated);
      if(viewStudent?.roll===rollNo) setViewStudent(null);
      showToast("✓ Student removed");
    }catch(e){ showToast("✗ "+e.message); }
    }});
    return;
  };

  const searchStudent=()=>{
    const s=activeProject?.students.find(x=>x.roll.toLowerCase()===searchRoll.trim().toLowerCase());
    if(!s) return showToast("✗ Not found");
    setViewStudent(s);
  };

  const signOut=async()=>{
    await firebase.auth.signOut();
    setProjects([]); setActiveProjectIndex(null); setIsAccountBlocked(false);
    go("home"); showToast("✓ Signed out");
  };

  const meritRanks=activeProject?getMeritRank(activeProject.students):{};
  const [meritSort, setMeritSort] = useState("roll"); // "roll" | "merit"
  const sortedByRoll=activeProject?[...activeProject.students].sort((a,b)=>String(a.roll).localeCompare(String(b.roll),undefined,{numeric:true})):[];
  const sortedByMerit=activeProject?[...activeProject.students].sort((a,b)=>b.gpa-a.gpa||b.total-a.total):[];
  const meritTableList = meritSort==="merit" ? sortedByMerit : sortedByRoll;
  const top3=activeProject?[...activeProject.students].sort((a,b)=>b.gpa-a.gpa||b.total-a.total).slice(0,3):[];
  const medalColors=["#f59e0b","#94a3b8","#cd7c2f"];
  const medals=["🥇","🥈","🥉"];

  const summaryStats=()=>{
    if(!activeProject?.students.length) return{pass:0,fail:0,rate:"0.0",avg:"0.00",total:0};
    const pass=activeProject.students.filter(s=>s.status==="PASS").length;
    const fail=activeProject.students.length-pass;
    const avg=(activeProject.students.reduce((a,s)=>a+s.gpa,0)/activeProject.students.length).toFixed(2);
    return{pass,fail,rate:((pass/activeProject.students.length)*100).toFixed(1),avg,total:activeProject.students.length};
  };

  const getSubjectStats=()=>{
    if(!activeProject?.students.length) return[];
    const names=activeProject.subjectNames||Array.from({length:activeProject.subjectCount},(_,i)=>`Subject ${i+1}`);
    return names.map((name,i)=>{
      const withResults=activeProject.students.filter(s=>(s.subjectResults||[])[i]);
      const results=withResults.map(s=>s.subjectResults[i]);
      const avgPct=results.length?results.reduce((s,r)=>s+(r.pct||0),0)/results.length:0;
      const passedStudents=withResults.filter(s=>s.subjectResults[i].pass);
      const failedStudents=withResults.filter(s=>!s.subjectResults[i].pass);
      const highest=results.length?Math.max(...results.map(r=>r.ab)):0;
      const lowest=results.length?Math.min(...results.map(r=>r.ab)):0;
      return{name,avg:avgPct.toFixed(1),passRate:((passedStudents.length/(results.length||1))*100).toFixed(0),highest:highest.toFixed(1),lowest:lowest.toFixed(1),passedStudents,failedStudents};
    });
  };

  // ── Helpers for subject settings input ────────────────────────────
  const updateSubjectSetting=(i,field,val)=>{
    setSubjectSettings(prev=>{
      const u=[...prev];
      u[i]={...(u[i]||{caFull:30,examFull:100,examPct:70}),[field]:val};
      return u;
    });
  };
  const getSS=(i)=>({caFull:30,examFull:100,examPct:70,...(subjectSettings[i]||{})});

  // ── Subject reorder: setup form (pre-creation, plain local-state swap) ──
  const moveSetupSubject = (idx, dir) => {
    const target = idx + dir;
    if (target < 0 || target >= Number(subjectCount)) return;
    const namesArr = [...subjectNames]; const setsArr = [...subjectSettings];
    const [nMoved] = namesArr.splice(idx, 1); namesArr.splice(target, 0, nMoved);
    const [sMoved] = setsArr.splice(idx, 1); setsArr.splice(target, 0, sMoved);
    setSubjectNames(namesArr); setSubjectSettings(setsArr);
  };
  const handleSetupPointerDown = (idx) => (e) => {
    setSetupDragIdx(idx); setupDragStartY.current = e.clientY;
    e.currentTarget.setPointerCapture?.(e.pointerId);
  };
  const handleSetupPointerMove = (e) => {
    if (setupDragIdx === null) return;
    const deltaY = e.clientY - setupDragStartY.current;
    const itemHeight = 150;
    if (Math.abs(deltaY) >= itemHeight) {
      const shift = deltaY > 0 ? 1 : -1;
      const target = setupDragIdx + shift;
      if (target >= 0 && target < Number(subjectCount)) {
        moveSetupSubject(setupDragIdx, shift);
        setSetupDragIdx(target); setupDragStartY.current = e.clientY;
      }
    }
  };
  const handleSetupPointerUp = () => setSetupDragIdx(null);

  // ── Subject reorder: existing project (modal + Firestore transaction) ──
  const openReorderModal = () => {
    const full = Array.from({ length: activeProject.subjectCount || 0 }, (_, i) => ({
      origIndex: i, name: (activeProject.subjectNames?.[i] || "").trim() || `Subject ${i + 1}`,
    }));
    setReorderList(full); setReorderModalOpen(true);
  };
  const moveReorderItem = (idx, dir) => {
    setReorderList(prev => {
      const target = idx + dir;
      if (target < 0 || target >= prev.length) return prev;
      const arr = [...prev];
      const [moved] = arr.splice(idx, 1); arr.splice(target, 0, moved);
      return arr;
    });
  };
  const handleReorderPointerDown = (idx) => (e) => {
    setDragIdx(idx); dragStartY.current = e.clientY;
    e.currentTarget.setPointerCapture?.(e.pointerId);
  };
  const handleReorderPointerMove = (e) => {
    if (dragIdx === null) return;
    const deltaY = e.clientY - dragStartY.current;
    const itemHeight = 56;
    if (Math.abs(deltaY) >= itemHeight) {
      const shift = deltaY > 0 ? 1 : -1;
      const target = dragIdx + shift;
      if (target >= 0 && target < reorderList.length) {
        moveReorderItem(dragIdx, shift);
        setDragIdx(target); dragStartY.current = e.clientY;
      }
    }
  };
  const handleReorderPointerUp = () => setDragIdx(null);
  const saveReorderOrder = async () => {
    const newOrder = reorderList.map(it => it.origIndex);
    const newNames = reorderList.map(it => it.name);
    setSaving(true);
    try {
      const result = await firebase.db.reorderSubjects(user.uid, activeProject.id, newOrder, newNames);
      const pr = { ...activeProject, ...result };
      const updated = [...projects]; updated[activeProjectIndex] = pr;
      setProjects(updated);
      setSubjectEntries(freshEntries(pr));
      setViewStudent(null);
      setReorderModalOpen(false);
      showToast("✓ Subject order updated");
    } catch (e) { showToast("✗ " + e.message); }
    setSaving(false);
  };

  // ── Render ─────────────────────────────────────────────────────────
  return (
    <>
      <GlobalStyle dark={dark}/>
      {toast&&<Toast msg={toast}/>}
      {splash&&<LogoSplash onDone={()=>setSplash(false)}/>}
      {confirmModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
          <div style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:20,padding:24,width:"100%",maxWidth:340,animation:"up 0.2s ease"}}>
            <div style={{fontWeight:700,fontSize:16,marginBottom:8}}>{confirmModal.title}</div>
            <div style={{color:"var(--text2)",fontSize:13,marginBottom:20,lineHeight:1.5}}>{confirmModal.msg}</div>
            <div style={{display:"flex",gap:10}}>
              <Btn variant="ghost" onClick={()=>setConfirmModal(null)} style={{flex:1}}>Cancel</Btn>
              <Btn variant="danger" onClick={()=>{setConfirmModal(null);confirmModal.onOk();}} style={{flex:1}}>{confirmModal.ok}</Btn>
            </div>
          </div>
        </div>
      )}
      {isAccountBlocked&&<BlockedModal onSignOut={signOut}/>}
      {showAdmin&&firebase&&<AdminPanel firebase={firebase} showToast={showToast} onClose={()=>setShowAdmin(false)}/>}
      {studentModal&&<StudentListModal title={studentModal.title} students={studentModal.students} color={studentModal.color} onClose={()=>setStudentModal(null)}/>}

      <div style={{maxWidth:520,margin:"0 auto",padding:"0 16px 120px",minHeight:"100vh"}}>

        {/* ── Not logged in ── */}
        {fbReady&&authReady&&!user&&(
          <AuthPage firebase={firebase} onAuth={()=>{}} showToast={showToast}/>
        )}

        {fbReady&&authReady&&user&&!fbError&&!isAccountBlocked&&(
          <>


            {/* ── HOME ── */}
            {page==="home"&&(
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                {/* Greeting */}
                <div style={{marginBottom:24}}>
                  <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"var(--sky)",marginBottom:6,display:"flex",alignItems:"center",gap:6}}>
                    <span style={{display:"inline-block",width:18,height:2,background:"linear-gradient(90deg,var(--sky),transparent)",borderRadius:2}}/>
                    Dashboard
                  </div>
                  <h1 style={{fontSize:26,fontWeight:800,letterSpacing:"-0.02em",marginBottom:3}}>
                    Hi, {userData?.name||user.displayName||user.email.split("@")[0]} 👋
                  </h1>
                  <p style={{color:"var(--text2)",fontSize:13}}>{user.email}</p>
                </div>

                {/* Projects */}
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  <div style={{fontWeight:700,fontSize:14,letterSpacing:"-0.01em"}}>My Projects</div>
                  <Btn size="sm" onClick={()=>go("setup")}>+ New</Btn>
                </div>

                {projectsLoading?([1,2].map(k=>(
                  <div key={k} style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:16,marginBottom:9,padding:"14px 16px",display:"flex",gap:14,alignItems:"center"}}>
                    <div style={{width:46,height:46,borderRadius:14,background:"var(--border)",flexShrink:0,animation:"pulse 1.5s ease-in-out infinite"}}/>
                    <div style={{flex:1}}>
                      <div style={{height:13,borderRadius:6,background:"var(--border)",marginBottom:9,width:"55%",animation:"pulse 1.5s ease-in-out infinite"}}/>
                      <div style={{height:10,borderRadius:6,background:"var(--border)",width:"38%",animation:"pulse 1.5s ease-in-out 0.2s infinite"}}/>
                    </div>
                  </div>
                ))):projects.length===0?(
                  <div style={{textAlign:"center",padding:"48px 24px",background:"var(--surface)",border:"1.5px dashed var(--border)",borderRadius:20}}>
                    <div style={{display:"flex",justifyContent:"center",marginBottom:16,opacity:0.5}}><IcoDoc s={56} c="var(--sky)"/></div>
                    <div style={{fontWeight:700,fontSize:16,marginBottom:8}}>No projects yet</div>
                    <div style={{fontSize:13,color:"var(--text2)",marginBottom:20,lineHeight:1.6}}>Create your first grading project<br/>to start entering student marks</div>
                    <Btn onClick={()=>go("setup")}>＋ New Project</Btn>
                  </div>
                ):projects.map((p,i)=>{
                  const pass=p.students.filter(s=>s.status==="PASS").length;
                  const passRate=p.students.length?((pass/p.students.length)*100).toFixed(0):0;
                  return(
                    <div key={p.id} className="project-card" style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderLeft:`3px solid ${["#38bdf8","#a78bfa","#34d399","#fb923c","#f472b6","#facc15"][i%6]}`,borderRadius:16,marginBottom:9,overflow:"hidden"}}>
                      <div style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px"}}>
                        <div style={{width:46,height:46,borderRadius:14,flexShrink:0,background:`color-mix(in srgb,${["#38bdf8","#a78bfa","#34d399","#fb923c","#f472b6","#facc15"][i%6]} 12%,var(--surface2))`,border:`1.5px solid color-mix(in srgb,${["#38bdf8","#a78bfa","#34d399","#fb923c","#f472b6","#facc15"][i%6]} 20%,var(--border))`,display:"flex",alignItems:"center",justifyContent:"center"}}>
                          <IcoDoc s={22} c={["#38bdf8","#a78bfa","#34d399","#fb923c","#f472b6","#facc15"][i%6]}/>
                        </div>
                        <div onClick={()=>{setActiveProjectIndex(i);setSubjectEntries(freshEntries(p));setViewStudent(null);go("app");}} style={{flex:1,cursor:"pointer"}}>
                          <div style={{fontWeight:700,fontSize:14.5,marginBottom:2,letterSpacing:"-0.01em"}}>{p.examName}</div>
                          <div style={{color:"var(--text2)",fontSize:12}}>
                            {p.institution&&<span>{p.institution}{p.className?" · "+p.className:""} · </span>}
                            {p.students.length} students · {p.subjectCount} subjects
                            {p.students.length>0&&<span style={{marginLeft:8,color:"var(--green)",fontWeight:600}}>{passRate}% pass</span>}
                          </div>
                          {p.students.length>0&&(
                            <div style={{height:3,borderRadius:2,background:"var(--border)",marginTop:7,overflow:"hidden"}}>
                              <div style={{height:3,borderRadius:2,background:"var(--green)",width:`${passRate}%`,transition:"width 0.8s ease"}}/>
                            </div>
                          )}
                        </div>
                      </div>
                      <div style={{display:"flex",borderTop:"1px solid var(--border)"}}>
                        {[
                          {icon:"✏️",label:"Rename",action:()=>{setRenamingProject(p.id);setRenameValue(p.examName);}},
                          {icon:"🗑",label:"Delete",action:()=>deleteProject(p.id),danger:true},
                        ].map(({icon,label,action,danger})=>(
                          <button key={label} onClick={e=>{e.stopPropagation();action();}} style={{flex:1,padding:"8px 2px",border:"none",background:"transparent",color:danger?"var(--red)":"var(--text2)",fontSize:10.5,fontWeight:600,cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,borderRight:label!=="Delete"?"1px solid var(--border)":undefined}}>
                            <span style={{fontSize:14}}>{icon}</span><span>{label}</span>
                          </button>
                        ))}
                      </div>
                    </div>
                  );
                })}

                {renamingProject&&(
                  <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:999,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 20px"}}>
                    <div style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:20,padding:24,width:"100%",maxWidth:380}}>
                      <div style={{fontWeight:700,fontSize:16,marginBottom:16}}>Rename Project</div>
                      <input autoFocus value={renameValue} onChange={e=>setRenameValue(e.target.value)} onKeyDown={e=>e.key==="Enter"&&renameProject(renamingProject,renameValue)} placeholder="Project name" style={{marginBottom:14}}/>
                      <div style={{display:"flex",gap:10}}>
                        <Btn variant="secondary" onClick={()=>setRenamingProject(null)} style={{flex:1}}>Cancel</Btn>
                        <Btn onClick={()=>renameProject(renamingProject,renameValue)} style={{flex:1}}>Save</Btn>
                      </div>
                    </div>
                  </div>
                )}


              </div>
            )}

            {/* ── SETUP ── */}
            {page==="setup"&&(
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                <SectionHeader label="New Project" title="New Exam" subtitle="Fill in your project details"/>

                <div style={{fontSize:10,fontWeight:700,color:"var(--sky)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8,marginTop:4}}>① Basic Info</div>
                <Card style={{marginBottom:14}}>
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    <div>
                      <Label>Exam / Project Name ✱</Label>
                      <input placeholder="e.g. Annual Exam 2025" value={examName} onChange={e=>setExamName(e.target.value)}/>
                    </div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                      <div>
                        <Label>Institution</Label>
                        <input placeholder="School name" value={institution} onChange={e=>setInstitution(e.target.value)}/>
                      </div>
                      <div>
                        <Label>Class / Grade</Label>
                        <input placeholder="e.g. Class 6" value={className} onChange={e=>setClassName(e.target.value)}/>
                      </div>
                    </div>
                    <div>
                      <Label>Number of Subjects ✱</Label>
                      <input type="number" placeholder="e.g. 11" value={subjectCount} min={1} max={20}
                        onChange={e=>{
                          setSubjectCount(e.target.value);
                          const n=Number(e.target.value);
                          setSubjectNames(Array(n).fill(""));
                          setSubjectSettings(Array(n).fill(null).map(()=>({caFull:30,examFull:100,examPct:70})));
                        }}/>
                    </div>
                  </div>
                </Card>

                {subjectCount&&Number(subjectCount)>=1&&(
                  <div>
                    <div style={{fontSize:10,fontWeight:700,color:"var(--sky)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>② Subjects <span style={{opacity:0.7,textTransform:"none",letterSpacing:0}}>· drag ⠿ or use arrows to reorder</span></div>
                    <Card style={{marginBottom:14}}>
                      <div style={{display:"flex",flexDirection:"column",gap:10}} onPointerMove={handleSetupPointerMove} onPointerUp={handleSetupPointerUp} onPointerLeave={handleSetupPointerUp}>
                        {Array(Number(subjectCount)).fill(null).map((_,i)=>{
                          const ss=getSS(i);
                          const subFull=Number(ss.caFull)+((Number(ss.examPct)||70)/100)*Number(ss.examFull||100);
                          return(
                            <div key={i} style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:14,padding:"14px 16px",position:"relative",overflow:"hidden",opacity:setupDragIdx===i?0.6:1}}>
                              <div style={{position:"absolute",top:0,left:0,width:3,height:"100%",background:"linear-gradient(180deg,var(--sky),var(--indigo))",opacity:0.6}}/>
                              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
                                <div onPointerDown={handleSetupPointerDown(i)} style={{cursor:"grab",touchAction:"none",fontSize:16,color:"var(--text3)",padding:"2px 4px",flexShrink:0,userSelect:"none"}}>⠿</div>
                                <div style={{width:22,height:22,borderRadius:7,background:"color-mix(in srgb,var(--sky) 16%,var(--surface))",color:"var(--sky)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,flexShrink:0}}>{i+1}</div>
                                <input placeholder={`Subject ${i+1} name`} value={subjectNames[i]||""} onChange={e=>{const u=[...subjectNames];u[i]=e.target.value;setSubjectNames(u);}} style={{margin:0}}/>
                                <div style={{display:"flex",flexDirection:"column",gap:2,flexShrink:0}}>
                                  <button onClick={()=>moveSetupSubject(i,-1)} disabled={i===0} style={{background:"var(--surface3)",border:"1px solid var(--border)",borderRadius:5,color:"var(--text2)",width:22,height:16,fontSize:9,cursor:i===0?"default":"pointer",opacity:i===0?0.35:1,lineHeight:"1"}}>▲</button>
                                  <button onClick={()=>moveSetupSubject(i,1)} disabled={i===Number(subjectCount)-1} style={{background:"var(--surface3)",border:"1px solid var(--border)",borderRadius:5,color:"var(--text2)",width:22,height:16,fontSize:9,cursor:i===Number(subjectCount)-1?"default":"pointer",opacity:i===Number(subjectCount)-1?0.35:1,lineHeight:"1"}}>▼</button>
                                </div>
                              </div>
                              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
                                <div>
                                  <div style={{fontSize:9.5,fontWeight:700,color:"#818cf8",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:4}}>CA Full</div>
                                  <input type="number" value={ss.caFull??30} min={0} onChange={e=>updateSubjectSetting(i,"caFull",e.target.value)} style={{textAlign:"center"}}/>
                                </div>
                                <div>
                                  <div style={{fontSize:9.5,fontWeight:700,color:"var(--sky)",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:4}}>Exam Full</div>
                                  <input type="number" value={ss.examFull??100} min={1} onChange={e=>updateSubjectSetting(i,"examFull",e.target.value)} style={{textAlign:"center"}}/>
                                </div>
                                <div>
                                  <div style={{fontSize:9.5,fontWeight:700,color:"var(--amber)",letterSpacing:"0.06em",textTransform:"uppercase",marginBottom:4}}>Exam %</div>
                                  <input type="number" value={ss.examPct??70} min={0} max={100} onChange={e=>updateSubjectSetting(i,"examPct",e.target.value)} style={{textAlign:"center"}}/>
                                </div>
                              </div>
                              <div style={{marginTop:10,fontSize:11,color:"var(--text2)",display:"flex",gap:12,flexWrap:"wrap",paddingTop:8,borderTop:"1px solid var(--border)"}}>
                                <span>CA: <b style={{color:"#818cf8"}}>{ss.caFull??30}</b></span>
                                <span>B: <b style={{color:"var(--amber)"}}>{((Number(ss.examPct)||70)/100*(Number(ss.examFull)||100)).toFixed(0)}</b></span>
                                <span>Max: <b style={{color:"var(--sky)"}}>{Math.round(Number(ss.caFull??30)+((Number(ss.examPct)||70)/100)*Number(ss.examFull||100))}</b></span>
                              </div>
                            </div>
                          );
                        })}
                      </div>
                    </Card>
                  </div>
                )}

                <div style={{fontSize:10,fontWeight:700,color:"var(--sky)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>③ CA Settings</div>
                <Card style={{marginBottom:14}}>
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <span style={{fontSize:12,color:"var(--text2)",flex:1}}>Number of Assignments</span>
                      <input type="number" value={numAssignments} min={1} max={10} onChange={e=>setNumAssignments(Math.max(1,Number(e.target.value)))} style={{width:70,textAlign:"center"}}/>
                    </div>
                    <div style={{display:"flex",alignItems:"center",gap:10}}>
                      <span style={{fontSize:12,color:"var(--text2)",flex:1}}>Full Marks per Assignment</span>
                      <input type="number" value={assignOutOf} min={1} onChange={e=>setAssignOutOf(Number(e.target.value))} style={{width:70,textAlign:"center"}}/>
                    </div>
                    <div style={{fontSize:11,color:"var(--text3)"}}>
                      CA = (Σ assignments / {numAssignments}×{assignOutOf}) × CA Full
                    </div>
                  </div>
                </Card>

                <div style={{fontSize:10,fontWeight:700,color:"var(--sky)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:8}}>④ Pass & Grading</div>
                <Card style={{marginBottom:14}}>
                  <div style={{display:"flex",flexDirection:"column",gap:12}}>
                    <div style={{display:"flex",alignItems:"center",gap:12}}>
                      <div style={{flex:1}}>
                        <div style={{fontWeight:700,fontSize:12.5,marginBottom:2}}>Pass Threshold %</div>
                        <div style={{fontSize:11,color:"var(--text2)"}}>Minimum % to pass each subject</div>
                      </div>
                      <input type="number" value={passThreshold} onChange={e=>setPassThreshold(Number(e.target.value))} min={0} max={100} style={{width:70,textAlign:"center",fontWeight:700}}/>
                      <span style={{fontSize:12,color:"var(--text3)"}}>%</span>
                    </div>
                    <div>
                      <label style={{display:"flex",alignItems:"center",gap:9,fontSize:12.5,fontWeight:600,cursor:"pointer",marginBottom:8}}>
                        <input type="checkbox" checked={showGradingEditor} onChange={e=>{setShowGradingEditor(e.target.checked);if(!e.target.checked)setCustomGrading(null);}} style={{width:"auto",accentColor:"var(--sky)",cursor:"pointer"}}/>
                        Custom Grading Scale
                      </label>
                      {showGradingEditor&&(
                        <div style={{display:"flex",flexDirection:"column",gap:7}}>
                          <div style={{fontSize:11,color:"var(--text2)",marginBottom:2}}>Minimum % to achieve each grade:</div>
                          {(customGrading||DEFAULT_GRADING).map((row,i)=>(
                            <div key={i} style={{display:"flex",alignItems:"center",gap:8}}>
                              <div style={{width:36,fontWeight:700,fontSize:12,color:gradeColor(row.grade),fontFamily:"var(--font-mono)"}}>{row.grade}</div>
                              <input type="number" value={row.min} min={0} max={100}
                                onChange={e=>{const u=(customGrading||DEFAULT_GRADING).map((r,j)=>j===i?{...r,min:Number(e.target.value)}:r);setCustomGrading(u);}}
                                style={{width:70,textAlign:"center"}}/>
                              <span style={{fontSize:11,color:"var(--text3)"}}>%</span>
                              <span style={{fontSize:11,color:"var(--text2)",flex:1}}>→ GP {row.gp}</span>
                            </div>
                          ))}
                        </div>
                      )}
                    </div>
                  </div>
                </Card>

                <Btn onClick={createProject} loading={saving} style={{width:"100%",marginBottom:80}} size="lg">Create Project →</Btn>
              </div>
            )}

            {page==="app"&&(
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                {!activeProject?(
                  <div style={{textAlign:"center",padding:"60px 0"}}>
                    <div style={{fontSize:40,marginBottom:12}}>📋</div>
                    <div style={{color:"var(--text2)",marginBottom:16}}>Select a project from Home</div>
                    <Btn onClick={()=>go("home")}>← Go Home</Btn>
                  </div>
                ):(
                  <>
                    <SectionHeader
                      label="Mark Entry"
                      title={activeProject.examName}
                      subtitle={[activeProject.institution,activeProject.className].filter(Boolean).join(" · ")||"Entry"}
                    />
                    <div style={{display:"flex",justifyContent:"flex-end",marginBottom:14,marginTop:-8}}>
                      <button onClick={openReorderModal} style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:10,padding:"6px 10px",fontSize:12,fontWeight:700,color:"var(--sky)",cursor:"pointer"}}>🔀 Reorder Subjects</button>
                    </div>

                    {reorderModalOpen && (
                      <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:9999}} onClick={()=>setReorderModalOpen(false)}>
                        <div onClick={e=>e.stopPropagation()} style={{background:"var(--surface)",borderRadius:"20px 20px 0 0",padding:20,width:"100%",maxWidth:480,maxHeight:"80vh",overflowY:"auto",border:"1.5px solid var(--border)",borderBottom:"none"}}>
                          <div style={{fontWeight:800,fontSize:15,marginBottom:4}}>Reorder Subjects</div>
                          <div style={{fontSize:11.5,color:"var(--text2)",marginBottom:14}}>Drag ⠿ or use the arrows. This updates the order everywhere — entry page, marksheet, and every student already saved.</div>
                          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}} onPointerMove={handleReorderPointerMove} onPointerUp={handleReorderPointerUp} onPointerLeave={handleReorderPointerUp}>
                            {reorderList.map((item,i)=>(
                              <div key={item.origIndex} style={{display:"flex",alignItems:"center",gap:10,background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:12,padding:"10px 12px",opacity:dragIdx===i?0.6:1}}>
                                <div onPointerDown={handleReorderPointerDown(i)} style={{cursor:"grab",touchAction:"none",fontSize:16,color:"var(--text3)",userSelect:"none"}}>⠿</div>
                                <div style={{width:20,height:20,borderRadius:6,background:"color-mix(in srgb,var(--sky) 16%,var(--surface))",color:"var(--sky)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,flexShrink:0}}>{i+1}</div>
                                <div style={{flex:1,fontSize:13,fontWeight:600}}>{item.name}</div>
                                <button onClick={()=>moveReorderItem(i,-1)} disabled={i===0} style={{background:"var(--surface3)",border:"1px solid var(--border)",borderRadius:6,color:"var(--text2)",width:26,height:22,fontSize:11,cursor:i===0?"default":"pointer",opacity:i===0?0.35:1}}>▲</button>
                                <button onClick={()=>moveReorderItem(i,1)} disabled={i===reorderList.length-1} style={{background:"var(--surface3)",border:"1px solid var(--border)",borderRadius:6,color:"var(--text2)",width:26,height:22,fontSize:11,cursor:i===reorderList.length-1?"default":"pointer",opacity:i===reorderList.length-1?0.35:1}}>▼</button>
                              </div>
                            ))}
                          </div>
                          <div style={{display:"flex",gap:10}}>
                            <Btn variant="secondary" onClick={()=>setReorderModalOpen(false)} style={{flex:1}}>Cancel</Btn>
                            <Btn onClick={saveReorderOrder} loading={saving} style={{flex:1}}>Save Order</Btn>
                          </div>
                        </div>
                      </div>
                    )}

                    {/* Add Student Card */}
                    <Card style={{marginBottom:14,borderRadius:18}}>
                      <div style={{fontWeight:700,fontSize:14,marginBottom:14,letterSpacing:"-0.01em"}}>+ Add / Update Student</div>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                        <div><Label>Student Name</Label>
                          <input placeholder="Full name" value={studentName} onChange={e=>setStudentName(e.target.value)}/>
                        </div>
                        <div><Label>Roll No.</Label>
                          <input placeholder="Roll" value={roll} onChange={e=>setRoll(e.target.value)}/>
                        </div>
                      </div>

                      {/* Per-subject entry */}
                      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
                        {Array(activeProject.subjectCount).fill(null).map((_,si)=>{
                          const subName=activeProject.subjectNames?.[si]||`Subject ${si+1}`;
                          const ss=activeProject.subjectSettings?.[si]||{caFull:30,examFull:100,examPct:70};
                          const entry=subjectEntries[si]||{assignments:[],mcq:"",cq:""};
                          const na=activeProject.numAssignments||1;
                          const aoof=activeProject.assignOutOf||20;
                          // Live calc
                          const r=calcSubject(entry,ss,activeProject);
                          const gpFn=makeGetGP(activeProject.gradingScale||null);
                          const gradeFn=makeGetGrade(activeProject.gradingScale||null);
                          const pctLive=r.pct;
                          const grLive=gradeFn(gpFn(pctLive));
                          const examFullLive   = Number(ss.examFull??100);
                          const examOnlyPctLive = examFullLive>0 ? (r.rawExam/examFullLive)*100 : 0;
                          const passLive=pctLive>=(activeProject.passThreshold??33) && examOnlyPctLive>=(activeProject.passThreshold??33);
                          const subFull=Number(ss.caFull)+((Number(ss.examPct)||70)/100)*Number(ss.examFull||100);

                          return(
                            <div key={si} style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:14,overflow:"hidden"}}>
                              {/* Subject header */}
                              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 14px",borderBottom:"1px solid var(--border)",background:"var(--surface3)"}}>
                                <div style={{fontWeight:700,fontSize:13}}>{subName}</div>
                                <div style={{display:"flex",alignItems:"center",gap:8}}>
                                  <span style={{fontSize:11,color:"var(--text2)"}}>/{Math.round(subFull)}</span>
                                  <span style={{fontWeight:800,fontSize:14,color:gradeColor(grLive),fontFamily:"var(--font-mono)"}}>{grLive}</span>
                                  <span style={{fontWeight:700,fontSize:13,color:"var(--sky)",fontFamily:"var(--font-mono)"}}>{r.ab}</span>
                                  <Pill color={passLive?"var(--green)":"var(--red)"}>{passLive?"P":"F"}</Pill>
                                </div>
                              </div>

                              <div style={{padding:"10px 12px",display:"flex",flexDirection:"column",gap:7}}>
                                {Array(na).fill(null).map((_,ai)=>(
                                  <div key={ai} style={{display:"flex",alignItems:"center",gap:10}}>
                                    <span style={{fontSize:11,color:"#818cf8",fontWeight:600,minWidth:80}}>Assignment {ai+1}</span>
                                    <input type="number" placeholder="0" min={0} max={aoof} value={(entry.assignments||[])[ai]??""} onChange={e=>{setSubjectEntries(prev=>prev.map((en,s)=>{if(s!==si)return en;const a=[...(en.assignments||[])];a[ai]=e.target.value;return{...en,assignments:a};}));}} style={{flex:1,textAlign:"center"}}/>
                                    <span style={{fontSize:11,color:"var(--text3)",minWidth:28}}>/{aoof}</span>
                                  </div>
                                ))}
                                <div style={{display:"flex",alignItems:"center",gap:10}}>
                                  <span style={{fontSize:11.5,color:"var(--sky)",fontWeight:600,minWidth:90}}>MCQ / Oral</span>
                                  <input type="number" placeholder="0" min={0} value={entry.mcq??""} onChange={e=>setSubjectEntries(prev=>prev.map((en,s)=>s===si?{...en,mcq:e.target.value}:en))} style={{flex:1,textAlign:"center"}}/>
                                  <span style={{fontSize:11,color:"var(--text3)",minWidth:28}}>/{ss.examFull||100}</span>
                                </div>
                                <div style={{display:"flex",alignItems:"center",gap:10}}>
                                  <span style={{fontSize:11.5,color:"var(--amber)",fontWeight:600,minWidth:90}}>CQ / Written</span>
                                  <input type="number" placeholder="0" min={0} value={entry.cq??""} onChange={e=>setSubjectEntries(prev=>prev.map((en,s)=>s===si?{...en,cq:e.target.value}:en))} style={{flex:1,textAlign:"center"}}/>
                                  <span style={{fontSize:11,color:"var(--text3)",minWidth:28}}>/{ss.examFull||100}</span>
                                </div>
                                <div style={{fontSize:11,color:"var(--text2)",display:"flex",gap:10,flexWrap:"wrap",paddingTop:6,marginTop:2,borderTop:"1px solid var(--border)"}}>
                                  <span>CA: <b style={{color:"#818cf8",fontFamily:"var(--font-mono)"}}>{r.ca}</b></span>
                                  <span>Exam: <b style={{color:"var(--sky)",fontFamily:"var(--font-mono)"}}>{r.rawExam}</b></span>
                                  <span>B: <b style={{color:"var(--amber)",fontFamily:"var(--font-mono)"}}>{r.b}</b></span>
                                  <span style={{fontWeight:700}}>Total: <b style={{color:"var(--green)",fontFamily:"var(--font-mono)"}}>{r.ab}/{Math.round(r.subFull)}</b></span>
                                  <span style={{marginLeft:"auto",color:passLive?"var(--green)":"var(--red)",fontWeight:700}}>{pctLive.toFixed(1)}%</span>
                                </div>
                              </div>
                            </div>
                          );
                        })}
                      </div>

                      <Btn onClick={addStudent} loading={saving} style={{width:"100%"}}>+ Add Student</Btn>
                    </Card>

                    {/* Search */}
                    <Card style={{marginBottom:12,borderRadius:18}}>
                      <div style={{fontWeight:700,fontSize:14,marginBottom:11,letterSpacing:"-0.01em"}}>Search Student</div>
                      <div style={{display:"flex",gap:9}}>
                        <input placeholder="Enter Roll No." value={searchRoll} onChange={e=>setSearchRoll(e.target.value)} onKeyDown={e=>e.key==="Enter"&&searchStudent()}/>
                        <Btn variant="secondary" onClick={searchStudent} style={{whiteSpace:"nowrap"}}>Search</Btn>
                      </div>
                    </Card>

                    {/* Student Card */}
                    {viewStudent&&(()=>{
                      const subCount = activeProject.subjectCount || (viewStudent.subjectResults||[]).length;
                      const maxMarks = Array.from({length:subCount}).reduce((acc,_,i)=>{
                        const ss = activeProject.subjectSettings?.[i] || {caFull:30,examFull:100,examPct:70};
                        return acc + Math.round((ss.caFull||30) + ((ss.examPct||70)/100) * (ss.examFull||100));
                      }, 0);
                      const pctDisplay = maxMarks>0 ? ((viewStudent.total/maxMarks)*100).toFixed(1)+"%" : "—";
                      const totalGP = (viewStudent.subjectResults||[]).reduce((s,r)=>s+(r.gp||0),0).toFixed(2);
                      const passCount = (viewStudent.subjectResults||[]).filter(r=>r.pass).length;
                      return (
                      <Card style={{border:`1.5px solid color-mix(in srgb,${gradeColor(viewStudent.grade)} 28%,var(--border))`,background:`color-mix(in srgb,${gradeColor(viewStudent.grade)} 4%,var(--surface))`,animation:"up 0.25s ease",borderRadius:18,marginBottom:12}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                          <div>
                            <div style={{fontWeight:800,fontSize:18,letterSpacing:"-0.02em"}}>{viewStudent.name}</div>
                            <div style={{color:"var(--text2)",fontSize:12,marginTop:2}}>Roll: {viewStudent.roll}</div>
                          </div>
                          <div style={{textAlign:"right"}}>
                            <div style={{fontSize:30,fontWeight:800,color:gradeColor(viewStudent.grade),fontFamily:"var(--font-mono)",lineHeight:1}}>{viewStudent.grade}</div>
                            <div style={{marginTop:4}}><Pill color={viewStudent.status==="PASS"?"var(--green)":"var(--red)"}>{viewStudent.status}</Pill></div>
                          </div>
                        </div>

                        {/* KPI row */}
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:9,marginBottom:14}}>
                          {[
                            {label:"Total Mark", value:typeof viewStudent.total==="number"?Math.round(viewStudent.total):viewStudent.total,color:"var(--sky)",big:false},
                            {label:"Percentage",value:pctDisplay,                                                       color:"var(--sky2)",big:false},
                            {label:"Total GP",  value:totalGP,                                                          color:"var(--indigo)",big:false},
                            {label:"GPA",       value:viewStudent.gpa,                                    color:gradeColor(viewStudent.grade),big:true},
                            {label:"Merit",     value:`#${meritRanks[viewStudent.roll]||"—"}`,            color:"var(--yellow)",big:true},
                            {label:"Subjects Passed",value:`${passCount}/${subCount} Pass`,                color:"var(--green)",big:false},
                          ].map(({label,value,color,big})=>(
                            <div key={label} style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:14,padding:"13px 10px",textAlign:"center"}}>
                              <div style={{fontSize:10,color:"var(--text2)",fontWeight:600,letterSpacing:"0.05em",textTransform:"uppercase",marginBottom:5}}>{label}</div>
                              <div style={{fontSize:big?26:18,fontWeight:800,color,fontFamily:"var(--font-mono)",lineHeight:1}}>{value}</div>
                            </div>
                          ))}
                        </div>

                        {/* Per-subject breakdown */}
                        {(viewStudent.subjectResults||[]).length>0&&(
                          <div style={{marginBottom:14}}>
                            <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--text2)",marginBottom:8}}>Subject Breakdown</div>
                            <div style={{display:"flex",flexDirection:"column",gap:7}}>
                              {(viewStudent.subjectResults||[]).map((sr,i)=>{
                                const subName=activeProject.subjectNames?.[i]||`Subject ${i+1}`;
                                const ss=activeProject.subjectSettings?.[i]||{caFull:30,examFull:100,examPct:70};
                                const subFull=Number(ss.caFull)+((Number(ss.examPct)||70)/100)*Number(ss.examFull||100);
                                return(
                                  <div key={i} style={{background:"var(--surface2)",border:`1.5px solid ${sr.pass?"var(--border)":"color-mix(in srgb,var(--red) 20%,var(--border))"}`,borderRadius:11,padding:"10px 12px"}}>
                                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                                      <span style={{fontWeight:700,fontSize:12.5}}>{subName}</span>
                                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                                        <span style={{fontWeight:800,fontSize:13,color:gradeColor(sr.grade),fontFamily:"var(--font-mono)"}}>{sr.grade}</span>
                                        <span style={{fontWeight:700,fontFamily:"var(--font-mono)",fontSize:12,color:"var(--sky)"}}>{sr.ab!=null?Math.round(sr.ab):"-"}/{Math.round(subFull)}</span>
                                        <Pill color={sr.pass?"var(--green)":"var(--red)"}>{sr.pass?"P":"F"}</Pill>
                                      </div>
                                    </div>
                                    <div style={{display:"flex",gap:12,fontSize:11,color:"var(--text2)",flexWrap:"wrap"}}>
                                      <span>CA: <b style={{color:"var(--indigo)",fontFamily:"var(--font-mono)"}}>{sr.ca!=null?Math.round(sr.ca):"-"}</b></span>
                                      <span>MCQ+CQ: <b style={{color:"var(--sky)",fontFamily:"var(--font-mono)"}}>{sr.rawExam??"-"}</b></span>
                                      <span>B: <b style={{color:"var(--amber)",fontFamily:"var(--font-mono)"}}>{sr.b!=null?Math.round(sr.b):"-"}</b></span>
                                      <span style={{marginLeft:"auto",color:sr.pass?"var(--green)":"var(--red)",fontWeight:700}}>{sr.pct!=null?Number(sr.pct).toFixed(1):"-"}%</span>
                                    </div>
                                    <div style={{marginTop:6,height:3,borderRadius:2,background:"var(--border)",overflow:"hidden"}}>
                                      <div style={{height:3,borderRadius:2,width:`${Math.min(sr.pct||0,100)}%`,background:sr.pass?"var(--green)":"var(--red)",transition:"width 0.5s ease"}}/>
                                    </div>
                                  </div>
                                );
                              })}
                            </div>
                          </div>
                        )}

                        <div style={{display:"flex",gap:9}}>
                          <Btn variant="ghost" size="sm" style={{flex:1}} onClick={()=>{
                            setStudentName(viewStudent.name);
                            setRoll(viewStudent.roll);
                            const entries=(viewStudent.subjectResults||[]).map(sr=>({
                              assignments:[sr.ca??0],
                              mcq:sr.mcq??sr.rawExam??0,
                              cq:sr.cq??0,
                            }));
                            const count=activeProject.subjectCount||entries.length;
                            while(entries.length<count) entries.push({assignments:[0],mcq:0,cq:0});
                            setSubjectEntries(entries);
                            setViewStudent(null);
                            go("app");
                            showToast("✏ Edit mode — update marks and save");
                          }}>✏ Edit</Btn>
                          <Btn onClick={()=>downloadMarksheet(viewStudent,activeProject,showToast)} style={{flex:1}} size="sm">📄 Marksheet</Btn>
                          <Btn variant="danger" onClick={()=>deleteStudent(viewStudent.roll)} size="sm" style={{whiteSpace:"nowrap"}}>✕ Remove</Btn>
                        </div>
                      </Card>
                      );
                    })()}

                    {/* All Students Table */}
                    <Card style={{padding:0,borderRadius:18,overflow:"hidden"}}>
                      <div style={{padding:"14px 16px",borderBottom:"1.5px solid var(--border)",fontWeight:700,fontSize:14}}>
                        All Students ({activeProject.students.length})
                      </div>
                      {activeProject.students.length===0?(
                        <div style={{textAlign:"center",padding:"40px 24px"}}>
                          <div style={{display:"flex",justifyContent:"center",marginBottom:12,opacity:0.35}}><IcoUsers s={48} c="var(--text2)"/></div>
                          <div style={{fontWeight:600,fontSize:13,marginBottom:4}}>No students yet</div>
                          <div style={{fontSize:12,color:"var(--text2)"}}>Go to Entry tab to add students</div>
                        </div>
                      ):(
                        <table>
                          <thead><tr>
                            <th style={{paddingLeft:16}}>Name</th>
                            <th>Roll</th><th>GPA</th><th>Grade</th><th>Total</th><th>Status</th><th></th>
                          </tr></thead>
                          <tbody>
                            {[...activeProject.students].sort((a,b)=>String(a.roll).localeCompare(String(b.roll),undefined,{numeric:true})).map(s=>(
                              <tr key={s.roll} style={{cursor:"pointer"}} onClick={()=>setViewStudent(s)}>
                                <td style={{fontWeight:600,paddingLeft:16}}>{s.name}</td>
                                <td style={{fontFamily:"var(--font-mono)",color:"var(--text2)",fontSize:12}}>{s.roll}</td>
                                <td style={{fontWeight:700,color:gradeColor(s.grade),fontFamily:"var(--font-mono)"}}>{s.gpa}</td>
                                <td><Pill color={gradeColor(s.grade)}>{s.grade}</Pill></td>
                                <td style={{fontFamily:"var(--font-mono)",fontSize:12.5}}>{typeof s.total==="number"?Math.round(s.total):s.total}</td>
                                <td><Pill color={s.status==="PASS"?"var(--green)":"var(--red)"}>{s.status}</Pill></td>
                                <td><button onClick={e=>{e.stopPropagation();deleteStudent(s.roll);}} style={{border:"none",background:"none",color:"var(--red)",cursor:"pointer",fontSize:16}}>✕</button></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                      )}
                    </Card>
                  </>
                )}
              </div>
            )}

            {/* ── STATS ── */}
            {page==="summary"&&(
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                {!activeProject?(
                  <div style={{textAlign:"center",padding:"60px 0"}}><div style={{color:"var(--text2)",marginBottom:16}}>Select a project from Home</div><Btn onClick={()=>go("home")}>← Go Home</Btn></div>
                ):(()=>{
                  const {pass,fail,rate,avg,total}=summaryStats();
                  const subStats=getSubjectStats();
                  const gradeDistrib={};
                  activeProject.students.forEach(s=>{gradeDistrib[s.grade]=(gradeDistrib[s.grade]||0)+1;});
                  const maxG=Math.max(1,...Object.values(gradeDistrib));
                  return(
                    <>
                      <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12,overflowX:"auto"}}>
                        {projects.map((p,i)=>(
                          <button key={p.id} onClick={()=>setActiveProjectIndex(i)} style={{whiteSpace:"nowrap",padding:"5px 12px",borderRadius:20,border:`1.5px solid ${i===activeProjectIndex?"var(--sky)":"var(--border)"}`,background:i===activeProjectIndex?"color-mix(in srgb,var(--sky) 15%,var(--surface))":"var(--surface)",color:i===activeProjectIndex?"var(--sky)":"var(--text2)",fontSize:11,fontWeight:600,cursor:"pointer"}}>{p.examName||p.id}</button>
                        ))}
                      </div>
                      <SectionHeader label="Analytics" title="Stats" subtitle={`${activeProject.examName} · ${total} students`}/>
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
                        <StatTile label="Total Students" value={total} color="var(--text)"/>
                        <StatTile label="Pass Rate" value={`${rate}%`} color="var(--green)"/>
                        <StatTile label="Avg GPA" value={avg} color="var(--sky)"/>
                        <StatTile label="Failed" value={fail} color={fail===0?"var(--green)":"var(--red)"}/>
                      </div>

                      <Card style={{marginBottom:14,borderRadius:16}}>
                        <div style={{fontWeight:700,fontSize:13.5,marginBottom:14}}>Pass / Fail</div>
                        <div style={{display:"flex",gap:9,marginBottom:10}}>
                          <StatTile label="Passed" value={pass} color="var(--green)"
                            onClick={()=>setStudentModal({title:"Passed Students",students:activeProject.students.filter(s=>s.status==="PASS"),color:"var(--green)"})}
                            style={{flex:1}}/>
                          <StatTile label="Failed" value={fail} color={fail===0?"var(--green)":"var(--red)"}
                            onClick={()=>setStudentModal({title:"Failed Students",students:activeProject.students.filter(s=>s.status==="FAIL"),color:"var(--red)"})}
                            style={{flex:1}}/>
                        </div>
                        <div style={{height:8,borderRadius:4,background:"var(--border)",overflow:"hidden"}}>
                          <div style={{height:8,borderRadius:4,background:"var(--green)",width:`${rate}%`,transition:"width 0.8s ease"}}/>
                        </div>
                      </Card>

                      <Card style={{marginBottom:14,borderRadius:16}}>
                        <div style={{fontWeight:700,fontSize:13.5,marginBottom:14}}>Grade Distribution</div>
                        {["A+","A","A-","B","C","D","F"].map(g=>{
                          const count=gradeDistrib[g]||0;
                          const pct=total>0?((count/total)*100).toFixed(0):0;
                          const bar=maxG>0?(count/maxG)*100:0;
                          const c=gradeColor(g);
                          return(
                            <div key={g} style={{marginBottom:10}}>
                              <div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}>
                                <div style={{display:"flex",alignItems:"center",gap:8}}>
                                  <span style={{width:28,fontWeight:800,fontSize:12,color:c,fontFamily:"var(--font-mono)"}}>{g}</span>
                                  <span style={{fontSize:11,color:"var(--text2)"}}>{count} students</span>
                                </div>
                                <div style={{display:"flex",alignItems:"center",gap:8}}>
                                  <span style={{fontSize:11,color:"var(--text2)"}}>{pct}%</span>
                                  {count>0&&<Btn variant="ghost" size="sm"
                                    onClick={()=>setStudentModal({title:`Grade ${g}`,students:activeProject.students.filter(s=>s.grade===g),color:c})}>
                                    View
                                  </Btn>}
                                </div>
                              </div>
                              <div style={{height:5,borderRadius:3,background:"var(--border)",overflow:"hidden"}}>
                                <div style={{height:5,borderRadius:3,width:`${bar}%`,background:c,transition:"width 0.7s ease"}}/>
                              </div>
                            </div>
                          );
                        })}
                      </Card>

                      {subStats.length>0&&(
                        <Card style={{marginBottom:14,borderRadius:16}}>
                          <div style={{fontWeight:700,fontSize:13.5,marginBottom:14}}>Subject Performance</div>
                          {subStats.map((sub,i)=>(
                            <div key={i} style={{padding:"10px 12px",borderRadius:11,marginBottom:8,background:"var(--surface2)",border:"1.5px solid var(--border)"}}>
                              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                                <div style={{fontWeight:600,fontSize:12.5}}>{sub.name}</div>
                                <div style={{display:"flex",gap:8,fontSize:11,color:"var(--text2)"}}>
                                  <span>Avg: <b style={{color:"var(--sky)",fontFamily:"var(--font-mono)"}}>{sub.avg}%</b></span>
                                  <span>Pass: <b style={{color:"var(--green)",fontFamily:"var(--font-mono)"}}>{sub.passRate}%</b></span>
                                </div>
                              </div>
                              <div style={{height:4,borderRadius:3,background:"var(--border)",overflow:"hidden"}}>
                                <div style={{height:4,borderRadius:3,width:`${sub.avg}%`,background:Number(sub.avg)>=60?"var(--green)":Number(sub.avg)>=40?"var(--yellow)":"var(--red)",transition:"width 0.7s ease"}}/>
                              </div>
                              <div style={{display:"flex",justifyContent:"space-between",fontSize:10.5,color:"var(--text3)",marginTop:4,marginBottom:10}}>
                                <span>Lowest: {sub.lowest}</span><span>Highest: {sub.highest}</span>
                              </div>
                              <div style={{display:"flex",gap:8}}>
                                <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"7px 10px",borderRadius:9,background:"color-mix(in srgb,var(--green) 8%,var(--surface))",border:"1.5px solid color-mix(in srgb,var(--green) 25%,var(--border))"}}>
                                  <span style={{fontSize:11,color:"var(--text2)"}}>Passed: <b style={{color:"var(--green)",fontFamily:"var(--font-mono)"}}>{sub.passedStudents.length}</b></span>
                                  {sub.passedStudents.length>0&&<Btn variant="ghost" size="sm"
                                    onClick={()=>setStudentModal({title:`${sub.name} · Passed`,students:sub.passedStudents,color:"var(--green)"})}>
                                    View
                                  </Btn>}
                                </div>
                                <div style={{flex:1,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"7px 10px",borderRadius:9,background:sub.failedStudents.length===0?"color-mix(in srgb,var(--green) 8%,var(--surface))":"color-mix(in srgb,var(--red) 8%,var(--surface))",border:`1.5px solid color-mix(in srgb,${sub.failedStudents.length===0?"var(--green)":"var(--red)"} 25%,var(--border))`}}>
                                  <span style={{fontSize:11,color:"var(--text2)"}}>Failed: <b style={{color:sub.failedStudents.length===0?"var(--green)":"var(--red)",fontFamily:"var(--font-mono)"}}>{sub.failedStudents.length}</b></span>
                                  {sub.failedStudents.length>0&&<Btn variant="ghost" size="sm"
                                    onClick={()=>setStudentModal({title:`${sub.name} · Failed`,students:sub.failedStudents,color:"var(--red)"})}>
                                    View
                                  </Btn>}
                                </div>
                              </div>
                            </div>
                          ))}
                        </Card>
                      )}
                    </>
                  );
                })()}
              </div>
            )}

            {/* ── MERIT ── */}
            {page==="settings"&&(
              <div>
                <div style={{fontSize:10,fontWeight:700,letterSpacing:"0.12em",color:"var(--sky)",marginBottom:6,display:"flex",alignItems:"center",gap:6}}>
                  <span style={{display:"inline-block",width:18,height:2,background:"linear-gradient(90deg,var(--sky),transparent)",borderRadius:2}}/>
                  ACCOUNT
                </div>
                <h1 style={{fontSize:26,fontWeight:800,letterSpacing:"-0.02em",marginBottom:20}}>Settings</h1>
                <Card style={{marginBottom:14}}>
                  <div style={{display:"flex",alignItems:"center",gap:14}}>
                    <div style={{width:52,height:52,borderRadius:16,background:"linear-gradient(135deg,var(--sky),var(--indigo))",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:22,color:"#fff",flexShrink:0}}>
                      {(userData?.name||user?.email||"?")[0].toUpperCase()}
                    </div>
                    <div>
                      <div style={{fontWeight:700,fontSize:16}}>{userData?.name||"—"}</div>
                      <div style={{color:"var(--text2)",fontSize:12,marginTop:2}}>{user?.email}</div>
                      {isAdmin&&<div style={{marginTop:4}}><Pill color="var(--yellow)">Admin</Pill></div>}
                    </div>
                  </div>
                </Card>
                <Card style={{marginBottom:14}}>
                  <div style={{fontSize:10,fontWeight:700,color:"var(--text2)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:14}}>Appearance</div>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div>
                      <div style={{fontWeight:600,fontSize:14}}>Dark Mode</div>
                      <div style={{color:"var(--text2)",fontSize:12,marginTop:2}}>Switch theme</div>
                    </div>
                    <button onClick={()=>setDark(d=>!d)} style={{width:48,height:26,borderRadius:13,border:"none",cursor:"pointer",background:dark?"var(--sky)":"var(--border)",position:"relative",transition:"background 0.2s",flexShrink:0}}>
                      <div style={{width:20,height:20,borderRadius:"50%",background:"#fff",position:"absolute",top:3,left:dark?24:3,transition:"left 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.3)"}}/>
                    </button>
                  </div>
                </Card>
                <Card style={{marginBottom:14}}>
                  <div style={{fontSize:10,fontWeight:700,color:"var(--text2)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:14}}>Your Data</div>
                  <div style={{display:"flex",gap:12}}>
                    <div style={{flex:1,textAlign:"center",padding:"12px 8px",background:"var(--surface2)",borderRadius:12}}>
                      <div style={{fontWeight:800,fontSize:22,color:"var(--sky)"}}>{projects.length}</div>
                      <div style={{fontSize:11,color:"var(--text2)",marginTop:2}}>Projects</div>
                    </div>
                    <div style={{flex:1,textAlign:"center",padding:"12px 8px",background:"var(--surface2)",borderRadius:12}}>
                      <div style={{fontWeight:800,fontSize:22,color:"var(--green)"}}>{projects.reduce((a,p)=>a+(p.students?.length||0),0)}</div>
                      <div style={{fontSize:11,color:"var(--text2)",marginTop:2}}>Students</div>
                    </div>
                  </div>
                </Card>
                <Btn variant="danger" onClick={signOut} style={{width:"100%"}}>Sign Out</Btn>
              </div>
            )}

            {page==="merit"&&(
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                {!activeProject||!sortedByRoll.length?(
                  <div style={{textAlign:"center",padding:"60px 0"}}><Card><div style={{color:"var(--text2)",textAlign:"center",fontSize:13,padding:"20px 0"}}>No students to display.</div></Card></div>
                ):(
                  <>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12,overflowX:"auto"}}>
                        {projects.map((p,i)=>(
                          <button key={p.id} onClick={()=>setActiveProjectIndex(i)} style={{whiteSpace:"nowrap",padding:"5px 12px",borderRadius:20,border:`1.5px solid ${i===activeProjectIndex?"var(--sky)":"var(--border)"}`,background:i===activeProjectIndex?"color-mix(in srgb,var(--sky) 15%,var(--surface))":"var(--surface)",color:i===activeProjectIndex?"var(--sky)":"var(--text2)",fontSize:11,fontWeight:600,cursor:"pointer"}}>{p.examName||p.id}</button>
                        ))}
                      </div>
                    <SectionHeader label="Rankings" title="Merit Table" subtitle={`${activeProject.examName} · sorted by ${meritSort==="merit"?"Merit":"Roll No."}`}/>
                    <div style={{display:"flex",gap:8,marginBottom:16,marginTop:-6}}>
                      <button onClick={()=>setMeritSort("roll")} style={{flex:1,padding:"8px 0",borderRadius:12,border:`1.5px solid ${meritSort==="roll"?"var(--sky)":"var(--border)"}`,background:meritSort==="roll"?"color-mix(in srgb,var(--sky) 15%,var(--surface))":"var(--surface2)",color:meritSort==="roll"?"var(--sky)":"var(--text2)",fontSize:12,fontWeight:700,cursor:"pointer"}}>Sort by Roll No.</button>
                      <button onClick={()=>setMeritSort("merit")} style={{flex:1,padding:"8px 0",borderRadius:12,border:`1.5px solid ${meritSort==="merit"?"var(--sky)":"var(--border)"}`,background:meritSort==="merit"?"color-mix(in srgb,var(--sky) 15%,var(--surface))":"var(--surface2)",color:meritSort==="merit"?"var(--sky)":"var(--text2)",fontSize:12,fontWeight:700,cursor:"pointer"}}>Sort by Merit</button>
                    </div>
                    {top3.length>0&&(
                      <div style={{marginBottom:20}}>
                        <div style={{fontSize:9.5,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--text2)",marginBottom:12}}>🏆 Top Performers</div>
                        <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
                          {[1,0,2].map(idx=>{
                            const s=top3[idx];
                            if(!s) return<div key={idx} style={{flex:1}}/>;
                            const mc=medalColors[idx];
                            const isFirst=idx===0;
                            return(
                              <div key={s.roll} style={{flex:1,background:`color-mix(in srgb,${mc} 8%,var(--surface))`,border:`1.5px solid color-mix(in srgb,${mc} 28%,var(--border))`,borderRadius:16,padding:isFirst?"20px 10px 16px":"14px 10px 14px",textAlign:"center",transform:isFirst?"translateY(-6px)":"none",boxShadow:isFirst?`0 8px 24px color-mix(in srgb,${mc} 18%,transparent)`:"none"}}>
                                <div style={{fontSize:isFirst?28:22,marginBottom:6}}>{medals[idx]}</div>
                                <div style={{fontWeight:800,fontSize:12.5,marginBottom:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",letterSpacing:"-0.01em"}}>{s.name}</div>
                                <div style={{color:"var(--text2)",fontSize:10,marginBottom:8}}>Roll: {s.roll}</div>
                                <div style={{display:"inline-block",padding:"3px 10px",borderRadius:10,background:`color-mix(in srgb,${mc} 18%,transparent)`,color:mc,fontSize:12,fontWeight:800,fontFamily:"var(--font-mono)"}}>GPA {s.gpa}</div>
                                <div style={{marginTop:6}}><Pill color={gradeColor(s.grade)}>{s.grade}</Pill></div>
                              </div>
                            );
                          })}
                        </div>
                      </div>
                    )}

                    <Card style={{overflowX:"auto",padding:0,marginBottom:14,borderRadius:18}}>
                      <table>
                        <thead><tr>
                          <th style={{paddingLeft:16}}>#</th>
                          <th>Name</th><th>Roll</th><th>Merit</th><th>GPA</th><th>Grade</th><th>Total</th>
                        </tr></thead>
                        <tbody>
                          {meritTableList.map((s,i)=>{
                            const rank=meritRanks[s.roll];
                            const rankColor=rank===1?"#f59e0b":rank===2?"#94a3b8":rank===3?"#cd7c2f":null;
                            return(
                              <tr key={s.roll}>
                                <td style={{paddingLeft:16,color:"var(--text2)",fontSize:12,fontFamily:"var(--font-mono)"}}>{i+1}</td>
                                <td style={{fontWeight:600,fontSize:13}}>{s.name}</td>
                                <td style={{fontFamily:"var(--font-mono)",color:"var(--text2)",fontSize:12}}>{s.roll}</td>
                                <td>{rankColor?(<span style={{display:"inline-flex",alignItems:"center",justifyContent:"center",width:26,height:26,borderRadius:"50%",fontSize:11,fontWeight:800,background:rankColor,color:"#fff"}}>{rank}</span>):(<span style={{color:"var(--text2)",fontSize:12,fontFamily:"var(--font-mono)"}}>#{rank}</span>)}</td>
                                <td style={{fontWeight:700,color:gradeColor(s.grade),fontFamily:"var(--font-mono)"}}>{s.gpa}</td>
                                <td><Pill color={gradeColor(s.grade)}>{s.grade}</Pill></td>
                                <td style={{fontFamily:"var(--font-mono)",fontSize:12.5}}>{typeof s.total==="number"?Math.round(s.total):s.total}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                      </table>
                    </Card>
                    <Btn onClick={()=>downloadMeritListPDF(activeProject,showToast,meritSort)} style={{width:"100%",padding:"14px"}} size="lg">⬇ Download Merit List PDF</Btn>
                  </>
                )}
              </div>
            )}
          </>
        )}
      </div>

      {fbReady&&authReady&&user&&!fbError&&!isAccountBlocked&&(
        <Dock page={page} go={go} isAdmin={isAdmin} onAdminOpen={()=>setShowAdmin(true)}/>
      )}
    </>
  );
}
