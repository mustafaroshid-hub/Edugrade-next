          ...
          <>
            ...
            {page==="home"&&(
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                ...
                {projectsLoading?([1,2,3].map(k=>(
                  <div key={k} style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:16,marginBottom:9,overflow:"hidden",padding:"14px 16px",display:"flex",gap:14,alignItems:"center"}}>
                    <div style={{width:46,height:46,borderRadius:14,background:"var(--border)",flexShrink:0,animation:"pulse 1.4s ease-in-out infinite"}}/>
                    <div style={{flex:1}}>
                      <div style={{height:14,borderRadius:6,background:"var(--border)",marginBottom:8,width:"60%",animation:"pulse 1.4s ease-in-out infinite"}}/>
                      <div style={{height:11,borderRadius:6,background:"var(--border)",width:"40%",animation:"pulse 1.4s ease-in-out infinite"}}/>
                    </div>
                  </div>
                ))):projects.length===0?(
                  <div style={{textAlign:"center",padding:"48px 24px",background:"var(--surface)",border:"1.5px dashed var(--border)",borderRadius:20}}>
                    <div style={{display:"flex",justifyContent:"center",marginBottom:16}}>
                      <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="var(--sky)" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" style={{opacity:0.7}}>
                        <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/>
                        <line x1="12" y1="18" x2="12" y2="12"/><line x1="9" y1="15" x2="15" y2="15"/>
                      </svg>
                    </div>
                    <div style={{fontWeight:700,fontSize:16,marginBottom:8}}>No projects yet</div>
                    <div style={{fontSize:13,color:"var(--text2)",marginBottom:20,lineHeight:1.5}}>Create your first grading project<br/>to start entering student marks</div>
                    ...
                    <div key={p.id} className="project-card" style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:16,marginBottom:9,overflow:"hidden"}}>
                      <div style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px"}}>
                        <div style={{width:46,height:46,borderRadius:14,flexShrink:0,background:"color-mix(in srgb,var(--sky) 10%,var(--surface2))",border:"1.5px solid color-mix(in srgb,var(--sky) 18%,var(--border))",display:"flex",alignItems:"center",justifyContent:"center"}}>
                          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--sky)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      {activeProject.students.length===0?(
                        <div style={{textAlign:"center",padding:"36px 24px"}}>
                          <div style={{display:"flex",justifyContent:"center",marginBottom:12}}>
                            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="var(--text2)" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" style={{opacity:0.4}}>
                              <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/>
                              <path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/>
                            </svg>
                          </div>
                          <div style={{fontWeight:600,fontSize:13,marginBottom:4}}>No students yet</div>
                          <div style={{fontSize:12,color:"var(--text2)"}}>Use the Entry tab to add students</div>
                        </div>
                      ):(
                        <table>
                          <thead><tr>
                          ...
                          <tbody>
                            {[...activeProject.students].sort((a,b)=>String(a.roll).localeCompare(String(b.roll),undefined,{numeric:true})).map(s=>(
                              <tr key={s.roll} style={{cursor:"pointer"}} onClick={()=>setViewStudent(s)}>
                                <td style={{fontWeight:600,paddingLeft:16,maxWidth:100,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}} title={s.name}>{s.name}</td>
                                <td style={{fontFamily:"var(--font-mono)",color:"var(--text2)",fontSize:12}}>{s.roll}</td>
                                <td style={{fontWeight:700,color:gradeColor(s.grade),fontFamily:"var(--font-mono)"}}>{s.gpa}</td>
                                <td><Pill color={gradeColor(s.grade)}>{s.grade}</Pill></td>
              ...
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                {!activeProject||!sortedByRoll.length?(
                  ...
                  <>
                    ...
                    <Btn onClick={()=>downloadMeritListPDF(activeProject,showToast)} style={{width:"100%"}} size="sm" variant="ghost">
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{marginRight:6,verticalAlign:"middle"}}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
