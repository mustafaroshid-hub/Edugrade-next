...
const AdminUserWorkspace = ({ firebase, showToast, targetUser, onBack }) => {
  ...
  function calcSubject(entry, ss, proj) {
    const skipCA=!!ss.skipCA, skipMCQ=!!ss.skipMCQ, skipCQ=!!ss.skipCQ;
    const caFull  = skipCA  ? 0 : Number(ss.caFull??30);
    const examFull= Number(ss.examFull??100), examPct=Number(ss.examPct??70);
    ...
    const ca   = skipCA  ? 0 : Math.round(na*aoof>0?(assignTotal/(na*aoof))*caFull:0);
  ...
  const createProject = async () => {
    ...
    try {
      const finalSettings = Array(n).fill(null).map((_, i) => ({
        ...
        skipCA:  !!subjectSettings[i]?.skipCA,
        skipMCQ: !!subjectSettings[i]?.skipMCQ,
        skipCQ:  !!subjectSettings[i]?.skipCQ,
      }));
      const np = {
                    ...
                    return (
                      <div key={i} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 12, padding: "12px 14px" }}>
                        ...
                        <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
                          {[{k:"skipCA",l:"Skip CA",col:"var(--indigo)"},{k:"skipMCQ",l:"Skip MCQ",col:"var(--sky)"},{k:"skipCQ",l:"Skip CQ",col:"var(--amber)"}].map(({k,l,col}) => (
                            <label key={k} style={{ display:"flex",alignItems:"center",gap:5,fontSize:11,fontWeight:700,cursor:"pointer",padding:"4px 10px",borderRadius:8,border:`1.5px solid ${ss[k]?col:"var(--border)"}`,background:ss[k]?`color-mix(in srgb,${col} 12%,var(--surface))`:"var(--surface3)",color:ss[k]?col:"var(--text3)",transition:"all 0.15s" }}>
                              <input type="checkbox" checked={!!ss[k]} onChange={e=>{ const u=[...subjectSettings]; u[i]={...(u[i]||{caFull:30,examFull:100,examPct:70}),[k]:e.target.checked}; setSubjectSettings(u); }} style={{width:"auto",accentColor:col,cursor:"pointer"}}/>
                              {l}
                            </label>
                          ))}
                        </div>
                        ...
                        <div key={si} style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: 14, overflow: "hidden" }}>
                          ...
                          <div style={{ padding: "8px 12px", display: "flex", flexDirection: "column", gap: 6 }}>
                            ...
                            <div style={{ fontSize: 11, color: "var(--text2)", display: "flex", gap: 10, flexWrap: "wrap", paddingTop: 4, borderTop: "1px solid var(--border)" }}>
                              {!ss.skipCA&&<span>CA: <b style={{ color: "var(--indigo)", fontFamily: "var(--font-mono)" }}>{r.ca}</b></span>}
                              ...
                              <span>Total: <b style={{ color: "var(--green)", fontFamily: "var(--font-mono)" }}>{r.ab}/{Math.round(r.subFull)}</b></span>
                            </div>
                          </div>
                        </div>
                      );
                        ...
                        {Array(Number(subjectCount)).fill(null).map((_,i)=>{
                          ...
                          return(
                            <div key={i} style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:12,padding:"12px 14px"}}>
                              ...
                              <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>
                                {[{k:"skipCA",l:"Skip CA",col:"var(--indigo)"},{k:"skipMCQ",l:"Skip MCQ",col:"var(--sky)"},{k:"skipCQ",l:"Skip CQ",col:"var(--amber)"}].map(({k,l,col})=>(
                                  <label key={k} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,fontWeight:700,cursor:"pointer",padding:"4px 10px",borderRadius:8,border:`1.5px solid ${ss[k]?col:"var(--border)"}`,background:ss[k]?`color-mix(in srgb,${col} 12%,var(--surface2))`:"",color:ss[k]?col:"var(--text3)",transition:"all 0.15s"}}>
                                    <input type="checkbox" checked={!!ss[k]} onChange={e=>updateSubjectSetting(i,k,e.target.checked)} style={{width:"auto",cursor:"pointer"}}/>
                                    {l}
                                  </label>
                                ))}
                              </div>
                              ...
                              <div style={{marginTop:8,fontSize:11,color:"var(--text2)",display:"flex",gap:12,flexWrap:"wrap"}}>
                                {!ss.skipCA&&<span>Max CA: <b style={{color:"var(--indigo)"}}>{ss.caFull??30}</b></span>}
                                {(!ss.skipMCQ||!ss.skipCQ)&&<span>Max B: <b style={{color:"var(--amber)"}}>{((Number(ss.examPct)||70)/100*(Number(ss.examFull)||100)).toFixed(0)}</b></span>}
                                <span>Total: <b style={{color:"var(--sky)"}}>{Math.round((ss.skipCA?0:Number(ss.caFull??30))+((Number(ss.examPct)||70)/100)*Number(ss.examFull||100))}</b></span>
                              </div>
                            </div>
                          );
                        })}
                              ...
                              <div style={{padding:"8px 12px",display:"flex",flexDirection:"column",gap:6}}>
                                {!ss.skipCA&&Array(na).fill(null).map((_,ai)=>(
                                  <div key={ai} style={{display:"flex",alignItems:"center",gap:10}}>
                                    ...
                                    <input type="number" placeholder="0" min={0} max={aoof}
                                      value={(entry.assignments||[])[ai]??""}
                                      onChange={e=>{setSubjectEntries(prev=>prev.map((en,s)=>{if(s!==si)return en;const a=[...(en.assignments||[])];a[ai]=e.target.value;return{...en,assignments:a};}));}}
                                      style={{flex:1,textAlign:"center"}}/>
                                    <span style={{fontSize:11,color:"var(--text3)",minWidth:28}}>/{aoof}</span>
                                  </div>
                                ))}
                                ...
                                <div style={{fontSize:11,color:"var(--text2)",display:"flex",gap:10,flexWrap:"wrap",paddingTop:4,borderTop:"1px solid var(--border)"}}>
                                  ...
                                  {(!ss.skipMCQ||!ss.skipCQ)&&<span>Exam: <b style={{color:"var(--sky)",fontFamily:"var(--font-mono)"}}>{r.rawExam}</b></span>}
                                  <span>B: <b style={{color:"var(--amber)",fontFamily:"var(--font-mono)"}}>{r.b}</b></span>
                                  <span style={{fontWeight:700}}>Total: <b style={{color:"var(--text)",fontFamily:"var(--font-mono)"}}>{r.ab}/{Math.round(r.subFull)}</b></span>
