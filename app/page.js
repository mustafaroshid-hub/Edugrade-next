...
const downloadMarksheet = async (student, project, showToast) => {
  ...
  // Header accent color based on student grade
  ...
  [
    ...
    const isFail = value === "FAIL" || (label === "GPA" && parseFloat(value) === 0) || (label === "LG" && value === "F");
    if (large) doc.setTextColor(...(isFail ? RED : [14, 100, 180])); else doc.setTextColor(...DARK);
  ...
  gpCols.forEach(({ label, value }, i) => {
    ...
    const isFailVal = value === "F" || (label === "GPA" && parseFloat(value) === 0);
    doc.setFont("helvetica", "bold"); doc.setFontSize(10); doc.setTextColor(...(isFailVal ? RED : [14, 100, 180]));
...
export default function Home() {
  ...
  const addStudent=async()=>{
    ...
    const {gpa,grade,total,subResults,status,hasZeroGP}=calcStudent(subjectEntries,activeProject);
    const obj={
      name:studentName.trim(), roll:roll.trim(),
      gpa, grade, total, status, hasZeroGP,
