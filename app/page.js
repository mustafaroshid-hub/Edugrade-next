...
const AdminPanel = ({ firebase, showToast, onClose }) => {
  ...
  // view mode: null | { user, projects }
  const [viewData, setViewData]     = useState(null);
  const [selProject, setSelProject] = useState(null);
  // edit mode: { userId, projectId, student }
  ...
  const toggleBlock = async (u) => {
    ...
    try {
      await firebase.db.collection("users").doc(u.id).update({ blocked: newBlocked });
      setUsers(prev => prev.map(x => x.id === u.id ? { ...x, blocked: newBlocked } : x));
      if (viewData?.user?.id === u.id) setViewData(vd => ({ ...vd, user: { ...vd.user, blocked: newBlocked } }));
  ...
  const loadUserData = async (u) => {
    try {
      const snap = await firebase.db.collection("users").doc(u.id)
        .collection("projects").orderBy("createdAt", "asc").get();
      const projects = snap.docs.map(d => ({ id: d.id, ...d.data() }));
      setViewData({ user: u, projects });
      setSelProject(projects[0] ?? null);
    } catch (e) { showToast("✗ " + e.message); }
  };

  const openEditStudent = (userId, projectId, student) => {
    setEditStudent({ userId, projectId, student });
    setEditName(student.name);
    setEditRoll(String(student.roll));
  };

  const saveEditStudent = async () => {
    if (!editName.trim() || !editRoll.trim()) return showToast("⚠ Name and roll required");
    setSaving(true);
    try {
      const { userId, projectId, student } = editStudent;
      const proj = viewData.projects.find(p => p.id === projectId);
      const updatedStudents = proj.students.map(s =>
        s.roll === student.roll ? { ...s, name: editName.trim(), roll: editRoll.trim() } : s
      );
      await firebase.db.collection("users").doc(userId)
        .collection("projects").doc(projectId).update({ students: updatedStudents });
      // update local state
      setViewData(vd => ({
        ...vd,
        projects: vd.projects.map(p => p.id === projectId ? { ...p, students: updatedStudents } : p),
      }));
      setSelProject(prev => prev?.id === projectId ? { ...prev, students: updatedStudents } : prev);
  ...
  const deleteStudent = async (userId, projectId, roll) => {
    try {
      const proj = viewData.projects.find(p => p.id === projectId);
      const updatedStudents = proj.students.filter(s => s.roll !== roll);
      await firebase.db.collection("users").doc(userId)
        .collection("projects").doc(projectId).update({ students: updatedStudents });
      setViewData(vd => ({
        ...vd,
  ...
  if (editStudent) {
    return (
      <div style={{ position:"fixed",inset:0,zIndex:6000,background:"rgba(0,0,0,0.75)",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(8px)",padding:"0 16px" }}>
        <div style={{ background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:22,width:"100%",maxWidth:380,padding:24,animation:"up 0.25s ease",boxShadow:"0 24px 64px rgba(0,0,0,0.4)" }}>
          <div style={{ fontSize:9.5,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"var(--sky)",marginBottom:6 }}>✏️ Edit Student</div>
          <h3 style={{ fontWeight:800,fontSize:18,marginBottom:20,letterSpacing:"-0.02em" }}>Modify Student Data</h3>
          ...
          <div style={{ display:"flex",gap:10 }}>
            <Btn variant="ghost" onClick={() => setEditStudent(null)} style={{ flex:1 }}>Cancel</Btn>
            <Btn onClick={saveEditStudent} loading={saving} style={{ flex:1 }}>Save Changes</Btn>
          </div>
        </div>
      </div>
    );
  ...
  // ── User Detail View ──────────────────────────────────────────────────────────
  if (viewData) {
    const u = viewData.user;
    const projects = viewData.projects;
    const sortedStudents = selProject
      ? [...selProject.students].sort((a,b) => String(a.roll).localeCompare(String(b.roll), undefined, { numeric:true }))
      : [];
    const meritRanks = selProject ? getMeritRank(selProject.students) : {};
    const stats = (() => {
      if (!selProject?.students.length) return { rate:"0.0", avg:"0.00" };
      const pass = selProject.students.filter(s => s.status === "PASS").length;
      return {
        rate: ((pass / selProject.students.length) * 100).toFixed(1),
        avg: (selProject.students.reduce((a,s) => a+s.gpa,0) / selProject.students.length).toFixed(2),
      };
    })();

    return (
      <div style={{ position:"fixed",inset:0,zIndex:5000,background:"rgba(0,0,0,0.72)",display:"flex",alignItems:"flex-end",justifyContent:"center",backdropFilter:"blur(8px)" }}
          ...
          <div style={{ padding:"18px 20px 0",borderBottom:"1.5px solid var(--border)" }}>
            <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12 }}>
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ fontSize:9.5,fontWeight:700,letterSpacing:"0.12em",textTransform:"uppercase",color:"var(--sky)",marginBottom:4 }}>👤 User Profile</div>
                <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                  <div style={{ width:42,height:42,borderRadius:14,background:u.blocked?"color-mix(in srgb,var(--red) 18%,var(--surface2))":"linear-gradient(135deg,#38bdf8,#0284c7)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,fontWeight:800,color:"#fff",flexShrink:0 }}>
                    {u.blocked ? "🚫" : (u.name||"?")[0].toUpperCase()}
                  </div>
                  <div>
                    <div style={{ fontWeight:800,fontSize:17,letterSpacing:"-0.02em",display:"flex",alignItems:"center",gap:6 }}>
                      {u.name}
                      {u.email===ADMIN_EMAIL&&<Pill color="var(--yellow)">Admin</Pill>}
                      {u.blocked&&<Pill color="var(--red)">Blocked</Pill>}
                    </div>
                    <div style={{ color:"var(--text2)",fontSize:11.5 }}>{u.email}</div>
                  </div>
                </div>
                {/* Block / Unblock button */}
                {u.email !== ADMIN_EMAIL && (
                  <div style={{ marginTop:10,marginBottom:4 }}>
                    <Btn variant={u.blocked?"green":"danger"} size="sm" onClick={() => toggleBlock(u)}>
                      {u.blocked ? "✓ Unblock User" : "🚫 Block User"}
                    </Btn>
                  </div>
                )}
              </div>
              <button onClick={() => setViewData(null)} style={{ width:34,height:34,borderRadius:"50%",border:"1.5px solid var(--border)",background:"var(--surface2)",color:"var(--text2)",fontSize:17,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>×</button>
            </div>
            ...
            {projects.length > 0 && (
              <div style={{ display:"flex",gap:6,overflowX:"auto",paddingBottom:12,scrollbarWidth:"none" }}>
                {projects.map(p => (
                  <button key={p.id} onClick={() => setSelProject(p)} style={{ padding:"6px 14px",borderRadius:9,border:"1.5px solid var(--border)",background:selProject?.id===p.id?"linear-gradient(135deg,#38bdf8,#0284c7)":"var(--surface2)",color:selProject?.id===p.id?"#fff":"var(--text2)",fontSize:12,fontWeight:600,whiteSpace:"nowrap",flexShrink:0,cursor:"pointer" }}>
                    {p.examName}
                  </button>
                ))}
              </div>
            ...
            {projects.length === 0 ? (
              ...
              <>
                ...
                <div style={{ display:"flex",gap:8,marginBottom:14 }}>
                  {[
                    { label:"Students", value:selProject.students.length, color:"var(--sky)" },
                    { label:"Pass Rate", value:`${stats.rate}%`, color:"var(--green)" },
                    { label:"Avg GPA", value:stats.avg, color:"var(--indigo)" },
                  ].map(({ label,value,color }) => (
                    <div key={label} style={{ background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:12,padding:"10px 12px",flex:1,textAlign:"center" }}>
                      ...
                      <div style={{ fontSize:19,fontWeight:800,color,fontFamily:"var(--font-mono)" }}>{value}</div>
                    </div>
                  ))}
                </div>
                ...
                {sortedStudents.length === 0 ? (
                  ...
                  <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
                    {sortedStudents.map(s => {
                      ...
                      return (
                        <div key={s.roll} style={{ background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:14,padding:"12px 14px" }}>
                          <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:8 }}>
                            <div style={{ width:34,height:34,borderRadius:10,background:"linear-gradient(135deg,var(--sky),var(--indigo))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:800,color:"#fff",flexShrink:0 }}>
                              {rankColor
                                ? <span style={{ fontSize:11,background:rankColor,borderRadius:"50%",width:24,height:24,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff" }}>{rank}</span>
                                : (s.name||"?")[0].toUpperCase()}
                            </div>
                            <div style={{ flex:1,minWidth:0 }}>
                              <div style={{ fontWeight:700,fontSize:13,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{s.name}</div>
                              <div style={{ color:"var(--text2)",fontSize:11 }}>Roll: {s.roll} · Merit #{rank}</div>
                            </div>
                            <div style={{ display:"flex",gap:6 }}>
                              <Btn variant="sky" size="sm" onClick={() => openEditStudent(u.id, selProject.id, s)}>✏️</Btn>
                              <Btn variant="danger" size="sm" onClick={() => {
                                if (window.confirm(`Delete ${s.name}? This cannot be undone.`)) deleteStudent(u.id, selProject.id, s.roll);
                              }}>🗑</Btn>
                            </div>
                          </div>
                          <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
                            <div style={{ background:"var(--surface3)",borderRadius:8,padding:"4px 10px",fontSize:11,fontFamily:"var(--font-mono)",fontWeight:700,color:gradeColor(s.grade) }}>GPA {s.gpa}</div>
                            <Pill color={gradeColor(s.grade)}>{s.grade}</Pill>
                            <Pill color={s.status==="PASS"?"var(--green)":"var(--red)"}>{s.status}</Pill>
                            <div style={{ background:"var(--surface3)",borderRadius:8,padding:"4px 10px",fontSize:11,fontFamily:"var(--font-mono)",color:"var(--text2)" }}>Total: {s.total}</div>
                          </div>
                        </div>
                      );
                    })}
      ...
      <div style={{ background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:"22px 22px 0 0",width:"100%",maxWidth:560,maxHeight:"88vh",display:"flex",flexDirection:"column",animation:"up 0.3s ease",boxShadow:"0 -16px 60px rgba(0,0,0,0.28)" }}>
        ...
        <div style={{ padding:"20px 22px 14px",borderBottom:"1.5px solid var(--border)" }}>
          ...
          {!loading && !permError && (
            <div style={{ display:"flex",gap:8,marginBottom:14 }}>
              {[
                { label:"Total Users", value:users.length, color:"var(--sky)" },
        ...
        <div style={{ flex:1,overflowY:"auto",padding:16 }}>
          {loading ? <Spinner /> : permError ? (
            ...
            <div style={{ padding:"24px 4px" }}>
              ...
              <p style={{ color:"var(--text2)",fontSize:12.5,lineHeight:1.7,marginBottom:16,textAlign:"center" }}>
                Your Firestore security rules are blocking admin access to user data. Update your rules in the Firebase Console:
              </p>
  ...
  match /databases/{database}/documents {
    ...
    match /adminNotifications/{docId} {
      allow read, write: if isAdmin() || request.auth != null;
              ...
              <p style={{ color:"var(--text2)",fontSize:11.5,marginTop:12,textAlign:"center",lineHeight:1.6 }}>
                Go to <strong>Firebase Console → Firestore → Rules</strong>, paste the above, then click <strong>Publish</strong>.
              </p>
            ...
            filteredUsers.map(u => (
              <div key={u.id} style={{ display:"flex",alignItems:"center",gap:12,padding:"12px 14px",borderRadius:14,marginBottom:8,background:u.blocked?"color-mix(in srgb,var(--red) 5%,var(--surface2))":"var(--surface2)",border:`1.5px solid ${u.blocked?"color-mix(in srgb,var(--red) 22%,var(--border))":"var(--border)"}` }}>
                {/* Avatar */}
                <div style={{ width:40,height:40,borderRadius:13,flexShrink:0,background:u.blocked?"color-mix(in srgb,var(--red) 18%,var(--surface))":"linear-gradient(135deg,#38bdf8,#0284c7)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:u.blocked?18:15,fontWeight:800,color:"#fff" }}>
                  {u.blocked ? "🚫" : (u.name||"?")[0].toUpperCase()}
                </div>
                ...
                <div style={{ flex:1,minWidth:0 }}>
                  ...
                  <div style={{ color:"var(--text2)",fontSize:11,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",marginTop:1 }}>{u.email}</div>
                </div>

                {/* Actions */}
                <div style={{ display:"flex",flexDirection:"column",gap:5,flexShrink:0 }}>
                  <Btn variant="sky" size="sm" onClick={() => loadUserData(u)} style={{ whiteSpace:"nowrap" }}>
                    📊 View
                  </Btn>
                  {u.email !== ADMIN_EMAIL && (
                    <Btn variant={u.blocked?"green":"danger"} size="sm" onClick={() => toggleBlock(u)} style={{ whiteSpace:"nowrap" }}>
