...
const downloadMarksheet = async (student, project, showToast) => {
  ...
  sortedGi.forEach((row, idx) => {
    ...
    const minLabel = nextRow ? `${nextRow.min + 1} – ${row.min > 99 ? 100 : row.min}` : `00 – ${row.min}`;
  ...
  doc.text(String(student.total != null ? Math.round(student.total) : "—"), W / 2 + 28, infoY + 20);
  ...
  const tableBody = subjectNames.map((subName, i) => {
    ...
    // MCQ/Oral and Creative/Written: if rawExam is a number, split 50/50 as approximation
    // In practice the app may store them separately; use rawExam as Total (exam part)
    const mcqOral  = sr.mcqOral   != null ? sr.mcqOral         : (sr.rawExam != null ? Math.round(sr.rawExam * 0.55) : "-");
    const creative = sr.creative  != null ? sr.creative        : (sr.rawExam != null ? Math.round(sr.rawExam * 0.45) : "-");
    const total    = sr.rawExam   != null ? sr.rawExam         : "-";   // total exam marks before scaling
    const b        = sr.b    != null ? Math.round(sr.b)        : "-";   // scaled exam mark
    const ab       = sr.ab   != null ? Math.round(sr.ab)       : "-";   // A+B final
    const gp       = sr.gp   != null ? sr.gp                   : (ab !== "-" ? gpFn(ab) : "-");
    ...
    didParseCell: (data) => {
      if (data.section === "body") {
        if (data.column.index === 7) {
          ...
          data.cell.styles.textColor = pct >= 80 ? GREEN : v < 33 ? RED : DARK;
        }
      }
    },
  ...
  [
    ...
    { label: "Total GP", value: String(student.gpa), large: true },
