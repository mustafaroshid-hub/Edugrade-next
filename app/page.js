  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        try {
          const nSnap = await firebase.db.collection("adminNotifications").orderBy("createdAt","desc").get();
        ...
        <div style={{ padding: "20px 22px 0", borderBottom: "1.5px solid var(--border)", paddingBottom: 0 }}>
          ...
          <div style={{ display: "flex", gap: 4, marginBottom: -1 }}>
            {[
              ...
              { id: "notifs", label: `Notifications${notifications.filter(n=>!n.read).length>0?` (${notifications.filter(n=>!n.read).length})`:""}` },
            ].map(t => (
              <button key={t.id} onClick={() => setTab(t.id)} style={{
                padding: "9px 16px", borderRadius: "10px 10px 0 0",
                border: "1.5px solid var(--border)",
                borderBottom: tab === t.id ? "1.5px solid var(--surface)" : "1.5px solid var(--border)",
                background: tab === t.id ? "var(--surface)" : "var(--surface2)",
          ...
          {loading ? <Spinner /> : (
            <>
              {tab === "users" && (
                <>
                  {users.length===0&&(
                    <div style={{textAlign:"center",padding:"40px 0",color:"var(--text2)",fontSize:13}}>No users found</div>
                  )}
                  {users.map(u => (
                    <div key={u.id} style={{
                      display: "flex", alignItems: "center", gap: 12,
                      ...
                      }}>{u.blocked ? "🚫" : (u.name?.[0]?.toUpperCase() || "👤")}</div>
                ...
                <>
                  {notifications.length > 0 && (
                    <div style={{display:"flex",justifyContent:"flex-end",marginBottom:10}}>
                      <Btn variant="ghost" onClick={markAllRead} size="sm">✓ Mark all read</Btn>
                    </div>
                  )}
                  {notifications.length === 0 && (
                    <div style={{textAlign:"center",padding:"40px 0",color:"var(--text2)",fontSize:13}}>No notifications</div>
                  )}
                  {notifications.map(n => (
                    <div key={n.id} style={{
                      padding:"12px 14px", borderRadius:12, marginBottom:8,
                      background: n.read ? "var(--surface2)" : "color-mix(in srgb,var(--sky) 8%,var(--surface2))",
                      border:`1.5px solid ${n.read ? "var(--border)" : "color-mix(in srgb,var(--sky) 30%,var(--border))"}`,
                      display:"flex", gap:10, alignItems:"flex-start",
                    }}>
                      <div style={{fontSize:18,flexShrink:0}}>{n.type==="newUser"?"🆕":"🔔"}</div>
                      <div style={{flex:1,minWidth:0}}>
                        <div style={{fontWeight:700,fontSize:13}}>{n.name}</div>
                        <div style={{color:"var(--text2)",fontSize:11.5,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{n.email}</div>
                        <div style={{color:"var(--text3)",fontSize:10.5,marginTop:3}}>{n.createdAt ? new Date(n.createdAt).toLocaleString() : ""}</div>
                      </div>
                      {!n.read && <div style={{width:7,height:7,borderRadius:"50%",background:"var(--sky)",flexShrink:0,marginTop:4}}/>}
                    </div>
                  ))}
                </>
              )}
  ...
  useEffect(() => {
    try {
      ...
      const fbAuth = {
        ...
        deleteUser: () => import('firebase/auth').then(({ deleteUser }) => deleteUser(auth.currentUser)),
...
export default function Home() {
  ...
  function calcStudent(entries, proj){
    ...
    const rawGpa=Math.min(parseFloat(avgGP.toFixed(2)),5);
    const gpa=fail?0:rawGpa;
    const grade=fail?"F":gradeFn(rawGpa);
  ...
  const deleteAccount=async()=>{
    try{
      for(const p of projects){
        await firebase.db.collection("users").doc(user.uid).collection("projects").doc(p.id).delete();
      }
      await firebase.db.collection("users").doc(user.uid).delete();
      await firebase.auth.deleteUser();
          ...
          <>
              ...
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                {!activeProject?(
                  ...
                  <>
                    ...
                    {/* Student Card */}
                    {viewStudent&&(
                      <Card style={{border:`1.5px solid color-mix(in srgb,${gradeColor(viewStudent.grade)} 35%,var(--border))`,background:`color-mix(in srgb,${gradeColor(viewStudent.grade)} 3%,var(--surface))`,animation:"up 0.25s ease",borderRadius:20,marginBottom:12,overflow:"hidden",padding:0}}>
                        {/* Header accent bar */}
                        <div style={{height:3,background:`linear-gradient(90deg,${gradeColor(viewStudent.grade)},color-mix(in srgb,${gradeColor(viewStudent.grade)} 40%,transparent))`}}/>
                        <div style={{padding:"16px 18px"}}>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
                          <div style={{flex:1}}>
                            <div style={{fontWeight:800,fontSize:20,letterSpacing:"-0.02em",lineHeight:1.1}}>{viewStudent.name}</div>
                            <div style={{color:"var(--text2)",fontSize:12,marginTop:3,fontFamily:"var(--font-mono)"}}>Roll: {viewStudent.roll}</div>
                          </div>
                          <div style={{textAlign:"right",display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5}}>
                            <div style={{fontSize:38,fontWeight:900,color:gradeColor(viewStudent.grade),fontFamily:"var(--font-mono)",lineHeight:1,letterSpacing:"-0.04em"}}>{viewStudent.grade}</div>
                        ...
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16}}>
                          {[
                            ...
                            <div key={label} style={{background:"var(--surface2)",border:"1.5px solid var(--border)",borderRadius:14,padding:"12px 10px",textAlign:"center",position:"relative",overflow:"hidden"}}>
                              <div style={{position:"absolute",top:0,left:0,right:0,height:2,background:`color-mix(in srgb,${color} 50%,transparent)`}}/>
                              <div style={{fontSize:9.5,color:"var(--text2)",fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",marginBottom:5}}>{label}</div>
                              <div style={{fontSize:big?28:20,fontWeight:800,color,fontFamily:"var(--font-mono)",lineHeight:1}}>{value}</div>
                            </div>
                          ))}
                        </div>
                        ...
                        {(viewStudent.subjectResults||[]).length>0&&(
                          <div style={{marginBottom:16}}>
                            <div style={{fontSize:9.5,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--text2)",marginBottom:9,display:"flex",alignItems:"center",gap:6}}>
                              <span style={{display:"inline-block",width:14,height:2,background:"linear-gradient(90deg,var(--sky),transparent)",borderRadius:2}}/>
                              Subject Breakdown
                            </div>
                            <div style={{display:"flex",flexDirection:"column",gap:8}}>
                              {(viewStudent.subjectResults||[]).map((sr,i)=>{
                                ...
                                const subFull=Number(ss.caFull)+((Number(ss.examPct)||70)/100)*Number(ss.examFull||100);
                                const pct=sr.pct!=null?Math.min(sr.pct,100):0;
                                const gc=gradeColor(sr.grade);
                                return(
                                  <div key={i} style={{background:"var(--surface2)",border:`1.5px solid ${sr.pass?"var(--border)":"color-mix(in srgb,var(--red) 25%,var(--border))"}`,borderRadius:13,overflow:"hidden"}}>
                                    <div style={{padding:"10px 13px"}}>
                                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7}}>
                                      <span style={{fontWeight:700,fontSize:13}}>{subName}</span>
                                      <div style={{display:"flex",alignItems:"center",gap:7}}>
                                        <span style={{fontWeight:800,fontSize:14,color:gc,fontFamily:"var(--font-mono)"}}>{sr.grade}</span>
                                        <span style={{fontWeight:700,fontFamily:"var(--font-mono)",fontSize:13,color:"var(--sky)"}}>{sr.ab!=null?Math.round(sr.ab):"-"}<span style={{fontSize:10,color:"var(--text3)",fontWeight:500}}>/{Math.round(subFull)}</span></span>
                                    ...
                                    <div style={{display:"flex",gap:14,fontSize:11,color:"var(--text2)"}}>
                                      ...
                                      <span style={{marginLeft:"auto",color:sr.pass?"var(--green)":"var(--red)",fontWeight:700,fontFamily:"var(--font-mono)"}}>{pct.toFixed(1)}%</span>
                                    </div>
                                    </div>
                                    {/* Progress bar at bottom */}
                                    <div style={{height:3,background:"var(--border)"}}>
                                      <div style={{height:3,width:`${pct}%`,background:sr.pass?`linear-gradient(90deg,var(--green),color-mix(in srgb,var(--green) 60%,var(--sky)))`:`linear-gradient(90deg,var(--red),color-mix(in srgb,var(--red) 60%,var(--amber)))`,transition:"width 0.6s ease"}}/>
                                    </div>
                                  </div>
                                );
                              })}
            ...
            {page==="settings"&&(
              <div>
                ...
                <Btn variant="danger" onClick={signOut} style={{width:"100%",marginBottom:12}}>Sign Out</Btn>
                <Card style={{marginBottom:80,border:"1.5px solid color-mix(in srgb,var(--red) 30%,var(--border))"}}>
                  <div style={{fontSize:10,fontWeight:700,color:"var(--red)",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>⚠ Danger Zone</div>
                  <div style={{fontSize:12,color:"var(--text2)",marginBottom:12,lineHeight:1.5}}>Permanently delete your account and all data. Cannot be undone.</div>
                  <Btn variant="danger" onClick={()=>setConfirmModal({
                    title:"Delete Account",
                    msg:"This will permanently delete your account, all projects, and all student data. This CANNOT be undone.",
                    ok:"Delete Forever",
                    onOk:deleteAccount,
                  })} style={{width:"100%"}}>🗑 Delete My Account</Btn>
                </Card>
              </div>
            )}
