...
export default function Home() {
  ...
  const deleteProject=async(id)=>{
    setConfirmModal({msg:"Delete this project and ALL its data? This cannot be undone.",onOk:async()=>{ setConfirmModal(null);
    try{
  ...
  const deleteStudent=async(rollNo)=>{
    if(!activeProject) return;
    setConfirmModal({msg:"Remove this student? This cannot be undone.",onOk:async()=>{
  ...
  return (
    <>
      ...
      {confirmModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:"0 24px"}}>
          <div style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:20,padding:24,width:"100%",maxWidth:340}}>
            <div style={{fontWeight:700,fontSize:16,marginBottom:8}}>⚠ Are you sure?</div>
            <div style={{color:"var(--text2)",fontSize:13,marginBottom:20}}>{confirmModal.msg}</div>
            <div style={{display:"flex",gap:10}}>
              <Btn variant="ghost" onClick={()=>setConfirmModal(null)} style={{flex:1}}>Cancel</Btn>
              <Btn variant="danger" onClick={confirmModal.onOk} style={{flex:1}}>Delete</Btn>
            </div>
          </div>
        </div>
      )}
            ...
            {page==="home"&&(
              <div style={{animation:"up 0.28s ease",paddingTop:24}}>
                ...
                {projects.length===0?(
                  ...
                  return(
                    <div key={p.id} className="project-card" style={{background:"var(--surface)",border:"1.5px solid var(--border)",borderRadius:16,marginBottom:9,overflow:"hidden",borderLeft:`3px solid ${["#38bdf8","#a78bfa","#34d399","#fb923c","#f472b6","#facc15","#60a5fa","#4ade80"][i%8]}`}}>
                      ...
                      <Card style={{border:`1.5px solid color-mix(in srgb,${gradeColor(viewStudent.grade)} 28%,var(--border))`,background:`color-mix(in srgb,${gradeColor(viewStudent.grade)} 4%,var(--surface))`,animation:"up 0.25s ease",borderRadius:18,marginBottom:12}}>
                        ...
                        <div style={{display:"flex",gap:9}}>
                          <Btn onClick={()=>{
                            ...
                            }} style={{flex:1}} size="sm" variant="ghost">✏ Edit</Btn>
